...
function Matrix3x3.__tostring(m)
  return string.format("Matrix3x3(%f,%f,%f,%f,%f,%f,%f,%f,%f)", m[1], m[2], m[3], m[4], m[5], m[6], m[7], m[8], m[9])
end

function Matrix3x3.SetAxisAngle(m, axis, rad)
  local s, c, vx, vy, vz, xx, yy, zz, xy, yz, zx, xs, ys, zs, one_c
  s = sin(rad)
  c = cos(rad)
  vx = axis[1]
  vy = axis[2]
...
_G.Matrix3x3 = Matrix3x3
local get = {}
_G["UnityEngine.Vector3.Instance"] = I
local set = {}

function LuaVector3.__index(t, k)
...
function LuaVector3.__newindex(t, k, v)
...
function LuaVector3.__call(t, x, y, z)
...
function LuaVector3.Equal(a, b)
  return abs(a[1] - b[1]) < Epsilon and abs(a[2] - b[2]) < Epsilon and abs(a[3] - b[3]) < Epsilon
end

function I:Equal(b)
  return abs(self[1] - b[1]) < Epsilon and abs(self[2] - b[2]) < Epsilon and abs(self[3] - b[3]) < Epsilon
end

function LuaVector3.Better_SetPosXYZ(t, x, y, z)
  t[1], t[2], t[3] = x, y, z
end

function I:SetPosXYZ(x, y, z)
  self[1], self[2], self[3] = x, y, z
end

function LuaVector3.Better_SetPos(t, b)
...
function get.back()
  return LuaVector3.New(0, 0, -1)
end

function get.down()
  return LuaVector3.New(0, -1, 0)
end
...
function get.left()
  return LuaVector3.New(-1, 0, 0)
end

function get.one()
  return LuaVector3.New(1, 1, 1)
end
...
function get:magnitude()
  return LuaVector3.Magnitude(self)
end

function get:sqrMagnitude()
  return LuaVector3.SqrMagnitude(self)
end
...
function LuaVector3.Zero()
  return LuaVector3.New(0, 0, 0)
end

function LuaVector3.One()
  return LuaVector3.New(1, 1, 1)
end

function LuaVector3.Forward()
  return LuaVector3.New(0, 0, 1)
end

function LuaVector3.Up()
  return LuaVector3.New(0, 1, 0)
end

function LuaVector3:Better_Clone()
...
function LuaVector3.Better_Set(t, x, y, z)
  t[1], t[2], t[3] = x or 0, y or 0, z or 0
end
...
function LuaVector3.Normalize(v)
  local v = LuaVector3.Better_Clone(v)
...
function LuaVector3.Cross(a, b)
  return LuaVector3.New(a[2] * b[3] - a[3] * b[2], a[3] * b[1] - a[1] * b[3], a[1] * b[2] - a[2] * b[1])
end
...
function LuaVector3.Lerp(a, b, t)
  t = clamp(t, 0, 1)
  return LuaVector3.New(a[1] + (b[1] - a[1]) * t, a[2] + (b[2] - a[2]) * t, a[3] + (b[3] - a[3]) * t)
end
...
function LuaVector3.Better_Min(a, b, c)
...
function LuaVector3.Better_Max(a, b, c)
...
function LuaVector3.MoveTowards(a, b, adv)
  ...
  v:Destroy()
  return LuaVector3.Better_Clone(b)
end

local towardHelper = LuaVector3.New(0, 0, 0)

function LuaVector3:SelfMoveTowards(b, adv)
...
local ClampedMove = function(a, b, mag)
  local delta = b - a
  if 0 < delta then
...
function LuaVector3:SelfRotateTowards(b, angleMove, mag)
...
function LuaVector3.Distance_Square(a, b)
  local x = a[1] - b[1]
  local y = a[2] - b[2]
  local z = a[3] - b[3]
  return x * x + y * y + z * z
...
function LuaVector3.OrthoNormalize(u, v, w)
  ...
  local tu = LuaVector3.Better_Clone(u)
...
function LuaVector3:SelfSmoothDamp(target, currentVelocity, smoothTime, maxSpeed, deltaTime)
...
function LuaVector3.ClampMagnitude(vector, maxLength)
  ...
  return LuaVector3.Better_Clone(vector)
end
...
function LuaVector3:Destroy()
  self = nil
end

function LuaVector3.TransformPoint(mat, vec3, outVec3)
