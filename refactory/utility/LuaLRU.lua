...
local MoveElementToLast = function(array, element)
  local count = #array
  ...
  if obj ~= nil then
    ...
    if newAdd then
      ...
      if count == self[1] then
        return tableRemove(array, 1), newAdd
      end
    else
...
function SimpleLuaLRU:Dispose(removeCb)
  local array, rmObj = self[2]
  for i = 1, #array do
    rmObj = tableRemove(array, 1)
    if removeCb then
      removeCb(rmObj)
    end
  end
