...
local FormatPercent = function(p)
  return StringFormat("{0:F2}%", p * 100)
end
local memStart, memEnd, totalCost
local sampleMap = {}
local sampleStack = {}
local running = false
...
local sortArray = {}
local SortComparator_Cost = function(v1, v2)
...
function Debug_LuaMemotry.Debug()
  ...
  LogSetEnable(true)
  LogFormat("<color=white>Lua Memory: </color>{0}-->{1}, {2}", FormatSize(memStart), FormatSize(memEnd), FormatSize(totalCost))
  if 0 < totalCost then
    local i = 1
    for k, v in pairs(sampleMap) do
      sortArray[i] = v
    ...
    for i = 1, #sortArray do
      local v = sortArray[i]
      LogFormat("<color=white>Lua Memory({0}): </color>{1}, {2}", v.key, FormatSize(v.cost), FormatPercent(v.cost / totalCost))
    end
