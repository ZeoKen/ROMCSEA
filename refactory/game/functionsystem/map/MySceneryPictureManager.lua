...
function MySceneryPictureManager:tryGetMySceneryThumbnail(roleId, index, time, isBelongAcc)
  ...
  if FunctionPhotoStorage.IsActive() then
    local pType = FunctionPhotoStorage.GetSceneryPhotoType(isBelongAcc)
    FunctionPhotoStorage.Me():GetSceneryPhoto(pType, roleId, index, time, true, function(progress)
  ...
  if FunctionPhotoStorage.IsActive() then
    ...
    for i = 1, #self.toBeDownload do
      ...
      if singleLoadData then
        self:tryGetMySceneryThumbnail(singleLoadData.roleId, singleLoadData.index, singleLoadData.time, singleLoadData.isBelongAcc)
      end
    end
...
function MySceneryPictureManager:RemoveAllThumbnailTextures(outCacheIndexs)
  local data
  for i = 1, #self.LRUTextureCache do
...
function MySceneryPictureManager:saveToPhotoAlbum(texture, index, time, anglez)
  if FunctionPhotoStorage.IsActive() then
    ...
    FunctionPhotoStorage.Me():SaveSceneryPhoto(texture, index, time, anglez, function(progress)
...
function MySceneryPictureManager:tryGetMySceneryOriginImage_Role(roleId, index, time)
  if FunctionPhotoStorage.IsActive() then
    local pType = FunctionPhotoStorage.GetSceneryPhotoType(true)
    FunctionPhotoStorage.Me():GetSceneryPhoto(pType, roleId, index, time, false, function(progress)
...
function MySceneryPictureManager:tryGetMySceneryOriginImage(roleId, index, time)
  if FunctionPhotoStorage.IsActive() then
    local pType = FunctionPhotoStorage.GetSceneryPhotoType(false)
    FunctionPhotoStorage.Me():GetSceneryPhoto(pType, roleId, index, time, false, function(progress)
      self:MySceneryOriginPhotoDownloadProgressCallback1(roleId, index, time, progress)
    end, function(bytes, errorMessage)
      if bytes then
...
function MySceneryPictureManager:MySceneryThumbnailDownloadProgressCallback1(roleId, index, time, progress)
  ...
  GameFacade.Instance:sendNotification(MySceneryPictureManager.MySceneryThumbnailDownloadProgressCallback, {
    roleId = roleId,
    index = index,
    time = time,
    progress = progress
...
function MySceneryPictureManager:MySceneryThumbnailDownloadCompleteCallback1(roleId, index, time, bytes)
  ...
  GameFacade.Instance:sendNotification(MySceneryPictureManager.MySceneryThumbnailDownloadCompleteCallback, {
    roleId = roleId,
    index = index,
    time = time,
    byte = bytes
...
function MySceneryPictureManager:MySceneryThumbnailDownloadErrorCallback1(roleId, index, time, errorMessage)
  ...
  GameFacade.Instance:sendNotification(MySceneryPictureManager.MySceneryThumbnailDownloadErrorCallback, {
    roleId = roleId,
    index = index,
    time = time
  })
...
function MySceneryPictureManager:MySceneryOriginPhotoDownloadProgressCallback1(roleId, index, time, progress)
  ...
  GameFacade.Instance:sendNotification(MySceneryPictureManager.MySceneryOriginPhotoDownloadProgressCallback, {
    roleId = roleId,
    index = index,
    time = time,
    progress = progress
...
function MySceneryPictureManager:MySceneryOriginPhotoDownloadCompleteCallback1(roleId, index, time, bytes)
  ...
  GameFacade.Instance:sendNotification(MySceneryPictureManager.MySceneryOriginPhotoDownloadCompleteCallback, {
    roleId = roleId,
    index = index,
    time = time,
    byte = bytes
...
function MySceneryPictureManager:MySceneryOriginPhotoDownloadErrorCallback1(roleId, index, time, errorMessage)
  ...
  GameFacade.Instance:sendNotification(MySceneryPictureManager.MySceneryOriginPhotoDownloadErrorCallback, {
    roleId = roleId,
    index = index,
    time = time
  })
...
function MySceneryPictureManager:MySceneryOriginUpProgressCallback1(index, time, progress)
  ...
  GameFacade.Instance:sendNotification(MySceneryPictureManager.MySceneryOriginUpProgressCallback, {
    index = index,
    time = time,
    progress = progress
  })
...
function MySceneryPictureManager:MySceneryOriginUpCompleteCallback1(index, time, bytes)
  ...
  GameFacade.Instance:sendNotification(MySceneryPictureManager.MySceneryOriginUpCompleteCallback, {
    index = index,
    time = time,
    byte = bytes
  })
