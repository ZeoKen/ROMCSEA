...
function SkillWorkerManager:_Reset()
  ...
  DestroyWorkers(self.skillHitDelayWorks)
end
...
function SkillWorkerManager:CreateWorker_SkillHitDelayWorks(hitwork, interval, fireIndex, fireCount)
  local worker = SkillHitWork_Delay.Create(hitwork, interval, fireIndex, fireCount)
  if worker:Alive() then
    TableUtility.ArrayPushBack(self.skillHitDelayWorks, worker)
  end
  return worker
end

function SkillWorkerManager:DestroyWorker_SkillHitDelayWorks(worker)
  TableUtility.ArrayRemove(self.skillHitDelayWorks, worker)
  worker:Destroy()
end
...
function SkillWorkerManager:Update(time, deltaTime)
  ...
  UpdateWorkers(self.skillHitDelayWorks, time, deltaTime)
end
