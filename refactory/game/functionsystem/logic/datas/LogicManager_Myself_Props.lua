...
function LogicManager_Myself_Props:ctor()
  ...
  self:AddUpdateCall("SaveHp", self.UpdateSaveHSp)
  self:AddUpdateCall("SaveSp", self.UpdateSaveHSp)
end
...
function LogicManager_Myself_Props:UpdateStateEffect(ncreature, propName, oldValue, p)
  if oldValue == 0 and p:GetValue() == 0 then
    return
  end
  ItemsWithRoleStatusChange:Instance():OnReceiveStatusChange(p)
end

function LogicManager_Creature_Props:UpdateSaveHSp(ncreature, propName, oldValue, p)
  EventManager.Me():PassEvent(MyselfEvent.SaveHSpUpdate)
end

function LogicManager_Myself_Props:UpdateAttrEffect3(ncreature, propName, oldValue, p)
  LogicManager_Myself_Props.super.UpdateAttrEffect3(self, ncreature, propName, oldValue, p)
  GameFacade.Instance:sendNotification(MyselfEvent.UpdateAttrEffect)
end
