...
function Logic_Transform_DirMove:Update(logicTransform, time, deltaTime)
  ...
  if movedDistance == 0 then
    self:Stop(logicTransform)
    return
  end
  if not self.args[8] then
    NavMeshUtility.SelfSample(tempVector3_1)
  end
  LuaVector3.Better_Set(currentPosition, tempVector3_1[1], tempVector3_1[2], tempVector3_1[3])
