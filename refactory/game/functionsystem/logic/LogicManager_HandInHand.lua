...
function LogicManager_HandInHand:RegisterHandInHand(masterCreature, followerCreature)
  ...
  if nil ~= CameraController.singletonInstance then
    CameraController.singletonInstance:SetPuppetFocus(followerAssetRole.completeTransform, masterCreature.assetRole.completeTransform, CameraFocusTransformDuration)
  end
  ...
  LogUtility.InfoFormat("<color=yellow>RegisterHandInHand: </color>{0}, {1}, followerGUID={2}", masterCreature.data and masterCreature.data:GetName() or "No Name", followerCreature.data and followerCreature.data:GetName() or "No Name", followerCreature.data and followerCreature.data.id or "No GUID")
end
...
function LogicManager_HandInHand:UnregisterHandInHand(masterCreature, followerCreature)
  ...
  local followerAssetRole = followerCreature.assetRole
  if nil ~= CameraController.singletonInstance then
    CameraController.singletonInstance:SetPuppetFocus(followerAssetRole.completeTransform, nil, CameraFocusTransformDuration)
  end
  followerAssetRole:SetForceInvisible(nil)
  followerAssetRole:SetForceColliderEnable(nil)
  ...
  LogUtility.InfoFormat("<color=yellow>UnregisterHandInHand: </color>{0}, {1}, followerGUID={2}", masterCreature and masterCreature.data and masterCreature.data:GetName() or "No Name", followerCreature.data and followerCreature.data:GetName() or "No Name", followerCreature.data and followerCreature.data.id or "No GUID")
end
...
function LogicManager_HandInHand:_UpdateHandInHand(time, deltaTime, masterCreature, followerCreature)
  ...
  LuaVector3.Better_Set(tempVector3_2, LuaGameObject.GetPosition(followerTransform))
  LuaVector3.Better_Sub(tempVector3, tempVector3_1, tempVector3_3)
  local y = tempVector3_2[2]
  LuaVector3.Add(tempVector3_2, tempVector3_3)
  tempVector3_2[2] = y
  followerCreature.logicTransform:NavMeshPlaceTo(tempVector3_2)
  followerCreature.logicTransform:SetTargetAngleY(masterCreature:GetAngleY())
  local masterScale = masterCreature:GetScaleVector()
  LuaVector3.Better_Set(tempVector3_3, masterScale[1], masterScale[2], masterScale[3])
  LuaVector3.Mul(tempVector3_3, followerCreature:GetDefaultScale())
...
function LogicManager_HandInHand:RegisterDoubleAction(masterCreature, followerCreature)
  ...
  followerAssetRole:SetForceColliderEnable(nil)
  followerCreature.logicTransform:SetRotateSpeedScale(0.2)
  helplog("RegisterDoubleAction", masterCreature and masterCreature.data and masterCreature.data:GetName() or "No Name", followerCreature and followerCreature.data and followerCreature.data:GetName() or "No Name")
  if masterCreature == Game.Myself then
    self.myDoubleAction_TargetId = followerCreature.data.id
  end
  if Game.Myself == masterCreature or Game.Myself == followerCreature then
    GameFacade.Instance:sendNotification(MyselfEvent.DoubleAction_Build)
  end
end
...
function LogicManager_HandInHand:UnRegisterDoubleAction(masterCreature, followerCreature)
  ...
  if Game.Myself == masterCreature or Game.Myself == followerCreature then
    GameFacade.Instance:sendNotification(MyselfEvent.DoubleAction_Break)
  end
  helplog("UnRegisterDoubleAction", masterCreature and masterCreature.data and masterCreature.data:GetName() or "No Name", followerCreature and followerCreature.data and followerCreature.data:GetName() or "No Name")
end
...
function LogicManager_HandInHand:_UpdateDoubleAction(time, deltaTime, masterCreature, followerCreature)
  ...
  LuaVector3.Better_Set(tempVector3, LuaGameObject.GetPosition(masterTransform))
