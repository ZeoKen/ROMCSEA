...
function AI_CMD_Myself_AccessHelper:_TryAccess(time, deltaTime, creature, targetCreature, range)
  ...
  if creature:IsConcurrentRotateOnly() then
    LuaVector3.Better_Add(currentPosition, targetPosition, tempV3)
    creature.logicTransform:RotateTo(tempV3)
    return true, false
  end
  if VectorUtility.DistanceXZ_Square(currentPosition, targetPosition) < range * range then
    return true, true
  end
  return false, false
end
...
function AI_CMD_Myself_AccessHelper:Update(time, deltaTime, creature, targetCreature, range, ignoreNavMesh)
  ...
  local targetPosition = targetCreature:GetPosition()
  if (nil == prevTargetPosition or VectorUtility.DistanceXZ_Square(prevTargetPosition, targetPosition) > TargetValidDistance) and not AI_CMD_Myself_MoveToHelper.Start(self, time, deltaTime, creature, targetPosition, ignoreNavMesh) then
    if creature:IsConcurrentRotateOnly() then
      LuaVector3.Better_Add(creature:GetPosition(), targetPosition, tempV3)
      creature.logicTransform:RotateTo(tempV3)
    else
      creature:Logic_NavMeshMoveTo(targetPosition)
    end
  end
...
function AI_CMD_Myself_Access:Start(time, deltaTime, creature)
  ...
  if self.args[6] == AccessCustomType.Furniture then
    targetCreature = HomeManager.Me():FindFurniture(self.args[1])
  else
    targetCreature = SceneCreatureProxy.FindCreature(self.args[1])
  end
