...
function SkillCMD:Start(time, deltaTime, creature)
  ...
  if 0 < launchRange then
    ...
    if VectorUtility.DistanceXZ_Square(currentPosition, targetPosition) > launchRange * launchRange then
  ...
  if SkillCMD_Run.Start(self, time, deltaTime, creature, targetCreature, args[16]) then
...
function SkillCMD_Run:Start(time, deltaTime, creature, targetCreature)
  return AI_CMD_Myself_Skill_Run.Start(self, time, deltaTime, creature, targetCreature, targetCreature:GetPosition(), self.args[7], self.args[9], self.args[10], self.args[11], self.args[16])
end
