...
function IdleAI_Attack:Prepare(idleElapsed, time, deltaTime, creature)
  ...
  local id = Game.Myself:GetFakeNormalAtkID()
  id = id or creature.data:GetAttackSkillIDAndLevel()
  if id == 0 then
    return false
  end
  local info = Game.LogicManager_Skill:GetSkillInfo(id)
  if info == nil then
    return false
  end
  local attackTarget = creature:Logic_GetAttackTarget()
  if nil ~= attackTarget and not info:CheckTarget(creature, attackTarget) then
