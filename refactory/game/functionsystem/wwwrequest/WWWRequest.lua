...
function WWWRequest:Request(url, overtime)
  ...
  local c = coroutine.create(function()
    self.www = Networking.UnityWebRequest.Get(self.url)
    self.overtimeStamp = UnityUnscaledTime + self.overtime
    Yield(self.www:SendWebRequest())
    if self.state == WWWRequestState.REQUESTING then
...
function WWWRequest:Update(time, deltaTime)
  if UnityUnscaledTime >= self.overtimeStamp then
