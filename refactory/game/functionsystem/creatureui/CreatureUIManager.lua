...
function CreatureUIManager:IsInWaitingQueue(creatureId, resourceId)
  for i = 1, #self.waitQueue do
    local single = self.waitQueue[i]
    if single[1] == creatureId and single[2] == resourceId then
      return true
    end
  end
end
...
function CreatureUIManager:AsyncCreateUIAsset(creatureId, resourceId, parent, call, callArgs, argsDeleter, useMovePool)
  if self:IsInWaitingQueue(creatureId, resourceId) then
    return
  end
...
function CreatureUIManager:RemoveCreatureWaitUI(creatureId, resourceId)
  for i = #self.waitQueue, 1, -1 do
    ...
    if data and data[1] == creatureId and data[2] == resourceId then
