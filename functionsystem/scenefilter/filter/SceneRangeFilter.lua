...
function SceneRangeFilter.CheckNotTeam(creature)
  ...
  elseif creature:GetCreatureType() == Creature_Type.Pet and creature.data.ownerID ~= Game.Myself.data.id then
...
function SceneRangeFilter.CheckNotTeamOther(creature)
  if creature:GetCreatureType() == Creature_Type.Me then
    ...
    return not TeamProxy.Instance:IsInMyTeam(playerID) and playerID ~= Game.Myself.data.id
  end
  return true
end
...
function SceneRangeFilter.CheckNotGuildOther(creature)
  if creature:GetCreatureType() == Creature_Type.Me then
    ...
    return not GuildProxy.Instance:CheckPlayerInMyGuild(playerID) and playerID ~= Game.Myself.data.id
  end
  return true
end
...
function SceneRangeFilter.CheckBoothOther(creature)
  if creature:GetCreatureType() == Creature_Type.Me then
    return false
  end
  if creature:GetCreatureType() == Creature_Type.Pet then
    if creature.data.ownerID ~= Game.Myself.data.id then
      local owner = FindCreature(creature.data.ownerID)
      if owner ~= nil then
        return owner:IsInBooth()
      end
    end
    return false
  end
  return creature:IsInBooth()
end
