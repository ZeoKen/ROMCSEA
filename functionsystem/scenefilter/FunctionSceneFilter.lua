...
function FunctionSceneFilter:ctor()
  self.enabled = false
  self.groupFilter = {}
  self.filterTree = LBehaviorTree.new("屏蔽决策树")
  self.filterTree:Run()
  ...
  self.checkFunc[SceneFilterDefine.Target.LuaGO] = self.SceneCheckLuaGOHandler
  self.checkFunc[SceneFilterDefine.Target.FriendNpc] = self.SceneCheckFriendNpcHandler
end
...
function FunctionSceneFilter:SetEnable(value)
  if self.enabled ~= value then
    ...
    if value then
      ...
      EventManager.Me():AddEventListener(SceneUserEvent.SceneRemoveNpcs, self.SceneRemoveNpcsHandler, self)
      ...
      EventManager.Me():AddEventListener(BigWorldClientEvent.ClientNpcLoaded, self.SceneAddNpcsHandler, self)
    else
      ...
      EventManager.Me():RemoveEventListener(SceneUserEvent.SceneRemoveNpcs, self.SceneRemoveNpcsHandler, self)
      EventManager.Me():RemoveEventListener(SceneUserEvent.SceneRemovePets, self.SceneRemoveCreaturesHandler, self)
      ...
      EventManager.Me():RemoveEventListener(BigWorldClientEvent.ClientNpcLoaded, self.SceneAddNpcsHandler, self)
    end
  end
end
...
function FunctionSceneFilter:SceneRemoveNpcsHandler(ids)
  for i = 1, #ids do
    SceneFilterProxy.Instance:RemoveNpc(ids[i])
  end
end
...
function FunctionSceneFilter:SceneAddNpcsHandler(npcs)
  if npcs == nil then
    self:CheckCreatures(NSceneNpcProxy.Instance:GetAll())
    self:CheckCreatures(NSceneNpcProxy.Instance:GetClientAll())
  else
    self:CheckCreatures(npcs)
  end
end
...
function FunctionSceneFilter:CheckCreature(creature)
  if creature then
    self.filterTree.blackBoard.ftarget = creature
    self.filterTree:Update()
    self.filterTree.blackBoard.ftarget = nil
  end
end

function FunctionSceneFilter:CheckCreatures(map)
...
function FunctionSceneFilter:SetApplyDefaultSceneFilterId(filter)
  self.applyDefaultSceneFilterId = filter
end
...
function FunctionSceneFilter:StartFilterById(id, creatures, counterwise)
  ...
  if not groupFilter and Table_ScreenFilter[id] then
    groupFilter = SceneFilterNode.new(self.filterTree, Table_ScreenFilter[id].Group, counterwise)
  end
  if groupFilter then
    ...
    errorLog(string.format("Table_ScreenFilter屏蔽表中找不到id %s", id))
  end
end
...
function FunctionSceneFilter:CheckLuaGO(luago)
  if luago then
    self.filterTree.blackBoard.ftarget = luago
    self.filterTree:Update()
    self.filterTree.blackBoard.ftarget = nil
  end
end

function FunctionSceneFilter:SceneCheckLuaGOHandler()
  local luagoMgrs = Game.GameObjectManagers
  for _, v in pairs(luagoMgrs) do
    if v.ForEachLuaGO then
      v:ForEachLuaGO(function(luago)
        self:CheckLuaGO(luago)
      end)
    end
  end
end

function FunctionSceneFilter:SceneCheckFriendNpcHandler(npcs)
  npcs = npcs or NSceneNpcProxy.Instance:GetAll()
  local IsFriendNpcByData = NpcMonsterUtility.IsFriendNpcByData
  for k, v in pairs(npcs) do
    if IsFriendNpcByData(v.data and v.data.staticData) then
      self:CheckCreature(v)
    end
  end
end
