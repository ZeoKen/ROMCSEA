...
function FunctionChatIO:CheckLocalFiles()
  ...
  for i = 1, #dateDirectory do
    ...
    if 1 < #foldstr and not ClientTimeUtil.TransTimeStrToTimeTick(dateDirectory[i], currentServerTime, self.limitDayNum) then
...
function FunctionChatIO:AddSaveCache(chatId, data)
  if data then
    ...
    chat.id = data:GetId()
    chat.time = data:GetTime()
    chat.str = data:GetStr()
    chat.audioId = data:GetVoiceid()
    chat.audioLength = data:GetVoicetime()
    chat.expressionId = data:GetExpressionId()
    chat.photo = data:GetPhotoInfo()
    chat.postcard = data:GetPostcardInfo()
