...
function PhotoCommandShowGhost:ResetCreatures()
  if self.ghosts then
    ...
    for k, id in pairs(self.ghosts) do
      ...
      if creature then
        manager:UpdateHiding(creature, nil, nil, creature.data.props:GetPropByName("Hiding"))
      end
    end
...
function PhotoCommandShowGhost:GhostOutSight(creature)
  if creature then
    local hide = Game.LogicManager_Npc_Props:UpdateHiding(creature, nil, nil, creature.data.props:GetPropByName("Hiding"))
    if hide then
