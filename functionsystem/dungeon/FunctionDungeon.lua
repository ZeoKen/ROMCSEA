...
function FunctionDungen:HandlerChangeJob()
  GameFacade.Instance:sendNotification(UIEvent.JumpPanel, {
    view = PanelConfig.ChangeJobView,
    force = true
  })
end
...
function FunctionDungen:DungenBattleSuccess(data)
  ...
  if fubenType == FuBenCmd_pb.ERAIDTYPE_LABORATORY then
    ...
    function vdata.callback()
      GameFacade.Instance:sendNotification(UIEvent.JumpPanel, {
        view = PanelConfig.InstituteResultPopUp,
...
function FunctionDungen:ShowDungenInfo(step, progress, score)
  if not self.dungenInfoData then
    self.dungenInfoData = {}
  else
    TableUtility.TableClear(self.dungenInfoData)
  end
  self.dungenInfoData.type = QuestDataType.QuestDataType_INVADE
  QuestProxy.Instance:AddTraceCell(self.dungenInfoData)
end

function FunctionDungen:UpdateDungenInfo(step, progress, score)
  if self.dungenInfoData then
  end
  QuestProxy.Instance:AddTraceCell(self.dungenInfoData)
end

function FunctionDungen:RemoveDungenInfo()
  QuestProxy.Instance:RemoveTraceCell(self.dungenInfoData)
end
