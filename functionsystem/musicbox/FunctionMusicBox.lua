...
FunctionMusicBox.SquareMusicBoxRange = FunctionMusicBox.MusicBoxRange * FunctionMusicBox.MusicBoxRange
...
function FunctionMusicBox:AddMusicBox(creature)
  ...
  if nil ~= oldMusicBox and oldMusicBox.UnregisterWeakObserver then
    oldMusicBox:UnregisterWeakObserver(self)
  end
  if nil ~= self.musicBox and self.musicBox.RegisterWeakObserver then
...
function FunctionMusicBox:RemoveMusicBox(creature)
  if creature == self.musicBox then
    self.musicBox = nil
    if nil ~= creature and creature.UnregisterWeakObserver then
...
function FunctionMusicBox:SetMusic(musicId, startTime, playTimes, force)
  if not force and musicId == self.musicId and startTime == self.startTime and playTimes == self.playTimes then
    return
  end
...
function FunctionMusicBox:TimeTickUpdate(deltatime)
  if self.musicBox then
    ...
    if self.djPlayer then
      ...
      if LuaVector3.Distance_Square(djPos, musicBoxPos) < FunctionMusicBox.SquareMusicBoxRange then
        self:DjPlay()
      else
        self:DjDisPlay()
      end
    ...
    if LuaVector3.Distance_Square(myPos, musicBoxPos) < FunctionMusicBox.SquareMusicBoxRange then
      self:PlayMusicBoxBg()
    else
      self:DisPlayMusicBoxBg()
    end
  ...
  if msData and self.startTime ~= 0 then
    ...
    if playTimes == 1 then
      if pasttime < msData.MusicTim then
        FunctionBGMCmd.Me():PlayJukeBox(msData.MusicAd, pasttime, nil, nil, nil, nil, nil, nil, force)
      end
    else
      FunctionBGMCmd.Me():PlayJukeBox(msData.MusicAd, pasttime, nil, playTimes, nil, nil, nil, nil, force)
    end
  end
end
...
function FunctionMusicBox:IsPlayMusicBoxBg()
  return self.musicPlaying == true
end
