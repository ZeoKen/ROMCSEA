...
function FunctionTransform:TransformTo(player, transformID)
  ...
  if player:GetCreatureType() == Creature_Type.Me then
    ...
    local canAtk = self:IsTransformAtkMap()
    if canAtk and player.data.transformData:IsNpcGroup() then
      canAtk = false
    end
    player.data:Camp_SetIsSelfTransformedAtk(canAtk)
  end
end
...
function FunctionTransform:TrySetOtherTransformedAtk(player)
  if player and player.data:GetProfesstion() ~= RoleDefines_Profession.Novice and self:IsTransformAtkMap() then
    local canAtk = true
    if player.data.transformData:IsNpcGroup() then
      canAtk = false
    end
    player.data:Camp_SetIsOtherTransformedAtk(canAtk)
  end
end
