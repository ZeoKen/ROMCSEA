...
function FunctionBarrage:Initialize()
  ...
  if self.transMainCamera == nil or Game.GameObjectUtil:ObjectIsNULL(self.transMainCamera) then
    self.transMainCamera = GameObject.FindGameObjectWithTag("MainCamera").transform
  end
  if self.barrage == nil or Game.GameObjectUtil:ObjectIsNULL(self.barrage.gameObject) then
    self.barrage = BarrageView.new(Game.AssetManager_UI:CreateAsset(ResourcePathHelper.UIView("Barrages"), self.uiroot))
  end
  if self.barrageCam == nil or Game.GameObjectUtil:ObjectIsNULL(self.barrageCam) then
    self.barrageCam = Game.GameObjectUtil:DeepFindChild(self.barrage.gameObject, "BarrageCamera"):GetComponent(CameraProjector)
  end
  self.barrageCam.sampleSize = Vector2(BarrageView.activeWidth, self.barrage:GetActiveHeight())
  if self.goMesh == nil or Game.GameObjectUtil:ObjectIsNULL(self.goMesh) then
    self:LoadMesh()
  else
...
function FunctionBarrage:LoadMesh()
  ...
  LuaVector3.Better_Set(vec3, posOfCamera.x, posOfCamera.y, posOfCamera.z)
  Asset_Effect.PlayAt(EffectMap.Maps.Barrage, vec3, function(go_mesh, owner)
    if not owner then
      return
    end
    owner:OnMeshBeLoaded(go_mesh)
  end)
end
...
function FunctionBarrage:OnMeshBeLoaded(effect_handle)
  ...
  LuaVector3.Better_Set(vec3, 0, 0, 0)
...
function FunctionBarrage:_AddText(data)
  ...
  go.transform.localScale = LuaGeometry.GetTempVector3(scale, scale * BarrageView.yScale, 1)
  if data.speed <= self.meshSpeed then
    TimeTickManager.Me():CreateOnceDelayTick(data.duration * 1000, function(owner, deltaTime)
      self:Remove(data)
    end, self)
  else
  ...
  if data.duration > 0.3 then
    go.timeTick = TimeTickManager.Me():CreateOnceDelayTick(data.duration * 1000 - 300, function(owner, deltaTime)
...
function FunctionBarrage:CreateFrame(go, frameid)
  ...
  if not frameConfig then
    LogUtility.Error(string.format("没有找到弹幕边框配置：%s", tostring(frameid)))
    return
  end
  local bounds = NGUIMath.CalculateRelativeWidgetBounds(go.transform, false)
  ...
  objFrame.transform.localScale = LuaGeometry.GetTempVector3(1, 1, 1)
...
function FunctionBarrage:CreateFrameAdorment(name, parent, posX, posY)
  ...
  if not obj then
    LogUtility.Error(string.format("创建弹幕边框装饰：%s 失败！", tostring(name)))
    return
  end
