...
function FunctionPlayerUI:CreatureMaskBy(id, uiType)
  ...
  if creature then
    return creature:IsUIMask(uiType)
  end
  return false
end
...
function FunctionPlayerUI:MaskTopFrame(creature, reason, autoSend)
  if creature:GetCreatureType() == Creature_Type.Npc and not creature:HasReasonInvisible() and not Game.MapManager:IsInAllGVG() then
    return
  end
...
function FunctionPlayerUI:MaskGiftSymbol(creature, reason, autoSend)
  return self:MaskUI(creature, reason, MaskPlayerUIType.GiftSymbol, autoSend)
end

function FunctionPlayerUI:UnMaskGiftSymbol(creature, reason, autoSend)
  return self:UnMaskUI(creature, reason, MaskPlayerUIType.GiftSymbol, autoSend)
end

function FunctionPlayerUI:MaskHPSP(creature, reason, autoSend)
  return self:MaskUI(creature, reason, MaskPlayerUIType.BloodType, autoSend)
end

function FunctionPlayerUI:UnMaskHPSP(creature, reason, autoSend)
  return self:UnMaskUI(creature, reason, MaskPlayerUIType.BloodType, autoSend)
end

function FunctionPlayerUI:MaskNameFaction(creature, reason, autoSend)
  return self:MaskUI(creature, reason, MaskPlayerUIType.NameType, autoSend)
end

function FunctionPlayerUI:UnMaskNameFaction(creature, reason, autoSend)
  return self:UnMaskUI(creature, reason, MaskPlayerUIType.NameType, autoSend)
end
