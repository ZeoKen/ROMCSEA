...
function SocialManager:ctor()
  ...
  self.relationhandler[relation.Friend] = {
    self._FriendAdd,
    self._FriendRemove
  }
  self.relationhandler[relation.Chat] = {
    self._ChatAdd,
    self._ChatRemove
  }
  self.relationhandler[relation.Team] = {
    self._TeamAdd,
    self._TeamRemove
  }
  self.relationhandler[relation.Apply] = {
    self._ApplyAdd,
    self._ApplyRemove
  }
  self.relationhandler[relation.Black] = {
    self._BlackAdd,
    self._BlackRemove
  }
  self.relationhandler[relation.BlackForever] = {
    self._BlackForeverAdd,
    self._BlackForeverRemove
  }
  self.relationhandler[relation.Tutor] = {
    self._TutorAdd,
    self._TutorRemove,
    nil,
    self._preAddTutor
  }
  self.relationhandler[relation.TutorApply] = {
    self._TutorApplyAdd,
    self._TutorApplyRemove
  }
  self.relationhandler[relation.Student] = {
    self._StudentAdd,
    self._StudentRemove,
    nil,
    self._preAddStudent
  }
  self.relationhandler[relation.StudentApply] = {
    self._TutorApplyAdd,
    self._TutorApplyRemove
  }
  self.relationhandler[relation.StudentRecent] = {
    self._StudentRecentAdd,
    self._StudentRecentRemove
  }
  self.relationhandler[relation.TutorClassmate] = {
    self._TutorClassmateAdd,
    self._TutorClassmateRemove
  }
  self.relationhandler[relation.Contract] = {
    self._ContractAdd,
    self._ContractRemove,
    self._ContractAddResult
  }
...
function SocialManager:PreProcess(serverData)
  ...
  for i = 1, self.relationCount do
    relationhandler = self.relationhandler[i]
    if relationhandler and (socialData == nil or socialData.relation ~= serverData.relation) then
...
function SocialManager:UpdateData(serverData)
  ...
  if socialData then
    ...
    for i = 1, #serverData.items do
      ...
      elseif item.type == SessionSociality_pb.ESOCIALDATA_WAITSIGN then
        socialData.waitRecall = item.value ~= 0
      elseif item.type == SessionSociality_pb.ESOCIALDATA_RELATION then
        local lastRelation = socialData.relation
        socialData.relation = item.value
        self:UpdateRelation(socialData, lastRelation)
      elseif item.type == SessionSociality_pb.ESOCIALDATA_TEAMNAME then
        socialData.teamname = item.data
      elseif item.type == SessionSociality_pb.ESOCIALDATA_HOME_ROOMID then
        socialData.roomid = item.value
      elseif item.type == SessionSociality_pb.ESOCIALDATA_PORTRAIT_FRAME then
        socialData.portraitframe = item.value
      elseif item.type == SessionSociality_pb.ESOCIALDATA_AFK then
        socialData.afk = item.value
      elseif item.type == SessionSociality_pb.ESOCIALDATA_CHEAT_MARK then
        socialData.cheatMark = item.value == 1
      elseif item.type == SessionSociality_pb.ESOCIALDATA_GUILDID then
        socialData.guildid = item.value
      elseif item.type == SessionSociality_pb.ESOCIALDATA_MERCENARY_GUILDID then
        socialData.mercenary_guildid = item.value
      elseif item.type == SessionSociality_pb.ESOCIALDATA_MERCENARY_GUILDNAME then
...
function SocialManager:TryRemoveDirty()
  for k, v in pairs(self.dataMap) do
    if v.accid == nil then
      self:Remove(k)
    end
