...
function FunctionSelectCharacter.CreateStashInfo(role)
  if nil ~= role then
    ...
    LuaQuaternion.Better_Set(info.rotation, LuaGameObject.GetLocalRotation(role.completeTransform))
    return info
  end
  return nil
end
...
function FunctionSelectCharacter:ctor()
  ...
  function self.selectedListener(obj)
    self:SetSelected(obj)
  end
  
  function self.updateListener()
    self:Update()
  end
end
...
function FunctionSelectCharacter:GetCharacterNumber(roleComplete)
  for k, v in ipairs(self.characterMap) do
    if roleComplete == v.maleRole.complete or roleComplete == v.femaleRole.complete then
      return k, v
    end
  end
  return 0
  ...
  if nil ~= characterInfo then
    ...
    if info.invalidate then
      ...
      if FunctionSelectCharacter.EASTER_EGGS_COUNT == characterInfo.easterEggs then
        ...
        local data = {number = characterNumber, info = characterInfo}
...
function FunctionSelectCharacter:Update()
  ...
  elseif FunctionSelectCharacter.PHASE_UNSELECTING == self.phase and (not currentState:IsName(self.rootAnimationName) or 0 >= currentState.normalizedTime) then
    self.phase = nil
    self:DoUnselectRole()
  end
end
...
function FunctionSelectCharacter:NewCharacterSelectorLaunch(selector)
  self.newCharacterSelectorMono = selector
end

function FunctionSelectCharacter:NewCharacterSelectorShutdown()
  self.newCharacterSelectorMono = nil
end

function FunctionSelectCharacter:NewCharacterSelectorSetDesPoint(point)
  if self.newCharacterSelectorMono then
    self.newCharacterSelectorMono:SetDesPoint(point)
  end
end
