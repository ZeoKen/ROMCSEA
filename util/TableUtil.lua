...
function TableUtil.HashToArray(hash, targetArray)
  targetArray = targetArray or {}
  for k, v in pairs(hash) do
    targetArray[#targetArray + 1] = v
  end
  return targetArray
end
...
function TableUtil.Array2RowArray(array, row, row_array)
  if not (array and row) or row <= 1 then
    return
  end
  row_array = row_array or {}
  for i = 1, #array do
    local divisor = math.floor(i / row)
    local remainder = i % row
    local _row = remainder == 0 and divisor or divisor + 1
    local row_data = row_array[_row]
    if not row_data then
      row_data = {}
      row_array[_row] = row_data
    end
    row_data[#row_data + 1] = array[i]
  end
  return row_array
end
...
function printData(key, data)
  if MyLuaSrv.EnablePrint then
    ...
    LogUtility.Info(innerString(data))
  end
end
...
function TableUtil.TableIsEmpty(t)
