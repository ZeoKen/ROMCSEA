...
function UnionLogo:GetOriginImage(caller_index, pos_index, timestamp, extension, progress_callback, success_callback, error_callback, is_keep_previous_callback, is_through_personalphotocallback)
  if FunctionPhotoStorage.IsActive() then
    FunctionPhotoStorage.Me():GetGuildIcon(self:GetUnionID(), pos_index, timestamp, extension, false, progress_callback, function(bytes, errorMessage)
      if bytes then
        if success_callback then
          success_callback(bytes)
        end
  ...
  if isLatestLocal then
    ...
    if not isDownloading then
      local downloadPath = ApplicationHelper.persistentDataPath .. "/" .. NetIngUnionLogo.Ins():GetTempDownloadPathOfLocal(photoID, true, extension)
      NetIngUnionLogo.Ins():Download(unionID, pos_index, photoID, timestamp, extension, function(x)
...
function UnionLogo:GetThumbnail(caller_index, pos_index, timestamp, extension, progress_callback, success_callback, error_callback, is_keep_previous_callback)
  if FunctionPhotoStorage.IsActive() then
    FunctionPhotoStorage.Me():GetGuildIcon(self:GetUnionID(), pos_index, timestamp, extension, true, progress_callback, function(bytes, errorMessage)
      if bytes then
        if success_callback then
          success_callback(bytes)
        end
  ...
  if isLatestLocal then
    ...
    if not isDownloading and not self:IsGetingOForMakeT(photoID) and not self:IsMakingTWhenGetThumbnail(photoID) then
      local downloadPath = ApplicationHelper.persistentDataPath .. "/" .. NetIngUnionLogo.Ins():GetTempDownloadPathOfLocal(photoID, false, extension)
      NetIngUnionLogo.Ins():Download(unionID, pos_index, photoID, timestamp, extension, function(x)
        local progressValue = x
        ...
        self:GetOriginImage(0, pos_index, timestamp, extension, nil, function(x, y)
...
function UnionLogo:SaveAndUpload(pos_index, bytes, timestamp, extension, progress_callback, success_callback, error_callback)
  ...
  if self:IsSaving(photoID) then
    ...
    LocalUnionLogo.Ins():SaveAsync(photoID, bytes, timestamp, extension, function()
      local tempUploadRootPath = ApplicationHelper.persistentDataPath .. "/" .. NetIngUnionLogo.Ins():GetTempUploadRootPathOfLocal()
      if not FileHelper.ExistDirectory(tempUploadRootPath) then
        FileHelper.CreateDirectory(tempUploadRootPath)
      end
      local tempUploadPathOfLocal = ApplicationHelper.persistentDataPath .. "/" .. NetIngUnionLogo.Ins():GetTempUploadPathOfLocal(photoID, extension)
      FileDirectoryHandler.WriteFile(tempUploadPathOfLocal, bytes, function()
        local n = self.isSaving[photoID]
        n = n - 1
        self.isSaving[photoID] = n
        NetIngUnionLogo.Ins():FormUpload(pos_index, photoID, extension, progress_callback, function()
...
function UnionLogo:ClearLocal(pos_index, extension)
  ...
  local unionID = self:GetUnionID()
  local photoID = self:GetPhotoID(unionID, pos_index)
  local downloadOPath = ApplicationHelper.persistentDataPath .. "/" .. NetIngUnionLogo.Ins():GetTempDownloadPathOfLocal(photoID, true, extension)
  if FileHelper.ExistFile(downloadOPath) then
    FileHelper.DeleteFile(downloadOPath)
  end
  local downloadTPath = ApplicationHelper.persistentDataPath .. "/" .. NetIngUnionLogo.Ins():GetTempDownloadPathOfLocal(photoID, false, extension)
