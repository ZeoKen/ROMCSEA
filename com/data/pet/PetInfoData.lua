...
local _HeadUpdateKey = {
  ...
  [ScenePet_pb.EPETDATA_LV] = 1,
  [ScenePet_pb.EPETDATA_BODY] = 1
}
...
function PetInfoData:Server_SetData(serverData)
  ...
  if server_curwears then
    ...
    for i = 1, #server_curwears do
      local petEquipData = {
        epos = server_curwears[i].epos,
        itemId = server_curwears[i].itemid
      }
...
function PetInfoData:Server_UpdateData(petMemberDatas)
  ...
  if needUpdatePetHeadData then
    GameFacade.Instance:sendNotification(MainViewEvent.PetInfoUpdate)
  end
end
