...
function EquipSuitData:GetSuitAddBuffIds()
  TableUtility.TableClear(buffIds)
  local buffs
  for _, suitData in pairs(self.suitDatas) do
    for _, field in pairs(suitAddFields) do
      buffs = suitData[field].buff
      if type(buffs) == "table" then
        for _, buff in pairs(buffs) do
          TableUtility.ArrayPushBack(buffIds, buff)
        end
      end
    end
  end
...
function EquipSuitData.GetBannedEquipSuitArray()
  return FunctionNpcFunc.Me():CheckSingleFuncForbidState(15) and Table_FuncState[15].Param or _EmptyTable
end
...
function SuitInfo:ctor(suitIds)
  self.equipSuitDatas = {}
  local teamMap, bannedSuits = {}, EquipSuitData.GetBannedEquipSuitArray()
  for i = 1, #suitIds do
    if TableUtility.ArrayFindIndex(bannedSuits, suitIds[i]) == 0 then
