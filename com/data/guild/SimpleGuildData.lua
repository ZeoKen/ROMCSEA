...
function SimpleGuildData:SetData(data)
  ...
  self.job = data[4]
  if data[5] ~= nil then
    self.mercenaryName = OverSea.LangManager.Instance():GetLangByKey(data[5])
  end
  self.jobName = not StringUtil.IsEmpty(self.job) and OverSea.LangManager.Instance():GetLangByKey(self.job) or ""
  local iconId = data[3]
  if iconId == "" then
    iconId = "1"
  end
