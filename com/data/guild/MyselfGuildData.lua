...
function MyselfGuildData:Server_SetApprovedInfo(server_data)
  self.approvedInfo.applied = server_data.applied
  self.approvedInfo.checked = server_data.checked
  self.approvedInfo.needlevel = server_data.needlevel
  self.approvedInfo.recruit = server_data.recruit
  self.approvedInfo.mercenary = server_data.mercenary
end
...
function MyselfGuildData:_Server_UpdateTask(server_updates)
  ...
  for i = 1, #server_updates do
    ...
    cachedata:UpdateTaskData(update)
  end
end

function MyselfGuildData:_Server_DelTasks(server_dels)
  if server_dels and 0 < #server_dels then
    for i = 1, #server_dels do
...
local _SortGuildChallengeTasks = function(a, b)
...
function MyselfGuildData:GetChallengeTaskList()
  ...
  for groupId, taskData in pairs(self.challenge_task_map) do
    local tracedatas = taskData:GetTraceTaskDatas()
  ...
  local buildingData = GuildBuildingProxy.Instance:GetTaskTraceData()
  TableUtil.InsertArray(self.challenge_task_list, buildingData)
  table.sort(self.challenge_task_list, _SortGuildChallengeTasks)
  return self.challenge_task_list
end

local _guildPackDirty = false

function MyselfGuildData:SetGuildPackItems(serverItems)
  if serverItems then
    _guildPackDirty = true
...
function MyselfGuildData:GetGuildAsset()
  if _guildPackDirty then
    _guildPackDirty = false
    TableUtility.ArrayClear(self.guildPackList)
    if not self.guildPack then
      return nil
    end
    for k, v in pairs(self.guildPack) do
      if v.staticData then
        self.guildPackList[#self.guildPackList + 1] = v
      end
    end
    table.sort(self.guildPackList, function(l, r)
...
function MyselfGuildData:RemoveGuildPackItems(dels)
  ...
  for i = 1, #dels do
    local guid = dels[i]
    if guid and self.guildPack[guid] then
...
function MyselfGuildData:GetGuildPackItemByGuid(id)
  if self.guildPack then
    for k, v in pairs(self.guildPack) do
      if k == id then
        return v
