  ...
  if tab_skills_have_not_learn ~= nil then
    for i = 1, #tab_skills_have_not_learn do
      ...
      if not table.ContainsValue(tabSkillShopItemsHaveNotLearn, skillShopItemID) then
        ...
        local skillShopItemData = ShopProxy.Instance:GetShopItemDataByTypeId(FuncAdventureSkill.iShopType, FuncAdventureSkill.Instance().iSerialNumber, skillShopItemID)
    ...
    if skillID and 0 < skillID then
      ...
      if filedCondition ~= nil then
        ...
        for i = 1, SkillItemData.ConditionSkillCount do
          suffix = i == 1 and "" or i
          skillid = filedCondition["skillid" .. suffix]
          if skillid == nil then
            break
          end
...
function UIModelAdventureSkill:SortFromUnlockToLock(tab_skill_shop_items_data)
  ...
  elseif tab_skill_shop_items_data ~= nil and not table.IsEmpty(tab_skill_shop_items_data) then
    ...
    for i = #tab_skill_shop_items_data, 1, -1 do
      ...
      if adventureLevel < requireAppellation then
        lockSkillShopItems[skillShopItemData.id] = skillShopItemData
        table.insert(lockSkillShopItemsAppellation, {
          id = skillShopItemData.id,
          appellation = requireAppellation
        })
...
function UIModelAdventureSkill:GetSkillRequireAppellation(i_shop_item_id)
  local retValue = 0
  if i_shop_item_id and 0 < i_shop_item_id then
    local skillShopItemData = ShopProxy.Instance:GetShopItemDataByTypeId(FuncAdventureSkill.iShopType, FuncAdventureSkill.Instance().iSerialNumber, i_shop_item_id)
    local skillID = skillShopItemData.SkillID
    local skillConf = Table_Skill[skillID]
    retValue = skillConf.Contidion.riskid
    if not retValue then
      redlog(string.format("冒险技能：%s 未配置Contidion.riskid！", tostring(skillID)))
    end
  end
  return retValue
end
