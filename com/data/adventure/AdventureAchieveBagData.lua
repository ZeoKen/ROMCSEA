...
function AdventureAchieveBagData:GetItems(tabType)
  return self:GetItemsForce(tabType, false)
end

function AdventureAchieveBagData:GetItemsForce(tabType, isForce)
  if tabType ~= nil then
    local tab = self.tabs[tabType]
    if tab ~= nil then
      return tab:GetItems(isForce)
    end
  end
  return self.wholeTab:GetItems(isForce)
end
...
function AdventureAchieveBagData:GetItem(tabType, id)
  if tabType ~= nil then
    local tab = self.tabs[tabType]
    if tab ~= nil then
      return tab:GetItemByStaticID(id)
    end
  end
  return self.wholeTab:GetItemByStaticID(id)
end
