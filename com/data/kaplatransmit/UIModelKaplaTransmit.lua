...
function UIModelKaplaTransmit:GetTeammates()
  ...
  if TeamProxy.Instance:IHaveTeam() and myselfTeamData ~= nil then
    ...
    if teammates ~= nil then
      ...
      local interactNpcManager = Game.InteractNpcManager
      local teammatesID = {}
      for _, v in pairs(teammates) do
        ...
        if isOnline then
          ...
          teammateDetail.isFollowing = interactNpcManager:GetMultiMountMasterID(teammateID) == myselfID
...
function UIModelKaplaTransmit:GetHandInHandPlayerID_Teammate_NotCat()
  ...
  if handInHandPlayerID ~= nil and 0 < handInHandPlayerID then
