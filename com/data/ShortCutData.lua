...
function ShortCutData:UnLockSkillShortCuts(data)
  ...
  local _SkillShortCuts = ShortCutProxy.SkillShortCut
  ...
  elseif shortcutType == _SkillShortCuts.Auto or _SkillShortCuts.Auto2 then
...
function ShortCutData:UnLockNormalSkillShortCuts(shortcutType, indexes)
  ...
  if indexes ~= nil then
    local init = self.skillUnlockMaxIndex[shortcutType] == nil or 1 >= self.skillUnlockMaxIndex[shortcutType]
    for i = 1, #indexes do
      local pos = indexes[i]
