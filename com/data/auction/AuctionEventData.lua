...
function AuctionEventData:SetInfo(batchid, itemid, orderid)
...
function AuctionEventData:GetTimeString()
  if self.timeString == nil then
    local time = os.date("*t", self.time / 1000)
  ...
  if self.content == nil then
    ...
    if self.event then
      ...
      elseif self.event == AuctionEventState.ResultSuccess then
        local str = ""
        if AuctionProxy.Instance:CheckEndItem(self.batchid, self.orderid) then
