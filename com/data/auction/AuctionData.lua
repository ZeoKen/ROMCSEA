...
function AuctionData:UpdateItemInfo(serverData)
  for i = 1, #self.itemList do
    ...
    if data.orderid == AuctionProxy.GetOrderID(serverData.signup_id, serverData.itemid) then
...
function AuctionData:SetNextInfo(serverData)
  ...
  self.lastOrderid = AuctionProxy.GetOrderID(serverData.last_signup_id, serverData.last_itemid)
  self.nextOrderid = AuctionProxy.GetOrderID(serverData.signup_id, serverData.itemid)
end
...
function AuctionData:TryGetPage(itemid, pageIndex, orderid, originOrder)
  local page, element = self:PureGetPage(orderid, pageIndex)
  if page == nil then
    ServiceAuctionCCmdProxy.Instance:CallReqAuctionFlowingWaterCCmd(self.batchId, itemid, pageIndex, nil, originOrder)
  end
  return page, element
end
