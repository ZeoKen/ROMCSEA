...
function PostData:SetData(mailData)
  if mailData then
    ...
    self:SetAttach(mailData.attach)
...
function PostData:SetAttach(blobAttach)
  self.postItems = {}
  if blobAttach then
    for i = 1, #blobAttach.attachs do
      local mailattach = blobAttach.attachs[i]
      if mailattach.mailid and mailattach.mailid > 0 then
        self.mailid = mailattach.mailid
      end
        ...
        if mailattach.id then
          ...
          if postTeamids then
            for tk, tv in pairs(postTeamids) do
              ...
              if tv.refinelv and tempItem:IsEquip() then
                tempItem.equipInfo:SetRefine(tv.refinelv)
              end
