...
local PACKAGECHECK = GameConfig.PackageMaterialCheck.produce
...
function BusinessmanMakeData.GetCanMakeTimes(id)
  ...
  if compose and compose.BeCostItem then
    ...
    for i = 1, #compose.BeCostItem do
      ...
      local haveCount = _BagProxy:GetItemNumByStaticID(item.id, PACKAGECHECK)
      local itemTimes = math.floor(haveCount / item.num)
      if times == nil or times > itemTimes then
        times = itemTimes
      end
    end
  end
