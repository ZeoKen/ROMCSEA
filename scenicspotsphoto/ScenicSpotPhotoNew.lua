...
function ScenicSpotPhotoNew:GetOriginImage_Share(role_id, scenic_spot_id, timestamp, progress_callback, success_callback, error_callback, is_keep_previous_callback, is_through_scenicspotphotocallback, extension)
  ...
  if isLatestLocal then
    ...
    if is_through_scenicspotphotocallback and success_callback ~= nil then
    ...
    if not isDownloading then
      local downloadPath = ApplicationHelper.persistentDataPath .. "/" .. NetIngScenicSpotPhotoNew.Ins():GetTempDownloadPathOfLocal(photoID, true, extension)
      NetIngScenicSpotPhotoNew.Ins():Download(role_id, scenic_spot_id, photoID, timestamp, function(x)
        local progressValue = x
        self.callback:FireProgress(photoID, progressValue, true)
        if is_through_scenicspotphotocallback and progress_callback ~= nil then
        ...
        if is_through_personalphotocallback and success_callback ~= nil then
        ...
        if is_through_scenicspotphotocallback and error_callback ~= nil then
...
function ScenicSpotPhotoNew:GetOriginImage_Share_New(scenic_spot_id, timestamp, progress_callback, success_callback, error_callback, is_keep_previous_callback, is_through_scenicspotphotocallback)
  ...
  if isLatestLocal then
    ...
    if not isDownloading then
      local downloadPath = ApplicationHelper.persistentDataPath .. "/" .. NetIngScenicSpotPhotoNew.Ins():GetTempDownloadPathOfLocal(photoID, true, extension)
      NetIngScenicSpotPhotoNew.Ins():DownloadNew(scenic_spot_id, photoID, timestamp, function(x)
        local progressValue = x
        self.callback:FireProgress(photoID, progressValue, true)
        if is_through_scenicspotphotocallback and progress_callback ~= nil then
          progress_callback(progressValue)
...
function ScenicSpotPhotoNew:GetThumbnail_Share(role_id, scenic_spot_id, timestamp, progress_callback, success_callback, error_callback, is_keep_previous_callback, extension)
  ...
  if isLatestLocal then
    ...
    if not isDownloading and not self:IsGetingOForMakeT(photoID) and not self:IsMakingTWhenGetThumbnail(photoID) then
      local downloadPath = ApplicationHelper.persistentDataPath .. "/" .. NetIngScenicSpotPhotoNew.Ins():GetTempDownloadPathOfLocal(photoID, false, extension)
      NetIngScenicSpotPhotoNew.Ins():Download(role_id, scenic_spot_id, photoID, timestamp, function(x)
        local progressValue = x
        ...
        ScenicSpotPhotoHelperNew.Ins():GetOriginImage_Share(role_id, scenic_spot_id, timestamp, nil, function(x, y)
...
function ScenicSpotPhotoNew:GetThumbnail_Share_New(scenic_spot_id, timestamp, progress_callback, success_callback, error_callback, is_keep_previous_callback)
  ...
  if isLatestLocal then
    ...
    local isDownloading = NetIngScenicSpotPhotoNew.Ins():IsDownloading(photoID, false)
    if not isDownloading and not self:IsGetingOForMakeT(photoID) and not self:IsMakingTWhenGetThumbnail(photoID) then
      local downloadPath = ApplicationHelper.persistentDataPath .. "/" .. NetIngScenicSpotPhotoNew.Ins():GetTempDownloadPathOfLocal(photoID, false)
      NetIngScenicSpotPhotoNew.Ins():DownloadNew(scenic_spot_id, photoID, timestamp, function(x)
        local progressValue = x
        self.callback:FireProgress(photoID, progressValue, false)
      end, function()
        ...
        FunctionTextureScale.ins:Scale(texture, ScenicSpotPhoto.thumbnailCoefficient, function(x)
          Object.DestroyImmediate(texture)
          bytes = ImageConversion.EncodeToJPG(x)
        ...
        self.isGetingOForMakeT[photoID] = 0
        self:GetOriginImage_Share_New(scenic_spot_id, timestamp, nil, function(x, y)
          self.isGetingOForMakeT[photoID] = nil
          local bytesOfOriginImage = x
          local timestampOfOriginImage = y
          local texture = Texture2D(0, 0, TextureFormat.RGB24, false)
  ...
  if isLatestLocal then
    ...
    if not isDownloading and not self:IsGetingOForMakeT(photoID) and not self:IsMakingTWhenGetThumbnail(photoID) then
      ...
      NetIngScenicSpotPhotoNew.Ins():Download(role_id, scenic_spot_id, photoID, timestamp, function(x)
        ...
        self.isGetingOForMakeT[photoID] = 0
        ScenicSpotPhotoHelperNew.Ins():GetOriginImage_Roles(role_id, scenic_spot_id, timestamp, nil, function(x, y)
          self.isGetingOForMakeT[photoID] = nil
          local bytesOfOriginImage = x
          local timestampOfOriginImage = y
          local texture = Texture2D(0, 0, TextureFormat.RGB24, false)
...
function ScenicSpotPhotoNew:SaveAndUpload(role_id, scenic_spot_id, bytes, timestamp, progress_callback, success_callback, error_callback)
  ...
  if self:IsSaving(scenic_spot_id) then
    ...
    m = m or 0
    m = m + 1
    self.isSaving[scenic_spot_id] = m
    LocalScenicSpotPhotoNew.Ins():SaveAsync_Share(scenic_spot_id, bytes, timestamp, function()
      local tempUploadRootPath = ApplicationHelper.persistentDataPath .. "/" .. NetIngScenicSpotPhotoNew.Ins():GetTempUploadRootPathOfLocal()
      if not FileHelper.ExistDirectory(tempUploadRootPath) then
        FileHelper.CreateDirectory(tempUploadRootPath)
      end
      local tempUploadPathOfLocal = ApplicationHelper.persistentDataPath .. "/" .. NetIngScenicSpotPhotoNew.Ins():GetTempUploadPathOfLocal(scenic_spot_id)
      FileDirectoryHandler.WriteFile(tempUploadPathOfLocal, bytes, function()
        local n = self.isSaving[scenic_spot_id]
        n = n - 1
        self.isSaving[scenic_spot_id] = n
        if not BranchMgr.IsTW() then
          NetIngScenicSpotPhotoNew.Ins():FormUpload(scenic_spot_id, progress_callback, function()
          ...
          NetIngScenicSpotPhotoNew.Ins():Upload(scenic_spot_id, progress_callback, function()
            FileHelper.DeleteFile(tempUploadPathOfLocal)
            if success_callback ~= nil then
              success_callback()
            end
...
function ScenicSpotPhotoNew:ClearLocal_Share(role_id, scenic_spot_id)
  ...
  local downloadOPath = ApplicationHelper.persistentDataPath .. "/" .. NetIngScenicSpotPhotoNew.Ins():GetTempDownloadPathOfLocal(photoID, true)
  if FileHelper.ExistFile(downloadOPath) then
    FileHelper.DeleteFile(downloadOPath)
  end
  downloadOPath = ApplicationHelper.persistentDataPath .. "/" .. NetIngScenicSpotPhotoNew.Ins():GetTempDownloadPathOfLocal(photoID, true, PhotoFileInfo.OldExtension)
  if FileHelper.ExistFile(downloadOPath) then
    FileHelper.DeleteFile(downloadOPath)
  end
  local downloadTPath = ApplicationHelper.persistentDataPath .. "/" .. NetIngScenicSpotPhotoNew.Ins():GetTempDownloadPathOfLocal(photoID, false)
  if FileHelper.ExistFile(downloadTPath) then
    FileHelper.DeleteFile(downloadTPath)
  end
  downloadTPath = ApplicationHelper.persistentDataPath .. "/" .. NetIngScenicSpotPhotoNew.Ins():GetTempDownloadPathOfLocal(photoID, false, PhotoFileInfo.OldExtension)
  if FileHelper.ExistFile(downloadTPath) then
    FileHelper.DeleteFile(downloadTPath)
  end
  local tempUploadPathOfLocal = ApplicationHelper.persistentDataPath .. "/" .. NetIngScenicSpotPhotoNew.Ins():GetTempUploadPathOfLocal(scenic_spot_id)
  ...
  local tempExtension = PhotoFileInfo.Extension
  PhotoFileInfo.Extension = PhotoFileInfo.OldExtension
  tempUploadPathOfLocal = ApplicationHelper.persistentDataPath .. "/" .. NetIngScenicSpotPhotoNew.Ins():GetTempUploadPathOfLocal(scenic_spot_id)
  if FileHelper.ExistFile(tempUploadPathOfLocal) then
    FileHelper.DeleteFile(tempUploadPathOfLocal)
  end
  PhotoFileInfo.Extension = tempExtension
