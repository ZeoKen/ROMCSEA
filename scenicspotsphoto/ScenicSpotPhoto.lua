...
function ScenicSpotPhoto:GetOriginImage(scenic_spot_id, timestamp, progress_callback, success_callback, error_callback, is_keep_previous_callback, is_through_scenicspotphotocallback, extension)
  if not is_through_scenicspotphotocallback then
    ...
    if self:IsGettingO(scenic_spot_id) then
      return
    end
  ...
  if isLatestLocal then
    ...
    if not isDownloading then
      local downloadPath = ApplicationHelper.persistentDataPath .. "/" .. NetIngScenicSpotPhoto.Ins():GetTempDownloadPathOfLocal(scenic_spot_id, true, extension)
      NetIngScenicSpotPhoto.Ins():Download(scenic_spot_id, timestamp, function(x)
...
function ScenicSpotPhoto:GetThumbnail(scenic_spot_id, timestamp, progress_callback, success_callback, error_callback, is_keep_previous_callback, extension)
  ...
  if self:IsGettingT(scenic_spot_id) then
    return
  end
  ...
  if isLatestLocal then
    ...
    if not isDownloading and not self:IsGetingOForMakeT(scenic_spot_id) and not self:IsMakingTWhenGetThumbnail(scenic_spot_id) then
      local downloadPath = ApplicationHelper.persistentDataPath .. "/" .. NetIngScenicSpotPhoto.Ins():GetTempDownloadPathOfLocal(scenic_spot_id, false, extension)
      NetIngScenicSpotPhoto.Ins():Download(scenic_spot_id, timestamp, function(x)
        local progressValue = x
        ...
        ScenicSpotPhotoHelper.Ins():GetOriginImage(scenic_spot_id, timestamp, nil, function(x, y)
...
function ScenicSpotPhoto:SaveAndUpload(scenic_spot_id, bytes, timestamp, progress_callback, success_callback, error_callback)
  if self:IsSaving(scenic_spot_id) then
    ...
    LocalScenicSpotPhoto.Ins():SaveAsync(scenic_spot_id, bytes, timestamp, function()
      local tempUploadRootPath = ApplicationHelper.persistentDataPath .. "/" .. NetIngScenicSpotPhoto.Ins():GetTempUploadRootPathOfLocal()
      if not FileHelper.ExistDirectory(tempUploadRootPath) then
        FileHelper.CreateDirectory(tempUploadRootPath)
      end
      local tempUploadPathOfLocal = ApplicationHelper.persistentDataPath .. "/" .. NetIngScenicSpotPhoto.Ins():GetTempUploadPathOfLocal(scenic_spot_id)
      FileDirectoryHandler.WriteFile(tempUploadPathOfLocal, bytes, function()
        ...
        if not BranchMgr.IsTW() then
          NetIngScenicSpotPhoto.Ins():FormUpload(scenic_spot_id, progress_callback, function()
          ...
          NetIngScenicSpotPhoto.Ins():Upload(scenic_spot_id, progress_callback, function()
            NetIngScenicSpotPhoto.Ins():SetExist(Game.Myself.data.id, scenic_spot_id)
            FileHelper.DeleteFile(tempUploadPathOfLocal)
            if success_callback ~= nil then
              success_callback()
...
function ScenicSpotPhoto:ClearLocal(scenic_spot_id)
  local downloadOPath = ApplicationHelper.persistentDataPath .. "/" .. NetIngScenicSpotPhoto.Ins():GetTempDownloadPathOfLocal(scenic_spot_id, true)
  if FileHelper.ExistFile(downloadOPath) then
    FileHelper.DeleteFile(downloadOPath)
  end
  downloadOPath = ApplicationHelper.persistentDataPath .. "/" .. NetIngScenicSpotPhoto.Ins():GetTempDownloadPathOfLocal(scenic_spot_id, true, PhotoFileInfo.OldExtension)
  if FileHelper.ExistFile(downloadOPath) then
    FileHelper.DeleteFile(downloadOPath)
  end
  local downloadTPath = ApplicationHelper.persistentDataPath .. "/" .. NetIngScenicSpotPhoto.Ins():GetTempDownloadPathOfLocal(scenic_spot_id, false)
  if FileHelper.ExistFile(downloadTPath) then
    FileHelper.DeleteFile(downloadTPath)
  end
  downloadTPath = ApplicationHelper.persistentDataPath .. "/" .. NetIngScenicSpotPhoto.Ins():GetTempDownloadPathOfLocal(scenic_spot_id, false, PhotoFileInfo.OldExtension)
  if FileHelper.ExistFile(downloadTPath) then
    FileHelper.DeleteFile(downloadTPath)
  end
  local tempUploadPathOfLocal = ApplicationHelper.persistentDataPath .. "/" .. NetIngScenicSpotPhoto.Ins():GetTempUploadPathOfLocal(scenic_spot_id)
  ...
  local tempExtension = PhotoFileInfo.Extension
  PhotoFileInfo.Extension = PhotoFileInfo.OldExtension
  tempUploadPathOfLocal = ApplicationHelper.persistentDataPath .. "/" .. NetIngScenicSpotPhoto.Ins():GetTempUploadPathOfLocal(scenic_spot_id)
  if FileHelper.ExistFile(tempUploadPathOfLocal) then
    FileHelper.DeleteFile(tempUploadPathOfLocal)
  end
  PhotoFileInfo.Extension = tempExtension
