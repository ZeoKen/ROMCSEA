...
function SSPUploadStatusManager:SyncUploadStatusToGameServer(s_scenic_spot_id)
  if s_scenic_spot_id ~= nil and 0 < #s_scenic_spot_id then
  ...
  if 0 < #arraySSID then
    if indicator <= #arraySSID then
      ...
      for i = indicator, endIndicator do
        ...
        NetIngScenicSpotPhotoNew.Ins():CheckExist(Game.Myself.data.id, ssID, function()
          ...
          ticketsCount = ticketsCount + 1
          self:DoSyncUploadStatusToGameServer()
        end, PhotoFileInfo.OldExtension)
        ticketsCount = ticketsCount - 1
