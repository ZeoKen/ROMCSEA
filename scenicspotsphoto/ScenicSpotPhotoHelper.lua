...
function ScenicSpotPhotoHelper:GetOriginImage(scenic_spot_id, timestamp, progress_callback, success_callback, error_callback, is_keep_previous_callback, is_through_scenicspotphotocallback)
  ...
  if isLatestLocal then
    ScenicSpotPhoto.Ins():GetOriginImage(scenic_spot_id, timestamp, progress_callback, success_callback, error_callback, is_keep_previous_callback, is_through_scenicspotphotocallback)
  else
    if self.tabCachedParamsO[scenic_spot_id] == nil then
      self.tabCachedParamsO[scenic_spot_id] = {}
    end
    table.insert(self.tabCachedParamsO[scenic_spot_id], {
      timestamp = timestamp,
    ...
    if self.tabIsCheckingExistO[scenic_spot_id] == nil then
      NetIngScenicSpotPhoto.Ins():CheckExist(Game.Myself.data.id, scenic_spot_id, function()
        ...
        if self.tabStopFlagO[scenic_spot_id] == nil then
          ...
          for i = 1, #params do
            ...
            ScenicSpotPhoto.Ins():GetOriginImage(scenic_spot_id, param.timestamp, param.progressCallback, param.successCallback, param.errorCallback, param.isKeepPreviousCallback, param.isThroughPersonalphotocallback, PhotoFileInfo.Extension)
          end
        end
        ...
        if self.tabStopFlagO[scenic_spot_id] == nil then
          ...
          for i = 1, #params do
            local param = params[i]
            ScenicSpotPhoto.Ins():GetOriginImage(scenic_spot_id, param.timestamp, param.progressCallback, param.successCallback, param.errorCallback, param.isKeepPreviousCallback, param.isThroughPersonalphotocallback, PhotoFileInfo.OldExtension)
          end
        end
        self.tabCachedParamsO[scenic_spot_id] = nil
...
function ScenicSpotPhotoHelper:GetThumbnail(scenic_spot_id, timestamp, progress_callback, success_callback, error_callback, is_keep_previous_callback)
  ...
  if isLatestLocal then
    ScenicSpotPhoto.Ins():GetThumbnail(scenic_spot_id, timestamp, progress_callback, success_callback, error_callback, is_keep_previous_callback)
  else
    if self.tabCachedParamsT[scenic_spot_id] == nil then
      self.tabCachedParamsT[scenic_spot_id] = {}
    end
    table.insert(self.tabCachedParamsT[scenic_spot_id], {
      timestamp = timestamp,
      progressCallback = progress_callback,
      successCallback = success_callback,
      errorCallback = error_callback,
    ...
    if self.tabIsCheckingExistT[scenic_spot_id] == nil then
      NetIngScenicSpotPhoto.Ins():CheckExist(Game.Myself.data.id, scenic_spot_id, function()
        ...
        if self.tabStopFlagT[scenic_spot_id] == nil then
          ...
          for i = 1, #params do
            local param = params[i]
            ScenicSpotPhoto.Ins():GetThumbnail(scenic_spot_id, param.timestamp, param.progressCallback, param.successCallback, param.errorCallback, param.isKeepPreviousCallback, PhotoFileInfo.Extension)
          end
        end
        ...
        if self.tabStopFlagT[scenic_spot_id] == nil then
          ...
          for i = 1, #params do
            local param = params[i]
            ScenicSpotPhoto.Ins():GetThumbnail(scenic_spot_id, param.timestamp, param.progressCallback, param.successCallback, param.errorCallback, param.isKeepPreviousCallback, PhotoFileInfo.OldExtension)
          end
        end
        self.tabCachedParamsT[scenic_spot_id] = nil
      end, PhotoFileInfo.Extension)
