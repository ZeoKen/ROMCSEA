...
function UnionWallPhoto:GetOriginImage_Personal(role_id, pos_index, timestamp, progress_callback, success_callback, error_callback, is_keep_previous_callback, is_through_unionwallphotocallback, extension)
  ...
  if isLatestLocal then
    ...
    if is_through_unionwallphotocallback and success_callback ~= nil then
    ...
    if not isDownloading then
      local downloadPath = ApplicationHelper.persistentDataPath .. "/" .. NetIngUnionWallPhoto_Personal.Ins():GetTempDownloadPathOfLocal(photoID, true, extension)
      NetIngUnionWallPhoto_Personal.Ins():Download(role_id, pos_index, photoID, timestamp, function(x)
        local progressValue = x
        self.callback:FireProgress(photoID, progressValue, true)
        if is_through_unionwallphotocallback and progress_callback ~= nil then
        ...
        if is_through_unionwallphotocallback and error_callback ~= nil then
...
function UnionWallPhoto:GetThumbnail_Personal(role_id, pos_index, timestamp, progress_callback, success_callback, error_callback, is_keep_previous_callback, extension)
  ...
  if isLatestLocal then
    ...
    if not isDownloading and not self:IsGetingOForMakeT(photoID) and not self:IsMakingTWhenGetThumbnail(photoID) then
      local downloadPath = ApplicationHelper.persistentDataPath .. "/" .. NetIngUnionWallPhoto_Personal.Ins():GetTempDownloadPathOfLocal(photoID, false, extension)
      NetIngUnionWallPhoto_Personal.Ins():Download(role_id, pos_index, photoID, timestamp, function(x)
        local progressValue = x
        ...
        UnionWallPhotoHelper.Ins():GetOriginImage_Personal(role_id, pos_index, timestamp, nil, function(x, y)
...
function UnionWallPhoto:ClearLocal_Personal(role_id, pos_index)
  local photoID = self:GetPhotoID(role_id, pos_index, "p")
  local downloadOPath = ApplicationHelper.persistentDataPath .. "/" .. NetIngUnionWallPhoto_Personal.Ins():GetTempDownloadPathOfLocal(photoID, true)
  if FileHelper.ExistFile(downloadOPath) then
    FileHelper.DeleteFile(downloadOPath)
  end
  downloadOPath = ApplicationHelper.persistentDataPath .. "/" .. NetIngUnionWallPhoto_Personal.Ins():GetTempDownloadPathOfLocal(photoID, true, PhotoFileInfo.OldExtension)
  if FileHelper.ExistFile(downloadOPath) then
    FileHelper.DeleteFile(downloadOPath)
  end
  local downloadTPath = ApplicationHelper.persistentDataPath .. "/" .. NetIngUnionWallPhoto_Personal.Ins():GetTempDownloadPathOfLocal(photoID, false)
  if FileHelper.ExistFile(downloadTPath) then
    FileHelper.DeleteFile(downloadTPath)
  end
  downloadTPath = ApplicationHelper.persistentDataPath .. "/" .. NetIngUnionWallPhoto_Personal.Ins():GetTempDownloadPathOfLocal(photoID, false, PhotoFileInfo.OldExtension)
  if FileHelper.ExistFile(downloadTPath) then
    FileHelper.DeleteFile(downloadTPath)
  end
...
function UnionWallPhoto:GetOriginImage_ScenicSpot(role_id, scenic_spot_id, timestamp, progress_callback, success_callback, error_callback, is_keep_previous_callback, is_through_unionwallphotocallback, extension)
  ...
  if isLatestLocal then
    ...
    if not isDownloading then
      local downloadPath = ApplicationHelper.persistentDataPath .. "/" .. NetIngUnionWallPhoto_ScenicSpot.Ins():GetTempDownloadPathOfLocal(photoID, true, extension)
      NetIngUnionWallPhoto_ScenicSpot.Ins():Download(role_id, scenic_spot_id, photoID, timestamp, function(x)
        local progressValue = x
        self.callback:FireProgress(photoID, progressValue, true)
        if is_through_unionwallphotocallback and progress_callback ~= nil then
          progress_callback(progressValue)
...
function UnionWallPhoto:GetThumbnail_ScenicSpot(role_id, scenic_spot_id, timestamp, progress_callback, success_callback, error_callback, is_keep_previous_callback, extension)
  ...
  if isLatestLocal then
    ...
    if not isDownloading and not self:IsGetingOForMakeT(photoID) and not self:IsMakingTWhenGetThumbnail(photoID) then
      local downloadPath = ApplicationHelper.persistentDataPath .. "/" .. NetIngUnionWallPhoto_ScenicSpot.Ins():GetTempDownloadPathOfLocal(photoID, false, extension)
      NetIngUnionWallPhoto_ScenicSpot.Ins():Download(role_id, scenic_spot_id, photoID, timestamp, function(x)
        local progressValue = x
        self.callback:FireProgress(photoID, progressValue, false)
      end, function()
        ...
        self.isGetingOForMakeT[photoID] = 0
        UnionWallPhotoHelper.Ins():GetOriginImage_ScenicSpot(role_id, scenic_spot_id, timestamp, nil, function(x, y)
          self.isGetingOForMakeT[photoID] = nil
          local bytesOfOriginImage = x
          local timestampOfOriginImage = y
          local texture = Texture2D(0, 0, TextureFormat.RGB24, false)
...
function UnionWallPhoto:ClearLocal_ScenicSpot(role_id, scenic_spot_id)
  local photoID = self:GetPhotoID(role_id, scenic_spot_id, "s")
  local downloadOPath = ApplicationHelper.persistentDataPath .. "/" .. NetIngUnionWallPhoto_ScenicSpot.Ins():GetTempDownloadPathOfLocal(photoID, true)
  if FileHelper.ExistFile(downloadOPath) then
    FileHelper.DeleteFile(downloadOPath)
  end
  downloadOPath = ApplicationHelper.persistentDataPath .. "/" .. NetIngUnionWallPhoto_ScenicSpot.Ins():GetTempDownloadPathOfLocal(photoID, true, PhotoFileInfo.OldExtension)
  if FileHelper.ExistFile(downloadOPath) then
    FileHelper.DeleteFile(downloadOPath)
  end
  local downloadTPath = ApplicationHelper.persistentDataPath .. "/" .. NetIngUnionWallPhoto_ScenicSpot.Ins():GetTempDownloadPathOfLocal(photoID, false)
  if FileHelper.ExistFile(downloadTPath) then
    FileHelper.DeleteFile(downloadTPath)
  end
  downloadTPath = ApplicationHelper.persistentDataPath .. "/" .. NetIngUnionWallPhoto_ScenicSpot.Ins():GetTempDownloadPathOfLocal(photoID, false, PhotoFileInfo.OldExtensio)
  if FileHelper.ExistFile(downloadTPath) then
    FileHelper.DeleteFile(downloadTPath)
  end
  LocalUnionWallPhoto.Ins():Delete(photoID, true)
