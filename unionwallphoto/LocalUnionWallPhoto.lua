...
function LocalUnionWallPhoto:Delete(photo_id, o_or_t)
  ...
  for i = 1, #sFileName do
    ...
    local startIndex = string.find(fileName, photo_id .. divideCharacter)
    if startIndex ~= nil and startIndex == 1 then
      local localPath = ApplicationHelper.persistentDataPath .. "/" .. self:AssemblePathOfLocal_FileName(fileName, o_or_t)
...
function LocalUnionWallPhoto:DeleteAllPhotosOfRole(role_id)
  local rootPath = ApplicationHelper.persistentDataPath .. "/" .. self:GetRootPathOfLocal(true)
  local sFileName = FileHelper.GetChildrenName(rootPath)
  for i = 1, #sFileName do
    local fileName = sFileName[i]
    local nameExceptExtension = StringUtility.Split(fileName, ".")[1]
    ...
    if roleID == role_id then
      local localPath = ApplicationHelper.persistentDataPath .. "/" .. self:AssemblePathOfLocal_FileName(fileName, true)
      FileHelper.DeleteFile(localPath)
    end
  end
  rootPath = ApplicationHelper.persistentDataPath .. "/" .. self:GetRootPathOfLocal(false)
  ...
  for i = 1, #sFileName do
    ...
    local roleID = tonumber[strRoleID]
    if roleID == role_id then
      local localPath = ApplicationHelper.persistentDataPath .. "/" .. self:AssemblePathOfLocal_FileName(fileName, false)
      FileHelper.DeleteFile(localPath)
    end
  end
end
