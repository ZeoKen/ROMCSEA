...
function UnionWallPhotoHelper:GetOriginImage_Personal(role_id, pos_index, timestamp, progress_callback, success_callback, error_callback, is_keep_previous_callback, is_through_unionwallphotocallback)
  ...
  if isLatestLocal then
    UnionWallPhotoNew.Ins():GetOriginImage_Personal(role_id, pos_index, timestamp, progress_callback, success_callback, error_callback, is_keep_previous_callback, is_through_personalphotocallback)
  else
    ...
    if self.tabIsCheckingExistO[photoID] == nil then
      NetIngUnionWallPhoto_Personal.Ins():CheckExist(role_id, pos_index, function()
        ...
        if self.tabStopFlagO[photoID] == nil then
          ...
          for i = 1, #params do
            ...
            UnionWallPhotoNew.Ins():GetOriginImage_Personal(role_id, pos_index, param.timestamp, param.progressCallback, param.successCallback, param.errorCallback, param.isKeepPreviousCallback, param.isThroughPersonalphotocallback, PhotoFileInfo.Extension)
          end
        end
        ...
        if self.tabStopFlagO[photoID] == nil then
          ...
          for i = 1, #params do
            local param = params[i]
            UnionWallPhotoNew.Ins():GetOriginImage_Personal(role_id, pos_index, param.timestamp, param.progressCallback, param.successCallback, param.errorCallback, param.isKeepPreviousCallback, param.isThroughPersonalphotocallback, PhotoFileInfo.OldExtension)
          end
        end
        self.tabCachedParamsO[photoID] = nil
...
function UnionWallPhotoHelper:GetThumbnail_Personal(role_id, pos_index, timestamp, progress_callback, success_callback, error_callback, is_keep_previous_callback)
  ...
  if isLatestLocal then
    UnionWallPhotoNew.Ins():GetThumbnail_Personal(role_id, pos_index, timestamp, progress_callback, success_callback, error_callback, is_keep_previous_callback)
  else
    ...
    if self.tabIsCheckingExistT[photoID] == nil then
      NetIngUnionWallPhoto_Personal.Ins():CheckExist(role_id, pos_index, function()
        ...
        if self.tabStopFlagT[photoID] == nil then
          ...
          for i = 1, #params do
            local param = params[i]
            UnionWallPhotoNew.Ins():GetThumbnail_Personal(role_id, pos_index, param.timestamp, param.progressCallback, param.successCallback, param.errorCallback, param.isKeepPreviousCallback, PhotoFileInfo.Extension)
          end
        end
        ...
        if self.tabStopFlagT[photoID] == nil then
          ...
          for i = 1, #params do
            local param = params[i]
            UnionWallPhotoNew.Ins():GetThumbnail_Personal(role_id, pos_index, param.timestamp, param.progressCallback, param.successCallback, param.errorCallback, param.isKeepPreviousCallback, PhotoFileInfo.OldExtension)
          end
        end
        self.tabCachedParamsT[photoID] = nil
      end, PhotoFileInfo.Extension)
...
function UnionWallPhotoHelper:GetOriginImage_ScenicSpot(role_id, scenic_spot_id, timestamp, progress_callback, success_callback, error_callback, is_keep_previous_callback, is_through_unionwallphotocallback)
  ...
  if isLatestLocal then
    UnionWallPhotoNew.Ins():GetOriginImage_ScenicSpot(role_id, scenic_spot_id, timestamp, progress_callback, success_callback, error_callback, is_keep_previous_callback, is_through_personalphotocallback)
  else
    if self.tabCachedParamsO[photoID] == nil then
      self.tabCachedParamsO[photoID] = {}
    end
    ...
    if self.tabIsCheckingExistO[photoID] == nil then
      NetIngUnionWallPhoto_ScenicSpot.Ins():CheckExist(role_id, scenic_spot_id, function()
        self.tabIsCheckingExistO[photoID] = nil
        if self.tabStopFlagO[photoID] == nil then
          local params = self.tabCachedParamsO[photoID]
          if not params then
          ...
          for i = 1, #params do
            local param = params[i]
            UnionWallPhotoNew.Ins():GetOriginImage_ScenicSpot(role_id, scenic_spot_id, param.timestamp, param.progressCallback, param.successCallback, param.errorCallback, param.isKeepPreviousCallback, param.isThroughPersonalphotocallback, PhotoFileInfo.Extension)
          end
        end
        self.tabCachedParamsO[photoID] = nil
      end, function()
        ...
        if self.tabStopFlagO[photoID] == nil then
          ...
          for i = 1, #params do
            local param = params[i]
            UnionWallPhotoNew.Ins():GetOriginImage_ScenicSpot(role_id, scenic_spot_id, param.timestamp, param.progressCallback, param.successCallback, param.errorCallback, param.isKeepPreviousCallback, param.isThroughPersonalphotocallback, PhotoFileInfo.OldExtension)
          end
        end
        self.tabCachedParamsO[photoID] = nil
      end, PhotoFileInfo.Extension)
...
function UnionWallPhotoHelper:GetThumbnail_ScenicSpot(role_id, scenic_spot_id, timestamp, progress_callback, success_callback, error_callback, is_keep_previous_callback)
  ...
  if isLatestLocal then
    UnionWallPhotoNew.Ins():GetThumbnail_ScenicSpot(role_id, scenic_spot_id, timestamp, progress_callback, success_callback, error_callback, is_keep_previous_callback)
  else
    if self.tabCachedParamsT[photoID] == nil then
      self.tabCachedParamsT[photoID] = {}
    end
    ...
    if self.tabIsCheckingExistT[photoID] == nil then
      NetIngUnionWallPhoto_ScenicSpot.Ins():CheckExist(role_id, scenic_spot_id, function()
        ...
        if self.tabStopFlagT[photoID] == nil then
          ...
          for i = 1, #params do
            local param = params[i]
            UnionWallPhotoNew.Ins():GetThumbnail_ScenicSpot(role_id, scenic_spot_id, param.timestamp, param.progressCallback, param.successCallback, param.errorCallback, param.isKeepPreviousCallback, PhotoFileInfo.Extension)
          end
        end
        self.tabCachedParamsT[photoID] = nil
      end, function()
        ...
        if self.tabStopFlagT[photoID] == nil then
          ...
          for i = 1, #params do
            local param = params[i]
            UnionWallPhotoNew.Ins():GetThumbnail_ScenicSpot(role_id, scenic_spot_id, param.timestamp, param.progressCallback, param.successCallback, param.errorCallback, param.isKeepPreviousCallback, PhotoFileInfo.OldExtension)
          end
        end
        self.tabCachedParamsT[photoID] = nil
      end, PhotoFileInfo.Extension)
