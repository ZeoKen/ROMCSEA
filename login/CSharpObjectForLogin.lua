...
local goGameRoleReadyForLogin, transCamera, cameraController
...
function CSharpObjectForLogin:GetRoleParnet()
  if not goGameRoleReadyForLogin then
    return
  end
  return Game.GameObjectUtil:DeepFindChild(goGameRoleReadyForLogin, "cp_1")
end

function CSharpObjectForLogin:GetMountParent()
  if not goGameRoleReadyForLogin then
    return
  end
  return Game.GameObjectUtil:DeepFindChild(goGameRoleReadyForLogin, "cp_2")
end
