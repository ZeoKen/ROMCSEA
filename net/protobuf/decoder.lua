...
ReadTag = pb.read_tag
local _SimpleDecoder = function(wire_type, decode_value)
...
local _ModifiedDecoder = function(wire_type, decode_value, modify_value)
...
local _StructPackDecoder = function(wire_type, value_size, format)
...
function _SkipVarint(buffer, pos, pend, descriptorName)
  local value
  value, pos = _DecodeVarint(buffer, pos)
  return pos
end

function _SkipFixed64(buffer, pos, pend, descriptorName)
  pos = pos + 8
  if pend < pos then
    error("Truncated message." .. descriptorName)
  end
  return pos
end

function _SkipLengthDelimited(buffer, pos, pend, descriptorName)
  local size
...
function _SkipFixed32(buffer, pos, pend, descriptorName)
  pos = pos + 4
  if pend < pos then
    error("Truncated message." .. descriptorName)
  end
...
function _RaiseInvalidWireType(buffer, pos, pend, descriptorName)
  error("Tag had invalid wire type." .. descriptorName)
end
...
function _FieldSkipper()
  ...
  local sub = string.sub
  return function(buffer, pos, pend, tag_bytes, descriptorName)
    local wire_type = ord(sub(tag_bytes, 1, 1)) % 8 + 1
    return WIRETYPE_TO_SKIPPER[wire_type](buffer, pos, pend, descriptorName)
  end
end
