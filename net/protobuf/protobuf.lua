...
autoImport("text_format")
autoImport("wire_format")
autoImport("type_checkers")
autoImport("encoder")
autoImport("decoder")
autoImport("listener")
autoImport("containers")
...
local MessageType = FieldDescriptor.TYPE_MESSAGE
module("protobuf")
local make_descriptor = function(name, descriptor, usable_key)
  local meta = {
...
local IsTypePackable = function(field_type)
  return NON_PACKABLE_TYPES[field_type] == nil
end
local GetTypeChecker = function(cpp_type, field_type)
...
local _DefaultValueConstructorForField = function(field)
  ...
  if field.cpp_type == FieldDescriptor.CPPTYPE_MESSAGE then
    local message_type = field.message_type
    return function(message)
      result = ProtobufPool_Get(message_type._concrete_class)
...
local _AttachFieldHelpers = function(message_meta, field_descriptor)
...
local _AddEnumValues = function(descriptor, message_meta)
...
local _InitMethod = function(message_meta)
  return function()
    local self = {}
...
local _AddPropertiesForRepeatedField = function(field, message_meta)
...
local _AddPropertiesForNonRepeatedCompositeField = function(field, message_meta)
  local property_name = field.name
  local message_type = field.message_type
  message_meta._getter[property_name] = function(self)
    local field_value = self._fields[field]
    if field_value == nil then
      field_value = ProtobufPool_Get(message_type._concrete_class)
...
local _AddPropertiesForNonRepeatedScalarField = function(field, message)
  local property_name = field.name
  local type_checker = GetTypeChecker(field.cpp_type, field.type)
...
local _AddPropertiesForField = function(field, message_meta)
...
local _ED_meta = {
  __index = function(self, extension_handle)
    ...
    elseif extension_handle.cpp_type == FieldDescriptor.CPPTYPE_MESSAGE then
      value = ProtobufPool_Get(extension_handle.message_type._concrete_class)
...
local _ExtensionDict = function(message)
  local o = {}
  o._extended_message = message
  return setmetatable(o, _ED_meta)
end
local _AddPropertiesForFields = function(descriptor, message_meta)
...
local _AddPropertiesForExtensions = function(descriptor, message_meta)
...
local _AddStaticMethods = function(message_meta)
...
local _IsPresent = function(descriptor, value)
...
local _AddListFieldsMethod = function(message_descriptor, message_meta)
...
local _AddHasFieldMethod = function(message_descriptor, message_meta)
...
local _AddClearFieldMethod = function(message_descriptor, message_meta)
...
local _AddClearExtensionMethod = function(message_meta)
...
local REPEATED_LABEL = FieldDescriptor.LABEL_REPEATED

local function _AddBackToPool(self)
  if self._fields then
    local class, poolNum
    for k, v in pairs(self._fields) do
      if k.type == MessageType then
        class = k.message_type._concrete_class
        poolNum = class.PoolNum
        if k.label == REPEATED_LABEL then
          for i = 1, #v do
            _AddBackToPool(v[i])
            ProtobufPool_Add(class, v[i], poolNum)
            v[i] = nil
          end
          if v._message_descriptor ~= nil then
            v._isActive = false
            ProtobufPool_Add(1, v)
          end
        else
          _AddBackToPool(v)
          ProtobufPool_Add(class, v, poolNum)
        end
      end
      self._fields[k] = nil
    end
...
local _AddClearMethod = function(message_descriptor, message_meta)
...
local _AddStrMethod = function(message_meta)
...
local _AddHasExtensionMethod = function(message_meta)
...
local _AddSetListenerMethod = function(message_meta)
...
local _AddByteSizeMethod = function(message_descriptor, message_meta)
...
local _AddSerializeToStringMethod = function(message_descriptor, message_meta)
...
local _AddSerializePartialToStringMethod = function(message_descriptor, message_meta)
...
local _AddMergeFromStringMethod = function(message_descriptor, message_meta)
  ...
  local _internal_parse = function(self, buffer, pos, pend)
    ...
    while pos ~= pend do
      ...
      if field_decoder == nil then
        new_pos = SkipField(buffer, new_pos, pend, tag_bytes, message_descriptor.name)
        if new_pos == -1 then
          return pos
        end
        pos = new_pos
  ...
  local internal_parse1 = function(self, address, offset, len)
    message_meta._member._Modified(self)
    local field_dict = self._fields
    local tag_bytes, new_pos, field_decoder
    while pos ~= len do
      tag_bytes, new_pos = ReadTag(address, offset)
      field_decoder = decoders_by_tag[tag_bytes]
      if field_decoder == nil then
        new_pos = SkipField(buffer, new_pos, len, tag_bytes, message_descriptor.name)
        if new_pos == -1 then
          return pos
        end
        pos = new_pos
      else
        pos = field_decoder(buffer, new_pos, len, self, field_dict)
      end
    end
    return pos
  end
  message_meta._member._InternalParse1 = internal_parse1
  ...
  local merge_from_string1 = function(self, address, offset, len)
    local length = len
    if internal_parse1(self, address, offset, length) ~= length then
      error("Unexpected end-group tag.")
    end
    return length
  end
  message_meta._member.MergeFromString1 = merge_from_string1
  ...
  function message_meta._member:ParseFromString1(address, offset, len)
    message_meta._member.Clear(self)
    merge_from_string1(self, address, offset, len)
  end
end
local _AddIsInitializedMethod = function(message_descriptor, message_meta)
...
local _AddMergeFromMethod = function(message_meta)
...
local _AddMessageMethods = function(message_descriptor, message_meta)
...
local _AddPrivateHelperMethods = function(message_meta)
...
local property_getter = function(message_meta)
...
local property_setter = function(message_meta)
...
local Message = function(descriptor)
  local message_meta = {}
