...
function NewServerSignInProxy:InitRewardData()
  ...
  self.dayTypeMap = {}
  self.maxDayCount = 0
  local day, rewardInfo, rewardItem
  for id, data in pairs(self.staticRewardTable) do
    if self:CheckRewardDataValid(data) then
      day = data.ServerID == nil and id or data.Days
...
function NewServerSignInProxy:CheckRewardDataValid(data)
  return data.ServerID == nil or data.ServerID == self.serverLineGroup
end
...
function NewServerSignInProxy:CallSignIn()
  ServiceNUserProxy.Instance:CallSignInUserCmd(nil, SceneUser2_pb.ESIGNINTYPE_DAILY)
end
...
function NewServerSignInProxy:GetStaticTextDataOfDay(day)
  if not self.staticTextTable then
    return
  end
...
function NewServerSignInProxy:GetRewardTypeOfDay(day)
  if not self.dayTypeMap or not next(self.dayTypeMap) then
    return
  end
