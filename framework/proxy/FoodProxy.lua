...
function FoodProxy:InitManualFoodInfo(...)
  local t_item = Table_Item
  for id, v in pairs(Table_Food) do
    if t_item[id] ~= nil then
      self.food_cook_info[id] = {
        itemid = id,
        status = SceneFood_pb.EFOODSTATUS_MIN,
        exp = 0,
        itemData = ItemData.new(nil, id),
        level = 0
      }
    else
...
function FoodProxy:Server_SetFoodLevelInfo(server_FoodInfo)
  ...
  for i = 1, #eat_foods do
    ...
    if foodItemInfo.itemid then
      self.eating_foods[#self.eating_foods + 1] = {
        itemid = foodItemInfo.itemid,
        level = foodItemInfo.level,
        invalidTime = foodItemInfo.invalid_time
      }
    end
  end
  ...
  for k, v in pairs(bufferMap) do
    ...
    if effect.type == "AttrChange" then
      for j, w in pairs(effect) do
        local prop = props:GetPropByName(j)
        ...
        if j ~= "type" and prop then
          ...
          elseif newInvalidBuffTime < oldPropInvalidTime then
            bufferInvalidTimeList[j] = newInvalidBuffTime
          end
        end
      end
...
function FoodProxy:GetManualDataNumByStatus(status, list)
  status = status or SceneFood_pb.EFOODSTATUS_ADD
  local num, single = 0
  for _, data in pairs(self.food_cook_info) do
    single = data.itemData and data.itemData.staticData
    if single and single.AdventureValue and single.AdventureValue ~= 0 and ItemUtil.CheckDateValidByItemId(single.id) and data.status == status then
      num = num + 1
      if list then
