...
ArtifactProxy.Type = {WeaponArtifact = 1, HeadBackArtifact = 2}
...
function ArtifactProxy:Init()
  ...
  self.makeList = {}
  self.makeTable = {}
  self:ProcessLv1Artifact()
  self:InitReturnData()
end

function ArtifactProxy:ProcessLv1Artifact()
  self.autoUnlockId = {}
  for k, v in pairs(Table_Artifact) do
    if v.Level == 1 then
      self.autoUnlockId[v.id] = v.Type
    end
  end
end
...
function ArtifactProxy:_AddAutoUnlock()
  for id, t in pairs(self.autoUnlockId) do
    if not self.produceMap[t] and not self.makeTable[id] then
      self.makeList[#self.makeList + 1] = id
    end
  end
  self:_setMakeTable()
end
...
function ArtifactProxy:IsOverLimitCount(data)
  ...
  if singleLimit <= single then
    return 1
  elseif totalLimit <= total then
    return 2
  end
  return 0
end
...
function ArtifactProxy:ShowFloatAward(itemid)
  ...
  temp.leftBtnText = ZhString.ArtifactMake_DisText
  
  function temp.leftBtnCallback()
    GameFacade.Instance:sendNotification(UIEvent.JumpPanel, {
      view = PanelConfig.GuildInfoView
    })
  end
  
  temp.rightBtnText = ZhString.FloatAwardView_Confirm
  
  function temp.rightBtnCallback()
    GameFacade.Instance:sendNotification(UIEvent.CloseUI, UIViewType.Show3D2DLayer)
  end
  
  FloatAwardView.addItemDatasToShow({itemData}, temp)
