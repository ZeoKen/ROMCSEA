...
local ParseSummaryTypeData = function(enum, valueName)
  if enum then
    WorkSpaceSummaryType[enum] = valueName
  else
    helplog("TeamData----> WorkSpaceSummaryType enum is nil! error valueName:", valueName)
  end
end
ParseSummaryTypeData(ScenePet_pb.EWORKSPACEDATA_PLAYTTIME_COUNT, "playtime_count")
...
function PetWorkSpaceProxy:RecvWorkSpaceDataUpdatePetCmd(data)
  if not data then
    return
  end
  ...
  local spaceData = self.spaceData[spaceid]
  if not spaceData then
    spaceData = {}
    self.spaceData[spaceid] = spaceData
  end
  for i = 1, #data.datas do
    local key = WorkSpaceSummaryType[data.datas[i].type]
    if key then
      local value = data.datas[i].value
      spaceData[key] = value
      local spaceData = self:GetWorkSpaceDataById(spaceid)
      if nil ~= spaceData[key] then
        spaceData[key] = value
      end
    end
  end
...
function PetWorkSpaceProxy:GetSpacePlayTimeCount(id)
  local spaceData = self:GetWorkSpaceDataById(id)
  if spaceData then
    return spaceData.playtime_count
  end
  return 0
end
...
function PetWorkSpaceProxy:SetExtra(data)
  ...
  self:SetMaxWorkSpace(data.max_space)
  local extras = data.extras
  self:SetExchangeMap(extras, true)
end

function PetWorkSpaceProxy:SetMaxWorkSpace(c)
  self.maxSpace = c
end
...
function PetWorkSpaceProxy.calcPetWorkCurDayRewardCount(starttime, endtime, frequency, maxreward, lastcount)
  ...
  if endtime > refreshTime then
    if 1 < (endtime - refreshTime) / DAY_SEC then
...
function PetWorkSpaceProxy:GetIntroduceDescData()
  ...
  for _, v in pairs(unlockData) do
    ...
    if rewardArray and 0 < #rewardArray then
