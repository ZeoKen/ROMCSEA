...
function KFCARCameraProxy:AddEvts()
  EventManager.Me():AddEventListener(PermissionEvent.RequestSuccess, self.RequestPermissionOnJumpPanel, self)
end
...
function KFCARCameraProxy:RequestPermissionOnJumpPanel(requestCode)
  if self.requestCode == requestCode then
    if self.cameraPermissionObtained then
      self.locationPermissionObtained = true
    else
      self.cameraPermissionObtained = true
      self.requestCode = FunctionPermission.Me():RequestFineLocationPermission()
      if self.requestCode == 0 then
        self.locationPermissionObtained = true
      end
    end
    if self.cameraPermissionObtained and self.locationPermissionObtained then
      self:OriJumpPanel()
    end
  end
end
...
function KFCARCameraProxy:JumpPanel()
  self.cameraPermissionObtained = false
  self.locationPermissionObtained = false
  self.requestCode = FunctionPermission.Me():RequestCameraPermission()
  if self.requestCode == 0 then
    self.cameraPermissionObtained = true
    self.requestCode = FunctionPermission.Me():RequestFineLocationPermission()
    if self.requestCode == 0 then
...
function KFCARCameraProxy:OriJumpPanel()
