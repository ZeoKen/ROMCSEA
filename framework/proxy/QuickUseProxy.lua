...
function QuickUseProxy:Init()
  ...
  self.rewardSelectQueue = {}
  self.isRewardSelectQueue = false
end
...
function QuickUseProxy:ContainsFashion(staticID)
  ...
  return map ~= nil and 0 < #map or false
end
...
function QuickUseProxy:AddSkillCallBack(data)
  self:AddQuickDataToQueue(QuickUseProxy.Type.Skill, data, 2)
  GameFacade.Instance:sendNotification(QuickUseProxy.CommonUseEvent)
end
...
function QuickUseProxy:RemoveCommonNow(data)
  self:RemoveCommon(data)
  GameFacade.Instance:sendNotification(QuickUseProxy.CommonUseEvent)
end
...
local TwelveShopIcon = {
  [1] = "12pvp_bg_01s",
  [2] = "12pvp_bg_02s"
}

function QuickUseProxy:AddTwelveTriggerData(trigger, camp)
  if camp then
    trigger.btnStr = ZhString.TwelvePVPShop_ShopTip
    trigger.iconID = TwelveShopIcon[camp]
    trigger.canClose = true
  end
  self:AddQuickDataToQueue(QuickUseProxy.Type.TwelvePVPTrigger, trigger, 1)
  GameFacade.Instance:sendNotification(TriggerEvent.Enter_TwelvePVPShopTrigger)
end

function QuickUseProxy:RemoveTwelveTriggerData(trigger)
  self:RemoveQuickData(trigger, 1)
  GameFacade.Instance:sendNotification(TriggerEvent.Remove_TwelvePVPShopTrigger, trigger)
end

function QuickUseProxy:AddRewardSelectQueue(item)
  table.insert(self.rewardSelectQueue, item)
  if not self.isRewardSelectQueue then
    self.isRewardSelectQueue = true
    self:PlayRewardSelectQueue()
  end
end

function QuickUseProxy:PlayRewardSelectQueue()
  if self.rewardSelectQueue and #self.rewardSelectQueue > 0 then
    local item = self.rewardSelectQueue[1]
    local callbackList = {
      {
        func = function()
          QuickUseProxy.Instance:PlayRewardSelectQueue()
        end
      }
    }
    ...
    local count
    if item.oldNum then
      count = item.num - item.oldNum
    else
      count = item.num
    end
    if not BagProxy.Instance.barrowOpen and not UIManagerProxy.Instance:HasUINode(PanelConfig.RepositoryView) then
      if useEffect.type == "selectreward" then
        GameFacade.Instance:sendNotification(UIEvent.JumpPanel, {
          view = PanelConfig.OptionalGiftRewardView,
          viewdata = {
            gift = item.staticData.id,
    ...
    table.remove(self.rewardSelectQueue, 1)
  else
    self.isRewardSelectQueue = false
  end
end
