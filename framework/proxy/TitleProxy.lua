...
local CheckInvalid = function(appellationId)
  return false
end
...
function TitleProxy:SetServiceData(data)
  ...
  for i = 1, #titles do
    ...
    if not CheckInvalid(title.id) then
...
function TitleProxy:GetCurAchievementTitle()
  ...
  return title and title.config.Name or ZhString.Charactor_NoTitleTip
end
...
function TitleProxy:GetPropsListByTitleId(id)
  local appellation = Table_Appellation[id]
  if appellation then
    local baseProp = appellation.BaseProp
    local result = {}
