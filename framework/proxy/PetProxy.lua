...
local EXIST_PET_CONFIG = GameConfig.Pet.ExistPet or {900100}
...
function PetProxy:GetCaptureDataByEggID(id)
  local petData = Game.Config_EggPet[id]
  if petData == nil then
    return
  end
  return self:GetCaptureDataByPetID(petData.id)
end

function PetProxy:GetPetDataByEggID(id)
  return Game.Config_EggPet[id]
end
...
function PetProxy:Server_UpdateMyBeingInfo(beingid, server_BeingMemberDatas)
  ...
  local oldEaten = beingInfo.eaten
  beingInfo:Server_UpdateData(server_BeingMemberDatas)
  if oldSummon ~= nil and beingInfo.summon ~= oldSummon then
    EventManager.Me():PassEvent(PetEvent.BeingInfoData_SummonChange, beingInfo)
  end
  if oldLive ~= nil and beingInfo.live ~= oldLive or oldEaten and beingInfo.eaten ~= oldEaten then
    EventManager.Me():PassEvent(PetEvent.BeingInfoData_AliveChange, beingInfo)
  end
end
...
function PetProxy:GetMySummonBeingList()
  if self.myBeingsMap == nil then
    return nil
  end
  local list = self.myBeingsList
  if list == nil then
    list = {}
    self.myBeingsList = list
  else
    TableUtility.ArrayClear(list)
  end
  for beingid, beingInfo in pairs(self.myBeingsMap) do
    if beingInfo:IsSummoned() then
      TableUtility.ArrayPushBack(list, beingid)
    end
  end
  return list
end

local ARRAY_FINDINDEX = TableUtility.ArrayFindIndex

function PetProxy:ResetCFGExistPets()
  local items = BagProxy.Instance:GetBagByType(BagProxy.BagType.Pet):GetItems()
  if not self.existPets then
    self.existPets = {}
  else
    TableUtility.ArrayClear(self.existPets)
  end
  for j = 1, #items do
    local item = items[j]
    if item and item.staticData and 0 ~= ARRAY_FINDINDEX(EXIST_PET_CONFIG, item.staticData.id) and 0 == ARRAY_FINDINDEX(self.existPets, item.staticData.id) then
      self.existPets[#self.existPets + 1] = item.staticData.id
    end
  end
  local equipItems = BagProxy.Instance.roleEquip:GetItems() or {}
  for i = 1, #equipItems do
    local equipItem = equipItems[i]
    if 0 ~= ARRAY_FINDINDEX(EXIST_PET_CONFIG, equipItem.staticData.id) and 0 == ARRAY_FINDINDEX(self.existPets, equipItem.staticData.id) then
      self.existPets[#self.existPets + 1] = equipItem.staticData.id
    end
  end
end

function PetProxy:GetExistPet()
  return self.existPets
end

function PetProxy:CheckOpen(eggid)
  if 0 == ARRAY_FINDINDEX(EXIST_PET_CONFIG, eggid) then
    return true
  end
  if self.existPets then
    return 0 ~= ARRAY_FINDINDEX(self.existPets, eggid)
  end
  return true
end
...
function PetProxy:RecvSetPippiStateMessCCmd(data)
  self.pippi_ReviveTime = data.time
  self.pippi_State = data.state
  redlog("RecvSetPippiStateMessCCmd", data.time, data.state)
end

function PetProxy:GetMyPippiInfoData()
  if Game.Myself.data:GetClassID() ~= 745 then
    return
  end
  return self.pippi_State, self.pippi_ReviveTime
end
