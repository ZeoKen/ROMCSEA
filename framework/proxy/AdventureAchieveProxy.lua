...
function AdventureAchieveProxy:getCanGetRewardNum(list)
  local num, items, single = 0
  for _, bagData in pairs(self.bagMap) do
    for _, v in pairs(bagData.tabs) do
      items = v:GetItems()
      for i = 1, #items do
        single = items[i]
        if single:canGetReward() then
...
function AdventureAchieveProxy:QueryUserResumeAchCmd(data)
  ...
  self.bepro_4_time = userResume.bepro_4_time
  self.bepro_5_time = userResume.bepro_5_time
...
function AdventureAchieveProxy:getAllUnlockedAchieve()
  local items = {}
  local ids = {}
  for k, v in pairs(self.bagMap) do
    local tbs = v:GetItems()
    for i = 1, #tbs do
      if not tbs[i]:canGetReward() and tbs[i]:getCompleteString() then
        table.insert(ids, tbs[i].id)
      end
    end
  end
  return ids
...
function AdventureAchieveProxy:getAllUnlockedAchieveProp()
  local achieveIds = self:getAllUnlockedAchieve()
