...
function ShopSaleProxy:InitBagData()
  ...
  self.walletBagData:Reset()
  local walletItems = BagProxy.Instance.walletBagData:GetItems()
  for i = 1, #walletItems do
    self.walletBagData:AddItem(walletItems[i]:Clone())
  end
  local walletWholeTab = self.bagData:GetTab()
  walletWholeTab.dirty = false
  walletWholeTab.parsedItems = {
    unpack(walletWholeTab.items)
  }
end
...
function ShopSaleProxy:AddToWaitSaleItems(guid, nums, price)
  ...
  self:FilterBagItemDatas(ShopSaleProxy.Operation.AddToWaitSaleLst, self.walletBagData)
end
...
function ShopSaleProxy:RemoveOutWaitSaleItems(guid, nums)
  ...
  self:FilterBagItemDatas(ShopSaleProxy.Operation.RemoveFromWaitSaleLst, self.walletBagData)
end
...
function ShopSaleProxy:GetBagData()
  ...
  if bagType then
    ...
    elseif bagType == BagProxy.BagType.Wallet then
      return self.walletBagData
    end
  end
  return nil
end
...
function ShopSaleProxy:GetItemByGuid(guid)
  ...
  if data == nil then
    data = self.walletBagData:GetItemByGuid(guid)
  end
  return data
end
