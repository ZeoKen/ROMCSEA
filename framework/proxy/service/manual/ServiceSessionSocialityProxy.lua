...
function ServiceSessionSocialityProxy:CallQuerySocialData()
  if self._canQuerySocialData then
    ...
    if self._callQuerySocialData == nil or random <= now - self._callQuerySocialData then
...
function ServiceSessionSocialityProxy:RecvQuerySocialData(data)
  ...
  if data.over then
    _SocialManager:Sort()
    BattlePassProxy.Instance:MarkFriendRankDataDirty()
...
function ServiceSessionSocialityProxy:RecvOperateTakeSocialCmd(data)
  ...
  local viewdata = {viewname = "DialogView", npcinfo = target}
...
local Split_OperateQueryConfig = {
  [SessionSociality_pb.EOperateType_MonthCard or 8] = {
    4041,
    4042,
    4043
  }
...
function ServiceSessionSocialityProxy:RecvQueryChargeVirginCmd(data)
  NewRechargeProxy.Ins:Deposit_SetFPRInfo(data)
...
function ServiceSessionSocialityProxy:RecvQueryUserInfoCmd(data)
  self:Notify(ServiceEvent.SessionSocialityQueryUserInfoCmd, data)
  EventManager.Me():DispatchEvent(ServiceEvent.SessionSocialityQueryUserInfoCmd, data)
end

function ServiceSessionSocialityProxy:RecvTutorFuncStateNtfSocialCmd(data)
  TutorProxy.Instance:RecvTutorFuncState(data.open)
