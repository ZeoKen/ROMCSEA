...
function ServiceProxy:handleNotification(note)
  ...
  if evt ~= nil then
    evt(self.viewComponent, note.body)
  end
end
...
function ServiceProxy:SendProto2(msgId, msgParams)
  NetProtocol.SendProto2(msgId, msgParams)
end
...
function ServiceProxy:Notify(event, data, type)
  if event == nil then
    helplog("ServiceProxy::Notify Error: event is nil")
    return
  end
  if Game.FacadeManager:Notify(event) then
    return
  end
