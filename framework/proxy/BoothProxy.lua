...
local _JumpBoothMainView = function(playerID)
  GameFacade.Instance:sendNotification(UIEvent.JumpPanel, {
    view = PanelConfig.BoothMainView,
    viewdata = playerID and {playerID = playerID} or nil
  })
end
...
function BoothProxy:GetExchangeBagSell()
  ...
  self:GetExchangeBagSellByBagType(BagProxy.BagType.Wallet)
  return self.bagSell
end
...
function BoothProxy:CanMapBooth()
  if self:IsUnLockMenu() == false then
    return false
  end
  return self:IsInBoothMap()
end

function BoothProxy:IsUnLockMenu()
  local config = GameConfig.SystemOpen_MenuId
  local menuid = config and config.BoothShopping or 9
  return FunctionUnLockFunc.Me():CheckCanOpen(menuid)
end

function BoothProxy:IsInBoothMap()
...
function BoothProxy:JumpBoothView()
  local isInGagTime, time = MyselfProxy.Instance:IsInGagTime()
  if isInGagTime then
    MsgManager.ShowMsgByID(91, math.floor(time / 60))
    return
  end
  GameFacade.Instance:sendNotification(UIEvent.JumpPanel, {
...
function BoothProxy:SafeJumpBoothMainView(playerID)
  FunctionSecurity.Me():TryDoRealNameCentify(_JumpBoothMainView, playerID)
end
...
function BoothProxy:RecvTakeAllLog(data)
  local info
  local _TradeType = BoothProxy.TradeType.Booth
  for i = 1, #data.infos do
    info = data.infos[i]
    if info.tradetype == _TradeType then
      self:_TakeLog(info.id, info.logtype)
    end
  end
end

function BoothProxy:_TakeLog(id, type)
