...
function GvgLandInfoPopUp:UpdatePopUp()
  self.ruleGuild, self.ruleGuild_type = GvgProxy.Instance:GetRuleGuildInfo(self.flagid, self.groupid)
  if self.ruleGuild ~= nil and self.ruleGuild_type == 2 and (self.ruleGuild.guildid == nil or self.ruleGuild.guildid == 0) then
    self.ruleGuild = nil
  end
  if self.ruleGuild then
    self.noneTip:SetActive(false)
    self.level.text = "Lv." .. self.ruleGuild.lv
    self.guildName.text = self.ruleGuild.name
    self.guildMemberNum.text = self.ruleGuild.membercount
    self.guildLevel.text = self.ruleGuild.lv
