...
function ExchangeShopView:AddEvt()
  self:RegistShowGeneralHelpByHelpID(1800, self.helpBtn)
end
...
function ExchangeShopView:HandleCloseQuickBuyView()
  TipManager.Instance:ShowExchangeGoodsTip(self.curData, self.tipStick, NGUIUtil.AnchorSide.Center, {0, 0})
end
...
function ExchangeShopView:OnEnter()
  EventManager.Me():AddEventListener(QuickBuyEvent.CloseUI, self.HandleCloseQuickBuyView, self)
end

function ExchangeShopView:OnExit()
  EventManager.Me():RemoveEventListener(QuickBuyEvent.CloseUI, self.HandleCloseQuickBuyView, self)
