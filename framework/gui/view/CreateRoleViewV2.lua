...
function CreateRoleViewV2:Init()
  Buglylog("CreateRoleViewV2:Init")
  ...
  local helpButton = self:LoadPreferb("cell/HelpButtonCell", self.goNameInput)
  helpButton.transform.localPosition = LuaGeometry.GetTempVector3(290, 8, 0)
  helpButton.transform.localScale = LuaGeometry.GetTempVector3(1.2, 1.2, 1)
  self:AddClickEvent(self:FindGO("HelpButton", helpButton), function(go)
...
function CreateRoleViewV2:OnExit()
  Buglylog("CreateRoleViewV2:OnExit")
...
function CreateRoleViewV2:DisableInput()
  local colliders = Game.GameObjectUtil:GetAllComponentsInChildren(self.gameObject, BoxCollider, false)
...
function CreateRoleViewV2:UpdateProfessionInfo()
  ...
  color = professionType == 1 and ColorUtil.CareerIconBg1 or professionType == 2 and ColorUtil.CareerIconBg2 or professionType == 3 and ColorUtil.CareerIconBg3 or professionType == 4 and ColorUtil.CareerIconBg4 or professionType == 5 and ColorUtil.CareerIconBg5 or professionType == 6 and ColorUtil.CareerIconBg6 or professionType == 8 and ColorUtil.CareerIconBg7 or nil
  tempsym.color = color
  ...
  IconManager:SetNewProfessionIcon(professionIconSpriteName, spriteProfessionIcon)
...
function CreateRoleViewV2:EnterInGame(note)
  ...
  if not Game.GameObjectUtil:ObjectIsNULL(goMainCamera) then
...
function CreateRoleViewV2:CurrentOccupationHairStylesID()
  local hairList
  if self.sex == E_Gender.Male then
    hairList = CharacterGender.Male.hairList
  else
    hairList = CharacterGender.Female.hairList
  end
  return hairList[self.number] or hairList.default
end
...
function CreateRoleViewV2:OnReceiveSelectedFromFSC(data)
  ...
  if BranchMgr.IsJapan() and PlayerPrefs.GetString("firstCreate") == "" then
