...
function BoothExchangeView:OnEnter()
  local playerChat = self:FindGO("Sprite", self:FindGO("PlayerChat")):GetComponent(UISprite)
  IconManager:SetUIIcon("tab_icon_28", playerChat)
...
function BoothExchangeView:UpdateShow()
  ...
  if data then
    ...
    if self.isSelf then
      ...
      if 0 < count and _BoothProxy:IsSimulateMode() then
...
function BoothExchangeView:ClickBagSell(cell)
  local data = cell.data
  if data ~= nil then
    local hasQuench = data:HasQuench()
    if hasQuench then
...
function BoothExchangeView:OnDestroy()
  local cells = self.itemWrapHelper:GetCells()
  for i = 1, #cells do
    cells[i]:OnDestroy()
  end
  self.itemWrapHelper:Destroy()
  self.bagWrapHelper:Destroy()
  BoothExchangeView.super.OnDestroy(self)
end
