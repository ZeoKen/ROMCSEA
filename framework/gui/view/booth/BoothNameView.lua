...
function BoothNameView:FindObjs()
  ...
  if FunctionPerformanceSetting.CheckInputForbidden() then
    local inputBoxColider = self.input.gameObject:GetComponent(BoxCollider)
    inputBoxColider.enabled = false
  end
...
function BoothNameView:AddEvts()
  ...
  self:AddSelectEvent(self.input, function(go, state)
    if state and FunctionUnLockFunc.Me():ForbidInput(FunctionUnLockFunc.ClientInputForbiddenType.BoothName) then
...
function BoothNameView:UpdateName()
  ...
  self.input.value = string.format(ZhString.Booth_Name, Game.Myself.data:GetName())
end
...
function BoothNameView:ClickConfirm()
  ...
  if self.playerID ~= nil then
    ...
    BoothProxy.Instance:SetSimulateMode(true, self.input.value)
    BoothProxy.Instance:SafeJumpBoothMainView()
    self:CloseSelf()
  end
end
