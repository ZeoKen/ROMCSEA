PersonalArtifactRefreshShareView = class("PersonalArtifactRefreshShareView", BaseView)
autoImport("PhotographResultPanel")
autoImport("ItemTipBaseCell")
PersonalArtifactRefreshShareView.ViewType = UIViewType.ShareLayer
PersonalArtifactRefreshShareView.ShareType = {
  NormalShare = 1,
  SkadaShare = 2,
  RaidResultShare = 3
}

function PersonalArtifactRefreshShareView:Init()
  self:initView()
  self:initData()
end

function PersonalArtifactRefreshShareView:initView()
...
function PersonalArtifactRefreshShareView:OnHideWeekShareTip()
  local rewardTips = self:FindGO("WeekRewardTips")
  if rewardTips then
    rewardTips:SetActive(false)
  end
...
function PersonalArtifactRefreshShareView:ShareToGlobalChannel()
  if ApplicationInfo.IsRunOnWindowns() then
    MsgManager.ShowMsgByID(43486)
    return
  end
  local sharedata = {}
  sharedata.type = ESHAREMSGTYPE.ESHARE_REMOULD_ARTIFACT
...
function PersonalArtifactRefreshShareView:setItemProperty(itemData)
  local label = ""
...
function PersonalArtifactRefreshShareView:OnEnter()
  self:SetData(self.viewdata.viewdata)
  local parent = self.Container.transform
  local effectPath = ResourcePathHelper.EffectCommon("PersonalArtifactRefreshShareView")
  self.focusEffect = Game.AssetManager_UI:CreateAsset(effectPath, parent)
  self.itemCell:SetData(self.viewdata.viewdata)
end

function PersonalArtifactRefreshShareView:InitItemCell(container)
...
function PersonalArtifactRefreshShareView:SetData(data)
  self.data = data
...
function PersonalArtifactRefreshShareView:GetGameObjects()
end

function PersonalArtifactRefreshShareView:RegisterButtonClickEvent()
end

function PersonalArtifactRefreshShareView:changeUIState(isStart)
  if isStart then
...
function PersonalArtifactRefreshShareView:initData()
...
function PersonalArtifactRefreshShareView:OnExit()
end
...
function PersonalArtifactRefreshShareView:SharePicture(platform_type, content_title, content_body)
  if ApplicationInfo.IsRunOnWindowns() then
    MsgManager.ShowMsgByID(43486)
    return
  end
  helplog("PersonalArtifactRefreshShareView SharePicture", platform_type)
  local weekReward = MyselfProxy.Instance:GetAccVarValueByType(Var_pb.EACCVARTYPE_SHARE_WEEK_REWARD) or 0
