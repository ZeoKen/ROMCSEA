EquipConvertResultShareView = class("EquipConvertResultShareView", BaseView)
autoImport("PhotographResultPanel")
EquipConvertResultShareView.ViewType = UIViewType.ShareLayer

function EquipConvertResultShareView:Init()
  self:initView()
  self:initData()
end

function EquipConvertResultShareView:initView()
  ...
  self:AddClickEvent(close, function()
    self:sendNotification(UIEvent.JumpPanel, {
      ...
      viewdata = self.viewdata.viewdata
    })
    self:CloseSelf()
  end)
...
function EquipConvertResultShareView:OnHideWeekShareTip()
  local rewardTips = self:FindGO("WeekRewardTips")
  if rewardTips then
    rewardTips:SetActive(false)
  end
...
function EquipConvertResultShareView:ShareToGlobalChannel()
  if ApplicationInfo.IsRunOnWindowns() then
    MsgManager.ShowMsgByID(43486)
    return
  end
  local sharedata = {}
  sharedata.type = ESHAREMSGTYPE.ESHARE_NEW_EQUIP
...
function EquipConvertResultShareView:FormatBufferStr(bufferId)
...
function EquipConvertResultShareView:setItemProperty(data)
  local label = ""
...
function EquipConvertResultShareView:InitItemCell(container)
...
function EquipConvertResultShareView:GetGameObjects()
end

function EquipConvertResultShareView:RegisterButtonClickEvent()
end

function EquipConvertResultShareView:changeUIState(isStart)
  if isStart then
...
function EquipConvertResultShareView:initData()
...
function EquipConvertResultShareView:OnExit()
end
...
function EquipConvertResultShareView:SharePicture(platform_type, content_title, content_body)
  if ApplicationInfo.IsRunOnWindowns() then
    MsgManager.ShowMsgByID(43486)
    return
  end
  helplog("EquipConvertResultShareView SharePicture", platform_type)
  local weekReward = MyselfProxy.Instance:GetAccVarValueByType(Var_pb.EACCVARTYPE_SHARE_WEEK_REWARD) or 0
