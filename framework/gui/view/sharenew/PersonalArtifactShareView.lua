PersonalArtifactShareView = class("PersonalArtifactShareView", BaseView)
autoImport("PhotographResultPanel")
autoImport("ItemTipBaseCell")
PersonalArtifactShareView.ViewType = UIViewType.ShareLayer
PersonalArtifactShareView.ShareType = {
  NormalShare = 1,
  SkadaShare = 2,
  RaidResultShare = 3
}

function PersonalArtifactShareView:Init()
  self:initView()
  self:initData()
end

function PersonalArtifactShareView:initView()
  ...
  if title then
    ...
    lbl.text = GameConfig.Share.Sharetitle[ESHAREMSGTYPE.ESHARE_COMPOSE_ARTIFACT]
  end
  ...
  for i = 1, #GameConfig.Share.Sns_platform do
    ...
    self.snsPlatform[GameConfig.Share.Sns_platform[i]].gameObject.transform.localPosition = LuaGeometry.GetTempVector3(-390 + (i - 1) * 75, 0, 0)
  end
  self:GetGameObjects()
...
function PersonalArtifactShareView:OnHideWeekShareTip()
  local rewardTips = self:FindGO("WeekRewardTips")
  if rewardTips then
    rewardTips:SetActive(false)
  end
...
function PersonalArtifactShareView:ShareToGlobalChannel()
  if ApplicationInfo.IsRunOnWindowns() then
    MsgManager.ShowMsgByID(43486)
    return
  end
  local sharedata = {}
  sharedata.type = ESHAREMSGTYPE.ESHARE_COMPOSE_ARTIFACT
  sharedata.share_items = ReusableTable.CreateArray()
  sharedata.share_items[1] = NetConfig.PBC and {} or ChatCmd_pb.ShareItemData()
  sharedata.share_items[1].guid = self.viewdata.viewdata.id
  sharedata.share_items[1].itemid = self.viewdata.viewdata.staticData.id
  sharedata.share_items[1].count = self.viewdata.viewdata.num
...
function PersonalArtifactShareView:setItemProperty(itemData)
  local label = ""
  ...
  if data.isInited then
    local s = data:GetAttriDescForShare()
...
function PersonalArtifactShareView:OnEnter()
  self:SetData(self.viewdata.viewdata)
  local parent = self.Container.transform
  local effectPath = ResourcePathHelper.EffectCommon("PersonalArtifactShareView")
  self.focusEffect = Game.AssetManager_UI:CreateAsset(effectPath, parent)
  self.itemCell:SetData(self.viewdata.viewdata)
end

function PersonalArtifactShareView:InitItemCell(container)
...
function PersonalArtifactShareView:SetData(data)
  self.data = data
...
function PersonalArtifactShareView:GetGameObjects()
end

function PersonalArtifactShareView:RegisterButtonClickEvent()
end

function PersonalArtifactShareView:changeUIState(isStart)
  if isStart then
...
function PersonalArtifactShareView:initData()
...
function PersonalArtifactShareView:OnExit()
end
...
function PersonalArtifactShareView:SharePicture(platform_type, content_title, content_body)
  if ApplicationInfo.IsRunOnWindowns() then
    MsgManager.ShowMsgByID(43486)
    return
  end
  helplog("PersonalArtifactShareView SharePicture", platform_type)
  local weekReward = MyselfProxy.Instance:GetAccVarValueByType(Var_pb.EACCVARTYPE_SHARE_WEEK_REWARD) or 0
