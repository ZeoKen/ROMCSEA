...
function TutorMatchView:FindObj()
  ...
  if status ~= TutorProxy.TutorMatchStatus.Start then
    ...
    local sameServer = self:FindGO("saveServerBG")
    self.serverCheck = self:FindGO("check", sameServer):GetComponent(UIToggle)
    local curIsMergeServer = ChangeZoneProxy.Instance:CheckCurIsMergeServer()
    sameServer:SetActive(curIsMergeServer)
...
function TutorMatchView:OnToggleChange()
  if self.maleCheck.value then
    ...
    self.femaleSymbol.gradientTop = colorWhite
    self.femaleSymbol.gradientBottom = colorWhite
  else
    self.maleSymbol.gradientTop = colorWhite
    self.maleSymbol.gradientBottom = colorWhite
...
function TutorMatchView:Match()
  ...
  if self.tutorType == TutorType.Student and not TutorProxy.Instance:CheckAsTutorLevel() then
  ...
  ServiceMatchCCmdProxy.Instance:CallJoinRoomCCmd(joinRoomType.TutorMatch, nil, nil, nil, nil, nil, nil, nil, nil, msg, nil, nil, nil, nil, self.serverCheck.value)
end
