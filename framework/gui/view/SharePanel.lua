...
function SharePanel:InitShare()
  ...
  local texName = GameConfig.KFCItems[4]
  if texName then
  ...
  self:AddClickEvent(self.QRCodePanelWeiBoBtn.gameObject, function()
    self:ShareAndReward(4, "", "", E_PlatformType.Sina, self.QRCodePanelBG_UITexture.mainTexture)
  end)
  self:AddClickEvent(self.QRCodePanelQQBtn.gameObject, function()
    self:ShareAndReward(4, "", "", E_PlatformType.QQ, self.QRCodePanelBG_UITexture.mainTexture)
  end)
  self:AddClickEvent(self.QRCodePanelWeChatBtn.gameObject, function()
    self:ShareAndReward(4, "", "", E_PlatformType.WechatMoments, self.QRCodePanelBG_UITexture.mainTexture)
  end)
  ...
  if not BranchMgr.IsChina() then
    self.QRCodePanelconfirmBtn:SetActive(false)
    self.QRCodePanelWeiBoBtn:SetActive(false)
    self.QRCodePanelQQBtn:SetActive(false)
    local sp = self:FindGO("icon", self.QRCodePanelWeChatBtn):GetComponent(UISprite)
