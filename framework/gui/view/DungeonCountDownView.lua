...
function DungeonCountDownView:InitShow()
  ...
  if viewdata then
    ...
    self.gomaptype = viewdata.gomaptype
  end
...
function DungeonCountDownView:UpdateTime(currentTime, leftTime, totalTime)
  ...
  LeanTween.sliderNGUI(self.timeSlider, value, 1, leftTime):setOnComplete(function()
    ...
    elseif self.type == SceneUser2_pb.ECOUNTDOWNTYPE_TOWER then
      ServiceInfiniteTowerProxy.Instance:CallEnterTower(0, Game.Myself.data.id, nil, self.time, self.sign, self.gomaptype)
