  ...
  for j = 1, row do
    for i = 1, col do
      ...
      if childCount >= index then
        ...
        if childTrans then
          LuaVector3.Better_Set(tempV3, x, y, 0)
...
function FoodMakeView:InitView()
  ...
  self.wrapObj = self:FindGO("FoodBagWrap")
  local wrapConfig = {
  ...
  self.tipStick = self:FindComponent("MakeTipStick", UIWidget)
end

function FoodMakeView:HandleClickItem(cellCtl)
  if self.tipActive then
    return
  end
  local canAdd = false
...
function FoodMakeView:AddFoodMaterials(count)
  local item = BagData:GetItemByGuid(self.nowItemId)
  if item then
    self:UpdateChoose_material_guids_Table(self.nowItemId, count)
    self.nowItemId = nil
  end
end
...
function FoodMakeView:HandleItemLongPress(param)
  ...
  if state then
    ...
    self.tipActive = true
    self.nowItemId = data.id
    if cellCtl.leftNum > 0 then
      local itemTip = self:ShowItemTip(showData, self.tipStick, nil, {-150, 320})
      local tipCell = itemTip:GetCell(1)
      if #self.itemIdCountList < 5 or TableUtility.ArrayFindByPredicate(self.choose_material_guids, function(obj, id)
        return obj.guid == id
      end, data.id) then
        tipCell:ActiveCountChooseBord(true, cellCtl.leftNum)
        tipCell:AddTipFunc(ZhString.FoodMakeView_PutFoodMaterials, self.AddFoodMaterials, self)
      end
    else
      self:ShowItemTip(showData, self.tipStick, nil, {-150, 280})
    end
  end
end
...
function FoodMakeView:updatePressItemCount()
  ...
  if holdTime < 200 then
    return
  elseif 3000 < holdTime and holdTime <= 6000 then
    changeCount = 10
  elseif 6000 < holdTime then
    changeCount = 50
  end
...
function FoodMakeView:UpdateChoose_material_guids(matchRecipe)
  ...
  self.matchRecipe = matchRecipe == true
end
...
function FoodMakeView:UpdateChooseBordPosition()
  if not self.init_chooseBord then
    ...
    if epTrans ~= nil then
      ...
      LuaVector3.Better_Set(tempV3, sp_x, sp_y, sp_z)
      self.chooseBord.transform.position = uicam:ViewportToWorldPoint(tempV3)
    end
  end
  if not self.restrice_chooseBord and self.chooseBord.activeInHierarchy then
    self.restrice_chooseBord = true
...
function FoodMakeView:_PlayChooseBgAnim()
  ...
  TimeTickManager.Me():CreateTickFromTo(0, 0, 0.1, 200, function(owner, deltaTime, curValue)
    self:_ScaleTo(curValue)
  end, self, 100):SetCompleteFunc(function(owner, id)
    TimeTickManager.Me():CreateTickFromTo(0, 0.1, 0, 200, function(owner, deltaTime, curValue)
      self:_ScaleTo(curValue)
    end, self, 100):SetCompleteFunc(function(owner, id)
...
function FoodMakeView:_ScaleTo(f)
  LuaVector3.Better_Set(tempV3, 1 + f, 1 - f, 1)
...
function FoodMakeView:ActiveCookButton(b)
  ...
  if not b then
    LuaColor.Better_Set(tempColor, 0.00392156862745098, 0.00784313725490196, 0.011764705882352941, 1)
    self.cookButton_Sprite.color = tempColor
    LuaColor.Better_Set(tempColor, 0.615686274509804, 0.615686274509804, 0.615686274509804, 1)
    self.cookButton_Label.effectColor = tempColor
    self.cookButton_Collider.enabled = false
  else
    LuaColor.Better_Set(tempColor, 1, 1, 1, 1)
    self.cookButton_Sprite.color = tempColor
    LuaColor.Better_Set(tempColor, 0.08627450980392157, 0.4235294117647059, 0.00392156862745098, 1)
    self.cookButton_Label.effectColor = tempColor
...
function FoodMakeView:DoServerCook()
  if self.recipeMap and #self.recipeMap > 0 then
    ...
    FunctionFood.Me():DoMakeFood(self.selectPot, self.choose_material_guids, self.skipAnim, self.recipeMap, not self.matchRecipe)
...
function FoodMakeView:HandleCookStateChange(note)
  ...
  if charid == Game.Myself.data.id then
    ...
    elseif state == SceneFood_pb.ECOOKSTATE_COMPLETE then
      ...
      self.delayCloselt = TimeTickManager.Me():CreateOnceDelayTick(8000, function(owner, deltaTime)
...
function FoodMakeView:UpdateFoodBag()
  ...
  if datas and #datas == 0 then
    self.wrapObj:SetActive(false)
    self.noneTip:SetActive(true)
    return
  else
    self.wrapObj:SetActive(true)
