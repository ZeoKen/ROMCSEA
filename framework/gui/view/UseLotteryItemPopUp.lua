...
function UseLotteryItemPopUp:UpdateInfo()
  ...
  local getName = ""
  local lotterydata = LotteryProxy.Instance:GetHeadLotteryData(timeInfo.year, timeInfo.month)
  if lotterydata then
    local items = lotterydata.items
    for i = 1, #items do
      if items[i].rarity == self.rarity then
        getName = items[i]:GetItemData():GetName()
      end
    end
  end
