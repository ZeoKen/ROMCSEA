...
function BeingInfoView:InitView()
  ...
  restBtnLabel.text = ZhString.BeingInfoView_Callback
...
function BeingInfoView:HandleClickHead(note)
  ...
  if 0 < #bodys then
    ...
    local fashionChooseTip = TipManager.Instance:ShowPetFashionChooseTip(bodyDatas, self.headGO_Bg, NGUIUtil.AnchorSide.TopLeft, {-226, -150}, self.BodyChooseCloseCall, self)
...
function BeingInfoView:ChooseBeingBody(data)
  if data and data[2] ~= self.beingInfoData.body then
...
function BeingInfoView:DoResetSkill()
  self:sendNotification(UIEvent.JumpPanel, {
    view = PanelConfig.CharactorBeingSkill
  })
end
...
function BeingInfoView:UpdateBriefInfo()
  ...
  local bodys = self.beingInfoData:GetBeingBodys()
  self.changeSymbol:SetActive(0 < #bodys)
end
