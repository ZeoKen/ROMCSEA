...
function BranchQuestPage:TryInit()
  if not self.loaded then
    self:ReLoadPerferb("view/BranchQuestPage", false)
    self.loaded = true
    self:initView()
...
function BranchQuestPage:SetMountModel(data)
  UIModelUtil.Instance:SetMountModelTexture(self.itemmodeltexture, data.staticData.id, 0.8, function(obj, args, assetRolePart)
...
function BranchQuestPage:SetNormalModel(data)
  ...
  UIModelUtil.Instance:SetRolePartModelTexture(self.itemmodeltexture, partIndex, sData.id, nil, function(obj, args, assetRolePart)
    if obj == nil then
      return
    end
    self.model = assetRolePart
    ...
    if isfashion then
      LuaVector3.Better_Set(tempV3, 0, 0.5, 0)
...
function BranchQuestPage:SetHairModel(data)
  ...
  UIModelUtil.Instance:SetRolePartModelTexture(self.itemmodeltexture, partIndex, hairid, nil, function(obj, args, assetRolePart)
    if obj == nil then
      return
    end
    self.model = assetRolePart
