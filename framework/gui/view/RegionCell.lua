...
function RegionCell:setIsSelected(isSelected)
  if self.isSelected ~= isSelected then
    ...
    if isSelected then
      ...
      self.bg.gameObject.transform.localScale = LuaGeometry.GetTempVector3(1.04, 1.04, 1.04)
    else
      self.bg.CurrentState = 1
      self.name.effectStyle = UILabel.Effect.None
      self.bg.gameObject.transform.localScale = LuaGeometry.GetTempVector3(1, 1, 1)
    end
  end
end
