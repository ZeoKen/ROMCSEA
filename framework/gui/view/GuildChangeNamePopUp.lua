...
function GuildChangeNamePopUp:CheckInputValid()
  self:AddSelectEvent(self.nameInput, function(go, state)
    if state and FunctionUnLockFunc.Me():ForbidInput(ProtoCommon_pb.EFUNCPARAM_RENAME_GUILD) then
...
function GuildChangeNamePopUp:TryChangeName()
  ...
  if length < GameConfig.System.guildname_min or length > GameConfig.System.guildname_max then
    MsgManager.ShowMsgByIDTable(883)
    self:ShowChangeError()
    return
  end
  if FunctionMaskWord.Me():CheckMaskWord(name, self.maskType) or not StringUtil.CheckTextValidForDisplay(name) then
