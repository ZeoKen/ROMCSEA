...
local evo_minimap = {
  [0] = "astrolabe_minimap_evo4",
  [2] = "astrolabe_minimap_evo2",
  [3] = "astrolabe_minimap_evo3",
  [4] = "astrolabe_minimap_evo4"
}
...
function AstrolabeView:InitMiniMap()
  ...
  local nowOccEvo = (Game.Myself.data:GetCurOcc() and Game.Myself.data:GetCurOcc().profession or 1) % 10
  if ProfessionProxy.IsHero(MyselfProxy.Instance:GetMyProfession()) then
    nowOccEvo = 4
  end
  if not evo_minimap[nowOccEvo] then
    nowOccEvo = 0
  end
  self.mapTexture = self:FindComponent("Map", UITexture, self.miniMap)
  PictureManager.Instance:SetUI(evo_minimap[nowOccEvo], self.mapTexture)
  self.mapSize = {
...
function AstrolabeView:AddMiniMapSearchPointEffect(point)
  ...
  if effect == nil then
    ...
    LuaVector3.Better_Set(tempV3, self:Pos_Map2MiniMap(point:GetWorldPos_XYZ()))
