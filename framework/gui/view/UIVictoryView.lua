...
function UIVictoryView:Init()
  ...
  self:InitCalls()
  self:FindObjs()
  if self.viewdata.viewdata.simply then
    self.normalRoot:SetActive(false)
    self.simplyRoot:SetActive(true)
    self:ShowSimplyResult()
    return
  else
    self.normalRoot:SetActive(true)
    self.simplyRoot:SetActive(false)
  end
  self:ShowModel()
  self:AddEvtListener()
  self:OpenEffect()
...
function UIVictoryView:FindObjs()
  ...
  self.FullScreenBg = self:FindGO("FullScreenBg")
  self.effectPos = self:FindGO("effectRoot")
  self.normalRoot = self:FindGO("Normal")
  self.simplyRoot = self:FindGO("Simply")
  self.simplyEffectPos = self:FindGO("effectR")
  self:AddClickEvent(self.simplyRoot, function()
    self:CloseSelf()
  end)
end
...
function UIVictoryView:ShowSimplyResult()
  if nil == self.Effect then
    self:PlayUIEffect(effectResID, self.simplyEffectPos, false, nil, self)
  end
end
...
function UIVictoryView:RefreshTexture(teamData)
  ...
  for i = 1, #teamData do
    ...
    tempV3 = LuaGeometry.GetTempVector3(config.rotation[1], config.rotation[2], config.rotation[3], tempV3)
    ...
    args[9] = nil
    args[10] = false
    UIMultiModelUtil.Instance:SetModels(i, args)
    PictureManager.ReFitFullScreen(self.uiTexture, 1)
    if 1 == result then
...
function UIVictoryView:ShowModel()
  ...
  if myTeam then
    ...
    if memberData and 0 < #memberData then
...
function UIVictoryView:_LoseHandle(index, actionState)
  ...
  if config.failedAction then
    ...
    helplog("GameConfig.MultiModelTrans 未配字段 failedAction")
  end
end
...
function UIVictoryView:_Victory_1Handle(index, actionState)
  ...
  if config.action then
    ...
    helplog("GameConfig.MultiModelTrans 未配字段 action")
  end
end
...
function UIVictoryView:_Victory_3Handle(index, actionState)
  ...
  if config.actions then
    ...
    helplog("GameConfig.MultiModelTrans 未配字段 actions")
  end
end
