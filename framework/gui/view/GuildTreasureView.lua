...
local UI_FLITER = GameConfig.GuildTreasure.SceneFilter or {
  11,
  12,
  20,
  24
...
local BtnPhase = {
  lottery = {BtnSpriteName = "com_btn_1", costId = 151},
  guildAsset = {BtnSpriteName = "com_btn_2", costId = 460}
}
...
function GuildTreasureView:OnClickOpenBtn()
  ...
  elseif self.viewType == GuildTreasureProxy.ViewType.GuildTreasure then
    ...
    if self.data:isLotteryType() then
      if self.limitCsv.NoNeedCredit and self.limitCsv.NoNeedCredit == 1 then
      elseif MyselfProxy.Instance:GetQuota() < self.quotaLimit then
      ...
      if curLimit >= totalLimit then
        if self.limitCsv.MoneyID and self.limitCsv.MoneyID == 100 then
      ...
      local money
      if self.limitCsv.MoneyID and self.limitCsv.MoneyID == 100 then
        money = MyselfProxy.Instance:GetROB()
      else
        money = MyselfProxy.Instance:GetLottery()
      end
    ...
    if self.data.staticData then
      ...
      if note and note.body then
        ...
        if serviceData.action == GuildTreasureProxy.ActionType.OPEN_GVG or serviceData.action == GuildTreasureProxy.ActionType.OPEN_GUILD then
          self:Hide(self.pos)
          self:clearLt()
          self.lt = TimeTickManager.Me():CreateOnceDelayTick(2500, function(owner, deltaTime)
            self:Refresh()
          end, self)
        else
          self:Refresh()
...
function GuildTreasureView:Refresh()
  ...
  if self.viewType == GuildTreasureProxy.ViewType.HoldTreasure then
    ...
    LuaVector3.Better_Set(tempVector3, 0, 4, 0)
  ...
  elseif self.viewType == GuildTreasureProxy.ViewType.GuildTreasure then
    ...
    LuaVector3.Better_Set(tempVector3, -50, 4, 0)
    self.openLab.transform.localPosition = tempVector3
...
local defaultLabelConfig = {labwidth = tempTipLabWidth}

function GuildTreasureView:showPreview(desc)
  TableUtility.TableClear(self.contextDatas)
  if not StringUtil.IsEmpty(desc) then
    local FuncDescTip = {
      label = {},
      hideline = true,
...
function GuildTreasureView:refreshGuildTreasure(data)
  ...
  local leftCount
  if bLottery then
    if self.limitCsv.MoneyID then
      costIconName = Table_Item[self.limitCsv.MoneyID].Icon
    else
  ...
  if self.limitCsv.NoNeedCredit and self.limitCsv.NoNeedCredit == 1 then
    self.scoreLimit.gameObject:SetActive(false)
  end
  IconManager:SetItemIcon(costIconName, self.costIcon)
  self.CostNum.text = self.curCount
  local rate = GameConfig.GuildTreasure.QuotaRate or 10
  self.quotaLimit = self.limitCsv.MoneyID == 100 and self.curCount // rate or self.curCount * 10000
