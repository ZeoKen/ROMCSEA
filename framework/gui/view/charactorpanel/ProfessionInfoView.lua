...
function ProfessionInfoView:initView()
  ...
  self.gameObject.transform.position = LuaGeometry.GetTempVector3(0, 0, 0)
end

function ProfessionInfoView:clickHandler(target)
  local skillId = target.data
  local skillItem = SkillItemData.new(skillId, nil, nil, self.currentPfn.id)
  local tipData = {}
...
function ProfessionInfoView:multiProfessionInfo(obj)
  ...
  local branch = ProfessionProxy.GetTypeBranchFromProf(self.currentPfn.id) or 0
