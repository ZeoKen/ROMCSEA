...
function ActivityDetailPanel.CanShow()
  local nowRoleLevel, configLevel = Game.Myself.data.userdata:Get(UDEnum.ROLELEVEL), GameConfig.Activity.LoginMinLevel
  return not configLevel or nowRoleLevel >= configLevel
end
...
function ActivityDetailPanel:initData()
  self.delta = 0
  self.groupId = self.viewdata.viewdata.groupId
  self.txWidth = 959
  self.txHeight = 502
  self.curShowIndex = 1
  self.subActivits = {}
...
function ActivityDetailPanel:ShowActivityDetail(actData)
  ...
  local entrance = activityData and activityData.entrance
  if entrance and 0 < #entrance then
    for i = 1, #entrance do
      helplog("entrance is ", entrance[i])
      if self.curShowIndex == entrance[i] then
        self.showDetailBtn.gameObject:SetActive(true)
        break
      end
      self.showDetailBtn.gameObject:SetActive(false)
    end
  else
    self.showDetailBtn.gameObject:SetActive(false)
  end
...
function ActivityDetailPanel:initView()
  ...
  self:AddClickEvent(dragCollider, function(go)
    redlog("点击了界面")
    if self.showDetailBtn.gameObject.activeInHierarchy then
      helplog("showDetailBtn 亮着")
      self:showDetailPanel()
      ServiceUserEventProxy.Instance:CallThemeDetailsUserEvent(1)
    end
  end)
  ...
  self:AddButtonEvent("ViewDetailButton", function(go)
    ...
    ApplicationInfo.OpenUrl(result)
    self:TrackEvent(2)
  end)
  ...
  self.showDetailBtn = self:FindGO("BtnGetDetail")
end
...
function ActivityDetailPanel:showDetailPanel()
  GameFacade.Instance:sendNotification(UIEvent.ShowUI, {
    viewname = "ActivityOverviewPanel"
  })
end
...
function ActivityDetailPanel:CloseSelf()
  self:TrackEvent(1)
  ActivityDetailPanel.super.CloseSelf(self)
end

function ActivityDetailPanel:TrackEvent(btnType)
  local subActData = self.subActivits and self.subActivits[self.curShowIndex]
  if subActData then
    ServiceSceneUser3Proxy.Instance:CallActionStatUserCmd(EACTIONSTATTYPE.EACTIONSTATTYPE_CLICK_BANNER, subActData.id, btnType)
  end
end
