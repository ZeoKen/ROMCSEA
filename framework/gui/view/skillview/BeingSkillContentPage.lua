...
BeingSkillContentPage = class("BeingSkillContentPage", SkillBaseContentPage)
local tmpPos = LuaVector3.Zero()

function BeingSkillContentPage:Init()
  self.gameObject = self:FindGO("BeingSkillContentPage", self:FindGO("SkillPages"))
  self.tipdata = {}
  self.cellHeight = 0
  self.cellWidth = 0
  self:FindObjs()
...
function BeingSkillContentPage:OnEnter()
  BeingSkillContentPage.super.OnEnter(self)
end

function BeingSkillContentPage:OnExit()
  BeingSkillContentPage.super.OnExit(self)
end
...
function BeingSkillContentPage:FindObjs()
  self.bottomleftMines = self:FindGO("BottomLeft")
  self.bottomrightBeings = self:FindGO("RightBtns")
  ...
  self.contentBeings = self:FindGO("Contents")
  ...
  local beingCellHeight = self.beingsPanel.height / 4
...
function BeingSkillContentPage:OnSwitch(val)
  self.gameObject:SetActive(val == true)
  self.bottomrightBeings:SetActive(false)
  self.container:ShowDefaultShortCutSkills(not val)
end
...
function BeingSkillContentPage:IsEditMode()
  local data = self:GetSelectSkillBeingData()
  return data and data:GetIsEditMode()
end
...
function BeingSkillContentPage:SwapSkill(obj)
  ...
  if currentBeingData:GetIsEditMode() and source.class == SkillCell then
...
function BeingSkillContentPage:RefreshEditMode()
  local currentBeingData = self:GetSelectSkillBeingData()
  if currentBeingData and currentBeingData:GetIsEditMode() then
...
function BeingSkillContentPage:OnDestroy()
  self.skillList:Destroy()
  self.beingList:Destroy()
  self.shortCutList:Destroy()
  BeingSkillContentPage.super.OnDestroy(self)
end
