...
local _sortData = function(a, b)
  ...
  local aid = a.id or a.FakeID
  local bid = b.id or b.FakeID
    ...
    for part, value in pairs(avatarData) do
      for _, v in pairs(value) do
        ...
        if v.forbidFlag ~= true then
          ...
          local id = v.id or v.FakeID / 1000
          local isUnlock = PetComposeProxy.Instance:IsItemUnlock(self.bodyid, epos, id)
          local data = {id = id, unlocked = isUnlock}
...
function PetDressingBord:UpdateList()
  ...
  self.curItemid = previewData and previewData[RoleDefines_EquipBodyIndex[TabCssonfig[self.nowTab]]] or self.cacheData[PetDressingBord.TabConfig[self.nowTab]]
