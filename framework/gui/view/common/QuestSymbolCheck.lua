...
function QuestSymbolCheck.GetQuestSymbolByQuest(quest)
  if quest.questDataStepType == QuestDataStepType.QuestDataStepType_VISIT then
    if quest.staticData.WhetherTrace ~= 3 then
      ...
      if Table_WorldQuest[quest.id] then
        if quest.staticData.headIcon and quest.staticData.headIcon ~= 0 then
          return quest.staticData.headIcon
        end
        return quest.staticData.IconFromServer or 20
      end
      ...
      elseif quest.staticData.headIcon and quest.staticData.headIcon ~= 0 then
        return quest.staticData.headIcon
      elseif quest.type and QuestDataTypeSymbolMap[quest.type] then
        return QuestDataTypeSymbolMap[quest.type]
      elseif quest.headicon and quest.headicon ~= 0 then
        return quest.headicon
      end
      if symbolType ~= 100 then
        return symbolType
      end
    end
  elseif (quest.questDataStepType == QuestDataStepType.QuestDataStepType_KILL or quest.questDataStepType == QuestDataStepType.QuestDataStepType_GATHER or quest.questDataStepType == QuestDataStepType.QuestDataStepType_COLLECT) and quest.staticData.Params.ShowIcon then
    return quest.staticData.ColorFromServer
  end
end
  ...
  if npcSData then
    ...
    if npcFuncs then
      for i = 1, #npcFuncs do
        ...
        if type(single) ~= "table" or single.type == nil then
          error(string.format("NpcFunction no find param:type.. (NPC ID:%s)", npcSData.id))
        end
...
function QuestSymbolCheck.HasQuestSymbolByMap(mapid)
  ...
  if config then
    ...
    if units then
      ...
      redlog("Cannot find config \"units\" in Table_MapInfo, id: " .. tostring(mapid))
    end
  end
  return false
end
