...
function StageView:initView()
  ...
  if not self.timeTick then
    self.timeTick = TimeTickManager.Me():CreateTick(0, 1000, self.UpdateCountDown, self, StageView.TickType.Showtime)
  end
  self.isShowStageView = true
  self:ShowStageView()
  TabNameTip.SwitchShowTabIconOrLabel(self.equibTabIconSp.gameObject, equibTabLabel)
  TabNameTip.SwitchShowTabIconOrLabel(self.fashionTabIconSp.gameObject, fashionTabLabel)
end
...
function StageView:initData()
  ...
  if not self.cameraController then
    self:DelayCallFunc(100, function(owner, deltaTime)
      self:CloseSelf()
    end)
    return
  end
...
function StageView:ApplyComposition()
  ...
  fvp.x = composition[1]
  fvp.y = composition[2]
  self.cameraController:FocusTo(fo, fvp, StageView.ChangeCompositionDuration, nil, false)
end
...
function StageView:initCameraData()
  ...
  local defaultCompositionIndex = GameConfig.StageConfig.defaultCompositionIndex or 1
  self:SetCompositionIndex(defaultCompositionIndex)
  ...
  if pgInfo then
    ...
    if nil ~= composition then
      ...
      LuaVector3.Better_Set(tempVector3, self.originFocusViewPort.x, self.originFocusViewPort.y, self.cameraData.Radius)
    end
  ...
  if defaultMode == StageView.DefaultMode.PHOTOGRAPHER then
    ...
    self:DelayCallFunc(1000, function(owner, deltaTime)
...
function StageView:calFOVByPos()
  ...
  if position then
    ...
    LuaVector3.Better_SetPos(self.forceRotation, tempVector3)
  end
end
...
function StageView:DelayCallFunc(delayTime, func)
...
function StageView:OnExit()
  ...
  if self.mount then
    FunctionItemFunc.EquipEvt(self.mount)
    self.mount = nil
  end
end
...
function StageView:takePic()
  ...
  if self.textInvisible then
    self.screenShotHelper:GetScreenShot(screenShotCallback, gmCm)
  else
    self.screenShotHelper:GetScreenShot(screenShotCallback, gmCm, self.uiCm)
  end
end
...
function StageView:HandleLongPress(param)
  ...
  TabNameTip.OnLongPress(isPressing, StageView.TabName[index], false, self.coreTabMap[index].go:GetComponent(UISprite))
end

function StageView:SetCurrentTabIconColor(currentTabGo)
  TabNameTip.ResetColorOfTabIcon(self.equibTabIconSp)
  TabNameTip.ResetColorOfTabIcon(self.fashionTabIconSp)
