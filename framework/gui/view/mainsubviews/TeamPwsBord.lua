...
function TeamPwsBord:OnBuffRedClicked()
  if self.red_BuffID == nil then
    return
  end
  local desc = Table_Buffer[self.red_BuffID].BuffDesc
  local normalTip = TipManager.Instance:ShowNormalTip(desc, self.stick, NGUIUtil.AnchorSide.Left, {-270, 0})
  normalTip:SetAnchor(NGUIUtil.AnchorSide.TopRight)
end

function TeamPwsBord:OnBuffBlueClicked()
