...
function MainViewAddTrace:AddOrUpdateTraceItem(itemid, monsterid, withTip)
  ...
  if origin then
    ...
    if itemData == nil or monsterData == nil then
      return
    end
    ...
    local hasNum = BagProxy.Instance:GetItemNumByStaticID(itemid)
    local traceData = {
...
function MainViewAddTrace:AddOrUpdateKapuraFightTrace()
end

function MainViewAddTrace:RemoveKapuraFightTrace()
end
