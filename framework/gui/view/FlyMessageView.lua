...
function FlyMessageView:OnButtonFrameSelectionClick()
  ...
  if self.isOpenFrameSel then
    self.objSelColorBoard:SetActive(false)
    if self.listFrameDatas then
      TableUtility.TableClear(self.listFrameDatas)
    else
      self.listFrameDatas = {}
    end
    self.listFrames:ResetDatas(TableUtil.HashToArray(Table_BarrageFrame, self.listFrameDatas))
...
function FlyMessageView:ExpandView()
  ...
  self.transFlyingMessageSwitch.localEulerAngles = LuaGeometry.GetTempVector3(0, 180, 0)
end
...
function FlyMessageView:FoldView()
  ...
  self.transFlyingMessageSwitch.localEulerAngles = LuaGeometry.GetTempVector3()
end
