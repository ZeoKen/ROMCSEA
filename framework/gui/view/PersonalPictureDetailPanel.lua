...
function PersonalPictureDetailPanel:initData()
  ...
  if self.PhotoData and (self.PhotoData.isupload or self.PhotoData.type == PhotoDataProxy.PhotoType.SceneryPhotoType) then
    self:Show(self.letterBtn)
  else
    self:Hide(self.letterBtn)
  end
end
...
function PersonalPictureDetailPanel:initView()
  ...
  self.letterBtn = self:FindGO("letterBtn")
  ...
  self:AddClickEvent(self.letterBtn, function()
    local url = self.PhotoData:TryGetCdnDlUrl(true)
    local local_path = self.PhotoData:TryGetLocalResPath(false)
    local source = EPOSTCARDPHOTOSOURCE.EPHOTO_SOURCE_PLAYER_ALBUM
  ...
  self:AddClickEvent(self.delBtn, function(go)
    ...
    if self.from == PicutureWallSyncPanel.PictureSyncFrom.GuildWall or self.from == PicutureWallSyncPanel.PictureSyncFrom.WeddingWall then
      ...
      if uploadData then
        ...
        if 0 < #uploadData and self.from == PicutureWallSyncPanel.PictureSyncFrom.GuildWall then
