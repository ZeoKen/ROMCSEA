...
function ServantEmojiView:InitView()
  ...
  self.curChooseEmojiCell = nil
end

function ServantEmojiView:ClickCell(cellctl)
  self.curChooseEmojiCell = cellctl
  if cellctl.type == UIEmojiType.Action then
    ...
    if nil == handFollowerId or handFollowerId ~= 0 then
...
function ServantEmojiView:MapEvts()
  self:AddListenEvt(ServiceEvent.NUserHandStatusUserCmd, self.DoServantAction)
end

function ServantEmojiView:DoServantAction()
  local sdata = Table_ActionAnime[self.curChooseEmojiCell.id]
  self:PlayAction(sdata.Name)
end
