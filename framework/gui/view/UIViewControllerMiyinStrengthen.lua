...
function UIViewControllerMiyinStrengthen:OnEnter()
  StrengthProxy.Instance:ResetStrengthType(SceneItem_pb.ESTRENGTHTYPE_GUILD)
...
function UIViewControllerMiyinStrengthen:LoadView_EquipChooseBoard()
  if self.equipChooseBord == nil then
    local func = function()
      return StrengthProxy.Instance:GetFakeGuildStrengthItems()
    end
    ...
    self.equipChooseBord:SetItemTipInValid()
...
function UIViewControllerMiyinStrengthen:OnEquipBeSelected(itemData)
  ...
  self.msViewController:Refresh(itemData.site)
end
