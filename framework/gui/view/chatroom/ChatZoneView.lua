...
function ChatZoneView:FindObjs()
  ...
  self.chatZoneJoinerBoard = self:FindGO("JoinerBoard")
  self.chatZoneJoinerBoard:SetActive(false)
end
...
function ChatZoneView:ShowTip(data)
  ...
  table.insert(self.funkey, "EnterHomeRoom")
...
function ChatZoneView:SendMessage(content, voice, voicetime, photo, expression)
...
function ChatZoneView:OnDestroy()
  self.itemWrapName:Destroy()
  ChatZoneView.super.OnDestroy(self)
end

function ChatZoneView:UpdateContentInfo(isReset)
  local data = ChatZoomProxy.Instance:Message()
  if isReset then
    self.itemContent:ResetPosition(data)
  else
    self.itemContent:UpdateInfo(data, self.isLock)
  end
  self.container:ShowEmptyTip(#data == 0, ChatRoomEnum.CHATZONE)
end
