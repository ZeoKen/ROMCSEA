...
function AdventureSceneList:Init()
  ...
  if BranchMgr.IsEU() then
    self.sceneryEnName.gameObject:SetActive(false)
    self.sceneryZhName.transform.localPosition = LuaGeometry.GetTempVector3(-15, 10, 0)
  end
end
...
function AdventureSceneList:SetData(datas, noResetPos)
  ...
  if self.itemDatas and #self.itemDatas > 0 then
    ...
    self:sendNotification(AdventurePanel.NoItemStage, AdventureDataProxy.NoItemTipStageType.ShowAll)
...
function AdventureSceneList:JumpToFirstClickableItem(force)
  if not force and self.startIndex ~= 1 then
    return
  end
  local firstClickableIndex = self:FindFirstClickableItemRowIndex(self.itemDatas)
  if firstClickableIndex or force then
    local newIndex = firstClickableIndex and math.floor((firstClickableIndex - 1) / 4) * 4 + 1 or 1
    if self.startIndex ~= newIndex and self.itemDatas and #self.itemDatas > 0 then
...
function AdventureSceneList:FindFirstClickableItemRowIndex(datas)
  if not datas then
    return
  end
  for i = 1, #datas do
    if datas[i]:canBeClick() then
      return i
    end
  end
end
...
function AdventureSceneList:setItemList(list)
  if 1 < #list then
    ...
    self:sendNotification(AdventurePanel.NoItemStage, AdventureDataProxy.NoItemTipStageType.ShowRight)
  end
...
function AdventureSceneList:OnExit()
  ...
  AdventureSceneList.super.OnDestroy(self)
end
...
function AdventureSceneList.GetTabDatas4CommonUsage(type, tab, propData, keys, isMyProfession)
  local bag = AdventureDataProxy.Instance.bagMap[type]
  if bag then
    local items = bag:GetItems(tab)
