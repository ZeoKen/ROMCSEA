...
local TEXTURE_NAME = "pet_bg_machine"
...
function SlotMachineView:InitView()
  ...
  self.tex = self:FindComponent("LabaBg", UITexture)
  PictureManager.Instance:SetUI(TEXTURE_NAME, self.tex)
  local effectPath = ResourcePathHelper.Effect(ResourcePathHelper.UIEffect("pet_bg_rocker3"))
  local lg = self:LoadPreferb_ByFullPath(effectPath, self:FindGO("PullRod"))
  lg.transform.localPosition = LuaGeometry.GetTempVector3(-223, -7.3, 0)
...
function SlotMachineView:_DoAnim(deltatime)
  ...
  if 250 < totaltime then
    self.scrollView2:Scroll(1)
  end
  if 500 < totaltime then
...
function SlotMachineView:_StopAnim(deltatime)
  ...
  elseif stopIndex == 1 then
    ...
    local _, y, _ = LuaGameObject.GetPosition(cTrans)
    if 0.1 > math.abs(self.scrollView2_centerY - y) then
  ...
  elseif stopIndex == 2 then
    ...
    local _, y, _ = LuaGameObject.GetPosition(cTrans)
    if 0.1 > math.abs(self.scrollView3_centerY - y) then
...
function SlotMachineView:OnEnter()
  ...
  if self.success then
    ...
    if self.stopIndex[1] == self.stopIndex[2] and self.stopIndex[1] == self.stopIndex[3] then
      self.stopIndex[3] = (self.stopIndex[3] + 1) % 3 + 1
    end
  end
  self:DoAnim()
  if self.closelt then
    self.closelt:Destroy()
    self.closelt = nil
  end
  local maxCatchCartoonTime = GameConfig.Pet.maxCatchCartoonTime or 5
  self.closelt = TimeTickManager.Me():CreateOnceDelayTick(maxCatchCartoonTime * 1000, function(owner, deltaTime)
    self.closelt = nil
    self:StopAnim()
  end, self)
...
function SlotMachineView:OnExit()
  ...
  self.stopAnim = false
  PictureManager.Instance:UnLoadUI(TEXTURE_NAME, self.tex)
