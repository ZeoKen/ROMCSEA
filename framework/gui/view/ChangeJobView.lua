...
function ChangeJobView:Step()
  ...
  if targetPos then
    ...
    LuaVector3.Better_Set(tempV3, targetPos[1], targetPos[2], targetPos[3])
    tempArgs.targetPos = tempV3
    tempArgs.distance = 0.5
    
    function tempArgs.callback(callerCMD, cmdEvent)
      if MissionCommand.CallbackEvent.Shutdown == cmdEvent and GameConfig.ChangeJob.movePos[self.step + 1] then
        self:EnableBtn(true)
      end
    end
