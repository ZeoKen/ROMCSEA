...
function FMEmission:Init()
  if Game.GameObjectUtil:ObjectIsNULL(self.transFlyerRoot) then
    self.transFlyerRoot = GameObject.Find("MessageFlyerRoot").transform
  end
  if Game.GameObjectUtil:ObjectIsNULL(self.transPosSimulation) then
    self.transPosSimulation = Game.GameObjectUtil:DeepFind(self.gameObject, "PosSimulation").transform
  end
...
function FMEmission:Launch(str, color, pos)
  if self.flyerCount > 0 then
    ...
    if Game.GameObjectUtil:ObjectIsNULL(mf.gameObject) then
      mf:AttachGO(self.transFlyerRoot.gameObject)
    end
...
function FMEmission:Open()
  if not self:IsPronteraSouthGate() then
    return
  end
