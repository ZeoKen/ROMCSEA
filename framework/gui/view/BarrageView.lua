...
function BarrageView:AddText(data)
  ...
  go.transform.localPosition = LuaGeometry.GetTempVector3(x, y, 0)
...
function BarrageView:CreateSystemMsgText()
  ...
  if not brgConfig then
    self:RemoveSysMsg()
    LogUtility.Error(string.format("没有找到系统弹幕：%s 的配置", tostring(sysBrgID)))
    return
  end
  ...
  self.objSysMsg.transform.localScale = LuaGeometry.GetTempVector3(1, BarrageView.yScale, 1)
end
...
function BarrageView:GetActiveHeight()
  local uiRoot = Game.GameObjectUtil:FindCompInParents(self.gameObject, UIRoot)
  if uiRoot ~= nil then
