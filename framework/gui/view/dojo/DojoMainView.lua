...
function DojoMainView:ClickEnterBtn()
  if TeamProxy.Instance:IHaveTeam() then
    local dojoRaid = Table_Guild_Dojo[self.dojoid] and Table_Guild_Dojo[self.dojoid].Raid
    if dojoRaid and TeamProxy.Instance:ForbiddenByRaidID(dojoRaid) then
...
function DojoMainView:ClickClose()
  self:sendNotification(UIEvent.JumpPanel, {
    view = PanelConfig.DojoGroupView
  })
...
function DojoMainView:HandleInvite(note)
  ...
  if data then
    if data.ret then
      self:sendNotification(UIEvent.JumpPanel, {
        view = PanelConfig.DojoWaitView,
        viewdata = self.dojoid
      })
    else
