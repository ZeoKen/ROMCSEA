...
function EngageDayView:ShowSelf(isShow)
  ...
  if isShow then
    ...
    if container.viewEnum == _ViewEnumCheck then
      container:UpdateDialog(ZhString.Wedding_EngageCheckDialog)
    elseif container.viewEnum == _ViewEnumBook then
      local tempstr = GameConfig.Wedding.Engage_Dialog.EngagementTime or ""
      container:UpdateDialog(string.format(ZhString.Wedding_EngageBookDayDialog, tempstr))
    end
    local data = self.container:GetCurDateData()
    if data ~= nil then
...
function EngageDayView:HandleNtfWeddingInfo(note)
  ...
  if data ~= nil and data.info.id ~= 0 then
