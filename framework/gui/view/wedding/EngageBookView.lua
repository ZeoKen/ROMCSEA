...
function EngageBookView:Book()
  ...
  if self.timeStamp ~= nil and self.configid ~= nil then
    ...
    local handMemberData = TeamProxy.Instance.myTeam:GetMemberByGuid(handFollowId)
    if handMemberData and not handMemberData:IsSameServer() then
      MsgManager.ShowMsgByID(9675)
      return
    end
    ...
    MsgManager.ConfirmMsgByID(9604, function()
      ...
      self:ShowSelf(false)
    end, nil, nil, sb:ToString(), self.month, self.day, self.starthour, self.endhour)
    sb:Destroy()
  end
end
