...
local ModelInfo = {
  [1] = {
    ...
    rotation = Quaternion.Euler(4, -39, 0)
  },
  [2] = {
    position = Vector3(-3.6, 0.8, 2.18),
    rotation = Quaternion.Euler(1.4, -8.6, 0)
  }
}
local Color = LuaColor.New(1, 1, 1, 1)
...
function WeddingManualMainView:FindObjs()
  ...
  self.process = self:FindGO("Process"):GetComponent(UILabel)
  self.process.text = ZhString.Wedding_Process
end
...
function WeddingManualMainView:InitShow()
  ...
  if weddingInfo ~= nil then
    ...
    self.processInfo.text = string.format(ZhString.Wedding_ManualInfo, starttime.year, starttime.month, starttime.day, starttime.hour, endtime.hour)
  end
end

function WeddingManualMainView:Detail()
  self:sendNotification(UIEvent.JumpPanel, {
    view = PanelConfig.WeddingProcessView
  })
end
...
function WeddingManualMainView:Invite()
  self:sendNotification(UIEvent.JumpPanel, {
    view = PanelConfig.WeddingInviteView
  })
end

function WeddingManualMainView:Package()
  self:sendNotification(UIEvent.JumpPanel, {
    view = PanelConfig.WeddingPackageView
  })
end

function WeddingManualMainView:Photograph()
  self:sendNotification(UIEvent.JumpPanel, {
    view = PanelConfig.MarriageManualPicDiy
  })
end
...
function WeddingManualMainView:HandleQueryUserInfo(note)
  ...
  if data ~= nil then
    ...
    if weddingInfo ~= nil and weddingInfo:GetPartnerGuid() == info.charid then
      ...
      UIMultiModelUtil.Instance:SetBGMat("marry_bg_wedding_mohu")
...
function WeddingManualMainView:HanleNtfWeddingInfoCCmd(note)
  local data = note.body
  if data ~= nil and WeddingProxy.Instance:GetWeddingInfo() == nil then
    self:CloseSelf()
  end
end
