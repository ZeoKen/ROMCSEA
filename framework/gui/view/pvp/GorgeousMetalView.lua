...
function GorgeousMetalView:InitView()
  ...
  local help_ID = PanelConfig.GorgeousMetalView.id
  self:RegistShowGeneralHelpByHelpID(help_ID, self.ruleButton)
...
function GorgeousMetalView:JoinRoom(cell)
  local data = cell.data
  if data ~= nil then
    if not TeamProxy.Instance:CheckDiffServerValidByPvpType(G_PVP_TYPE) then
...
function GorgeousMetalView:ReqServerRoomList()
  ...
  self.cdlt = TimeTickManager.Me():CreateOnceDelayTick(CD_TIME * 1000, function(owner, deltaTime)
