...
function DesertWolfView:AddEvts()
  ...
  local helpID = PanelConfig.DesertWolfView.id
  self:RegistShowGeneralHelpByHelpID(helpID, rule)
...
function DesertWolfView:HandleMyRoomMatchCCmd(note)
  local data = note.body
  if data and data.type == D_PVP_TYPE then
    self:UpdateRoomList()
    self:RefreshJoinBtn()
  end
end
...
function DesertWolfView:ClickJoin()
  ...
  if TeamProxy.Instance:CheckIHaveLeaderAuthority() then
    self:sendNotification(UIEvent.JumpPanel, {
      view = PanelConfig.DesertWolfJoinView
    })
  else
...
function DesertWolfView:_SelectCell(cell)
  ...
  if cell then
    ...
    if shouldMoveVertically then
      ...
      LuaVector3.Better_Set(tempV3, 0, -208, 0)
      LuaVector3.Better_Set(tempV3, LuaGameObject.TransformPoint(cell.gameObject.transform, tempV3))
