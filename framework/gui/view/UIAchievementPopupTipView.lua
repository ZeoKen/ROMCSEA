...
function UIAchievementPopupTipView:GetGameObjects()
  ...
  local l_sprCompleteIcon = self:FindComponent("CompleteIcon", UISprite)
  IconManager:SetArtFontIcon("Rewardtask_txt_1", l_sprCompleteIcon)
end
  ...
  if self.spIcon ~= nil then
    ...
    if atlasGroupName ~= nil and string.len(atlasGroupName) > 0 then
      ...
      if spriteName ~= nil and string.len(spriteName) > 0 then
        IconManager:SetIconByType(spriteName, self.spIcon, atlasGroupName)
        self.spIcon:MakePixelPerfect()
        self.spIcon.gameObject.transform.localScale = LuaGeometry.GetTempVector3(0.8, 0.8, 0.8)
      end
    end
  end
  self.labName.text = self.achievementConf.Name
  local combination = self.achievementConf.combination
  local strProcessedDetail = OverSea.LangManager.Instance():GetLangByKey(combination)
  local fit = false
  local start, over
  for i = 1, #processFormat do
    start, over = string.find(strProcessedDetail, processFormat[i].formatStr)
    if start and over then
      local totalNum = tonumber(string.sub(strProcessedDetail, start + processFormat[i].minusStart, over))
      if totalNum then
        fit = true
        strProcessedDetail = string.format(strProcessedDetail, totalNum)
        break
      end
    end
  end
  if not fit then
    strProcessedDetail = string.gsub(strProcessedDetail, "/", "")
    strProcessedDetail = string.gsub(strProcessedDetail, "%%s", "")
  end
  self.labDetail.text = strProcessedDetail
  local rewards = self.achievementConf.RewardItems
  self.spReward1.atlas = nil
  self.spReward2.atlas = nil
  self.spReward3.atlas = nil
  for i = 1, #rewards do
