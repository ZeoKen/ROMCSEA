...
autoImport("EquipRecommendNewView")
...
function ServantImproveViewNew:ShowEquipRecommendMainNew(b)
  if self.EquipRecommendNewView then
    if b then
      self.EquipRecommendNewView:Show()
    else
      self.EquipRecommendNewView:Hide()
    end
  end
end
...
function ServantImproveViewNew:FindObjs()
  ...
  self:RegisterRedTipCheck(SceneTip_pb.EREDSYS_SERVANT_GROWTH_STEP, self.Toggle_1, 9, {6, 6})
  self.Toggle_2 = self:FindGO("Toggle_2", self.ToggleRoot)
  self:RegisterRedTipCheck(SceneTip_pb.EREDSYS_SERVANT_GROWTH_EP, self.Toggle_2, 9, {4, 0})
  ...
  if self:IsToggle3Fobid() then
    ...
    self.ToggleRoot.transform.localPosition = LuaGeometry.GetTempVector3(150, 311, 0)
  else
  ...
  self.ImproveTable_UITable = self.ImproveTable:GetComponent(UITable)
  ...
  self.equipRecommendTitle = self:FindGO("EquipRecommendTitle")
  local batchConfig = GameConfig.System.Gen and GameConfig.System.Gen == 2
  if batchConfig then
    self.ToggleRoot.gameObject:SetActive(false)
    self.equipRecommendTitle:SetActive(true)
  else
    self.equipRecommendTitle:SetActive(false)
  ...
  self:AddClickEvent(self.Toggle_3, function(go)
    self:ShowEquipRecommend()
  end)
  if not batchConfig then
  ...
  EventManager.Me():AddEventListener(ServantImproveEvent.GoClick, self.OnGoClick, self)
end

function ServantImproveViewNew:ShowEquipRecommend()
  ...
  if not self.EquipRecommendNewView then
    self.EquipRecommendNewView = self:AddSubView("EquipRecommendNewView", EquipRecommendNewView)
  else
    self.EquipRecommendNewView:Show()
  end
end
...
function ServantImproveViewNew:AddMoneyRelatedDatasToFatherTable(idFromClickCell)
  ...
  for k, v in pairs(fatherTable) do
    if v.fatherGoal.NameZh == ZhString.ServantImproveCellFatherGoalUncompletedLabel then
      weiwanchengTable = v
    elseif v.fatherGoal.NameZh == ZhString.ServantImproveCellFatherGoalCompletedLabel then
      yiwanchengTable = v
    end
  end
  local AdvancedFartherTable = {}
  local fatherGoal = {}
  fatherGoal.NameZh = ZhString.ServantImproveCellFatherGoalUncompletedLabel
  local newGoal = {}
  ...
  local fatherGoal = {}
  fatherGoal.NameZh = ZhString.ServantImproveCellFatherGoalCompletedLabel
  local newGoal = {}
  newGoal.fatherGoal = fatherGoal
  newGoal.childGoals = {}
...
function ServantImproveViewNew:ClickFunctionBtnNew(cell)
  helplog("点了提升计划 ep卡某个cell")
  self.Node_ScrollView.gameObject:SetActive(false)
  self.Node_DetailView.gameObject:SetActive(true)
  if cell.data.id >= 1 and cell.data.id <= 100 then
    ServantRecommendProxy.Instance:SetCurrentChengZhangId(cell.data.id)
  elseif cell.data.id >= 101 and cell.data.id <= 1000 then
    ServantRecommendProxy.Instance:SetCurrentEPId(cell.data.id)
  end
  ...
  self:UpdateGiftProgressNew(cell.data.id)
...
function ServantImproveViewNew:UpdateRightJiHuaView(idFromClickCell)
  ...
  if groupData.itemList and #groupData.itemList > 0 then
    ...
    local fatherTable = {}
    self:MakeFatherTable(fatherTable, ZhString.ServantImproveCellFatherGoalUncompletedLabel, weiwanchengTable)
    self:MakeFatherTable(fatherTable, ZhString.ServantImproveCellFatherGoalCompletedLabel, yiwanchengTable)
    ...
    if not thisBigCellFatherTable then
      ...
      local fatherTable = {}
      self:MakeFatherTable(fatherTable, ZhString.ServantImproveCellFatherGoalUncompletedLabel, weiwanchengTable)
      self:MakeFatherTable(fatherTable, ZhString.ServantImproveCellFatherGoalCompletedLabel, suoyouTable)
      ServantRecommendProxy.Instance:SetFatherTable(idFromClickCell, fatherTable)
      thisBigCellFatherTable = fatherTable
    end
    self.goalListCtl:ResetDatas()
...
function ServantImproveViewNew:ClickFunctionBtn(cell)
  ...
  elseif cell.btnType == 2 then
    ...
    if cell.finish == "questfinish" then
      helplog("当前执事指引完成条件为questfinish")
      FuncShortCutFunc.Me():CallByQuestFinishID(cell.finishParams, cell.gotoMode, nil, nil, cell.data.dwid)
    elseif cell.gotoMode ~= _EmptyTable then
...
function ServantImproveViewNew:UpdateMainItemListNew()
  ...
  local groupData = ServantRecommendProxy.Instance:GetImproveGroup(currentid)
  local everRewardList = groupData and groupData.everReward
  local jiemianid = ServantRecommendProxy.Instance:GetCurrentShowJieMianId()
...
function ServantImproveViewNew:UpdateGiftProgressNew()
  ...
  local currentid = ServantRecommendProxy.Instance:GetCurrentJinDuId()
  local groupData = ServantRecommendProxy.Instance:GetImproveGroup(currentid)
  if groupData then
...
function ServantImproveViewNew:OnGoClick(cell)
  GameFacade.Instance:sendNotification(UIEvent.CloseUI, UIViewType.NormalLayer)
end

function ServantImproveViewNew:OnExit()
  self.super.OnExit(self)
  PictureManager.Instance:UnLoadServantBG()
end

function ServantImproveViewNew:RecSyncPassUserInfoCmd(data)
  if self.EquipRecommendNewView then
    self.EquipRecommendNewView:RecSyncPassUserInfoCmd(data)
  end
end
