  ...
  for k, v in pairs(shopDatas) do
    ...
    for k, v in pairs(shopItemDatas) do
      ...
      if shopID == i_serial_number then
        ...
        if skillID and 0 < skillID and not self:SkillIsHaveLearned(skillID) then
...
function FuncAdventureSkill:DoOpenUI()
  GameFacade.Instance:sendNotification(UIEvent.JumpPanel, {
    view = PanelConfig.AdventureSkill,
    viewdata = {
      isNpcFuncView = true,
      npcdata = self.npcCreature
    }
  })
end

function FuncAdventureSkill:SkillIsHaveLearned(i_skill_id)
  if i_skill_id and 0 < i_skill_id then
