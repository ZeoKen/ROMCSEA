...
function QueuePusherCtrl:FillWaitingByData()
  if #self.datas > 0 and #self.waiting == 0 then
    ...
    local classRef = data.classRef or self.classRef
    local cell = classRef.new(self.container, data.cellType)
    cell:SetData(data)
    self:AddCell(cell)
  end
end
...
function QueuePusherCtrl:VResetPushTargetPos(firstWaitCell)
  ...
  local datalen = self.datas and #self.datas or 0
  local speedFactor = datalen / 10 + 1
  if 4 < speedFactor then
    speedFactor = 4
  end
  for i = #self.pushing, 1, -1 do
    ...
    time = math.abs(endY - cell:GetY()) / (self.speed * speedFactor)
...
function QueuePusherCtrl:StartTimerCheck()
  if self.timeTick == nil then
    self.timeTick = TimeTickManager.Me():CreateTick(0, 16, function()
      self:CheckPush()
    end, self)
  end
end
