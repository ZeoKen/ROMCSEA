...
function AstrolabeCellPool:InitContainer()
  if self.container == nil then
    ...
    self.container:AddComponent(UIPanel)
...
function AstrolabeCellPool:AddPointCellToPool(pointCell)
  ...
  if count <= PointCell_PoolSzie then
    pointCell:OnRemove()
    pointCell.gameObject:SetActive(false)
...
function AstrolabeCellPool:CreateAstrolabeAsset(path)
  return Game.AssetManager_UI:CreateAstrolabeAsset(path, self.container)
end
