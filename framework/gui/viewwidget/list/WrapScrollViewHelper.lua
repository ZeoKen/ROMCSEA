...
function WrapScrollViewHelper:ctor(control, cellRid, scrollViewObj, tableObj, maxShowNum, onMoveCallBack)
  ...
  for i = 1, maxShowNum do
    ...
    obj.name = name
    obj.transform.localPosition = LuaGeometry.GetTempVector3(0, -i, 0)
    self.cellCtrlMap[name] = cellCtrl
  end
  self.wrapScrollView = WrapScrollView.new(self.scrollView, self.table)
  local activeH = Game.GameObjectUtil:GetUIActiveHeight(self.scrollView.gameObject)
...
function WrapScrollViewHelper:__OnViewDestroy()
  if self.wrapScrollView ~= nil then
    self.wrapScrollView:__OnViewDestroy()
  end
end
