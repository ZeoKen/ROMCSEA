  ...
  for i = 1, #cellCtrls do
    ...
    if distance < -extents then
      ...
      if cell then
        ...
        LuaVector3.Better_Set(tempV3, GetLocalPosition(t))
        tempV3[2] = posY
...
function ReuseableTableWrapHelper:Layout()
  ...
  LuaVector3.Better_Set(tempV3, 0, y, 0)
...
function ReuseableTableWrapHelper:SetStartPositionByIndex(index)
  ...
  LuaVector3.Better_Set(tempV3, GetLocalPosition(self.panel.transform))
  tempV3[2] = realPos
  self.panel.transform.localPosition = tempV3
  tempV3[2] = -realPos
  self.panel.clipOffset = tempV3
  LuaVector3.Better_Set(tempV3, GetLocalPosition(self.tableHolderTrans.transform))
  tempV3[2] = -realPos
...
function ReuseableTableWrapHelper:UpdateInfo(datas, isLayOut)
  self.datas = datas
  for index = 1, #self.ctls do
    local cell = self.ctls[index]
