...
function AdventureCombineNpcCell:ReloadTagData()
  ...
  local isNpc = self.data.type == SceneManual_pb.EMANUALTYPE_NPC and not self.data.isDefaultTag
  ...
  local isMonster = self.data.type == SceneManual_pb.EMANUALTYPE_MONSTER and not self.data.isDefaultTag
  ...
  if not self.data.isDefaultTag then
    self.labRewardNum.text = 0 < rewardCount and rewardCount or ""
    self.sprBtnReward.alpha = (0 < rewardCount or unFinishConditionCount < 1) and 1 or 0.5
    self.objRewardHighLight:SetActive(0 < rewardCount)
    local haveRewardToGet = 0 < unFinishConditionCount or 0 < rewardCount
...
function AdventureCombineNpcCell:OnCellDestroy()
