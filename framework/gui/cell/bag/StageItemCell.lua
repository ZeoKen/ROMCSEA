...
function StageItemCell:SetData(data)
  self:SetMydata(data)
  self:UpdateChoose()
  self:SetMainColorPalette(data, self._MainColorPaletteCellPartLoader)
end

function StageItemCell:_MainColorPaletteCellPartLoader()
...
function StageItemCell:SetMydata(data)
  ...
  if self.icon then
    self.icon.color = LuaGeometry.GetTempColor()
    local setSuc = false
    ...
    elseif self.dType == 823 or self.dType == 824 then
      ...
      self.icon.transform.localScale = LuaGeometry.GetTempVector3(1, 1, 1)
    end
    if setSuc then
