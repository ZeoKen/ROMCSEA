...
function DesertWolfCombineCell:ClickChallenge()
  if TeamProxy.Instance:CheckIHaveLeaderAuthority() then
    if self.data then
      ...
      if not _teamPy:CheckDiffServerValidByPvpType(PvpProxy.Type.DesertWolf) or _teamPy:ForbiddenByRaidID(roomid) then
        MsgManager.ShowMsgByID(42042)
        return
      end
      ServiceMatchCCmdProxy.Instance:CallJoinRoomCCmd(PvpProxy.Type.DesertWolf, roomid)
    end
  else
