...
function WorldMapMenuCell:InitCell()
  ...
  self:AddClickEvent(self.goBtn, function(go)
    ...
    tempArgs.targetMapID = self.chooseData.id
...
function WorldMapMenuCell:SetData(data)
  ...
  if data.LvRange and data.LvRange[2] then
    ...
    self.table.transform.localPosition = LuaGeometry.GetTempVector3(7.4, 77.5, 0)
  else
    self:Hide(self.lvname)
    self.table.transform.localPosition = LuaGeometry.GetTempVector3(7.4, 99.5, 0)
  end
