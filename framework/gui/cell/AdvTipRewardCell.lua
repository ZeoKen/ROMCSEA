...
function AdvTipRewardCell:SetData(data)
  ...
  if data then
    ...
    elseif data.type == "AdventureValue" then
      ...
      local str = "x"
      if data.showName and data.addbracket then
        str = "[" .. ZhString.AdventureRewardPanel_AdventureExp .. "]" .. str
      end
      self.label.text = str .. tostring(data.value)
    elseif data.type == "PropValue" then
      if data.color then
        self.label.color = data.color
      end
      local atlas = RO.AtlasMap.GetAtlas("NewUI8")
      self.icon.atlas = atlas
      self.icon.spriteName = "Equipmentopen_icon_attack"
      self.label.text = tostring(data.value)
      self.icon.width = 40
      self.icon.height = 40
    end
    ...
    local pos = self.preLabel.transform.localPosition
    self.preLabel.transform.localPosition = LuaGeometry.GetTempVector3(pos.x, 0, pos.z)
    if data.iconcolor then
      self.icon.color = data.iconcolor
    else
