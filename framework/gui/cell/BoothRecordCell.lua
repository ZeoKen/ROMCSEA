      ...
      if data.type then
        if data.type == ShopMallLogTypeEnum.NormalSell or data.type == ShopMallLogTypeEnum.PublicitySellSuccess then
          ...
          if exchangeData.Overlap == 0 then
            info = string.format("%s%s", info, string.format(ZhString.ShopMall_ExchangeRecordNormalSellNotOverLap, itemInfo, firstName, zoneStr, tax, getmoney))
          elseif exchangeData.Overlap == 1 then
            info = string.format("%s%s", info, string.format(ZhString.Booth_ExchangeRecordNormalSellOverLap, count, itemInfo, firstName, zoneStr, tax, getmoney))
          end
        elseif data.type == ShopMallLogTypeEnum.AutoOff then
          info = string.format("%s%s", info, string.format(ZhString.ShopMall_ExchangeRecordAutoOff, itemInfo))
        else
