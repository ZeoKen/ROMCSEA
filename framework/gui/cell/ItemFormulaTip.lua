...
FormulaGainPos = {
  Left = LuaVector3(-702, 0, 0),
  Right = LuaVector3(-45, 330, 0)
}
...
function ItemFormulaTip:InitAttriContext()
  ...
  self.formulaData = self.formulaData or {}
  TableUtility.TableClear(self.formulaData)
end
...
function ItemFormulaTip:GetFormula(data)
  ...
  if not self.formulaData or not (#self.formulaData > 0) then
...
function ItemFormulaTip:SetReplaceInfo(text)
  local y = 100
