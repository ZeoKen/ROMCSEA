...
function Astrolabe_PointCell:ctor()
  self.gameObject = AstrolabeCellPool.Instance:CreateAstrolabeAsset(Astrolabe_PointCell.Rid)
...
function Astrolabe_PointCell:SetData(data, priStateMap, bordData)
  ...
  LuaVector3.Better_Set(tempV3, data:GetLocalPos_XYZ())
...
function Astrolabe_PointCell:UpdateEffect()
  ...
  if effect then
    ...
    for attriType, value in pairs(effect) do
      local config = Game.Config_PropName[attriType]
      if config ~= nil then
