...
GroupInviteMembCell = class("GroupInviteMembCell", BaseCell)
autoImport("PlayerFaceCell")

function GroupInviteMembCell:Init()
  self:InitCell()
end

function GroupInviteMembCell:InitCell()
  ...
  self.labTeamInfo = self:FindComponent("labTeamInfo", UILabel)
...
function GroupInviteMembCell:SetData(data)
  self.data = data
  if data then
    ...
    if data.type == TeamInviteMemberType.Friend or data.type == TeamInviteMemberType.NearlyTeamMember then
      if sdata:IsSameServer() then
        self.diffServerObj:SetActive(false)
        local mzone = ChangeZoneProxy.Instance:ZoneNumToString(sdata.zoneid)
        self.zoneid.text = mzone
        self.zoneid.gameObject:SetActive(mzone ~= "" and sdata.zoneid ~= MyselfProxy.Instance:GetZoneId())
      else
        self.diffServerObj:SetActive(true)
        self.zoneid.gameObject:SetActive(false)
        self.diffServerId.text = tostring(sdata.serverid)
      end
    ...
    local isInTeam = TeamProxy.Instance:IsInMyGroup(data.id)
    self:ActiveInviteButton(not isInTeam)
    self.labTeamInfo.text = data.teamname and data.teamname ~= "" and data.teamname or ZhString.GroupInviteMembCell_Single
  else
...
function GroupInviteMembCell:ActiveInviteButton(b, nosetlab)
...
function GroupInviteMembCell:UpdateRestTip()
...
function GroupInviteMembCell:UpdateRestTime()
  local sdata = self.data.data
  local resttime = sdata and sdata.resttime
  resttime = resttime or 0
...
function GroupInviteMembCell:RemoveRestTimeTick()
...
function GroupInviteMembCell:OnRemove()
  self:RemoveRestTimeTick()
end
