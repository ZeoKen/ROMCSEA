...
function GuildHeadCell:SetData(data)
  ...
  if data then
    ...
    elseif data.type == GuildHeadData_Type.Custom and self.customPic then
...
function GuildHeadCell:LoadSetCustomPic()
  ...
  local success_callback = function(bytes, localTimestamp)
    ...
    if self.data and self.data.index == data.index and self.customPic then
      self.customPic.mainTexture = pic
    end
  end
  local pic_type = data.pic_type
  if pic_type == nil or pic_type == "" or BranchMgr.IsTW() then
