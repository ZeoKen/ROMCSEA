PreorderOverLapCell = class("PreorderOverLapCell", ItemTipBaseCell)

function PreorderOverLapCell:Init()
  PreorderOverLapCell.super.Init(self)
  self:FindObjs()
  self:AddEvts()
  self:InitShow()
end

function PreorderOverLapCell:FindObjs()
...
function PreorderOverLapCell:AddEvts()
...
function PreorderOverLapCell:InitShow()
  if self.quota then
...
function PreorderOverLapCell:SetData(data)
  self.infoData = data
  if data then
    if data.itemid then
...
function PreorderOverLapCell:PressCount(go, isPressed, change)
  if isPressed then
...
function PreorderOverLapCell:UpdateBuyCount(change)
...
function PreorderOverLapCell:InputOnChange()
  local count = tonumber(self.buyCountInput.value)
  if count == nil then
    return
  end
...
function PreorderOverLapCell:UpdateMoney(price)
  if self.money then
...
function PreorderOverLapCell:UpdateDiscountMoney(price)
  if self.discountMoney then
    self.discountMoney.text = StringUtil.NumThousandFormat(BoothProxy.Instance:GetDiscountMoney(price))
  end
end

function PreorderOverLapCell:UpdateQuota(price)
  if self.quota then
...
function PreorderOverLapCell:BuyItem()
  if self.isBuy then
    return
  end
...
function PreorderOverLapCell:CallBuyItemRecordTradeCmd(itemInfo)
  ShopMallProxy.Instance:CallBuyItemRecordTradeCmd(itemInfo, function()
    self:SetBuyBtn(true)
  end)
end

function PreorderOverLapCell:SetBuyBtn(isBuy)
  if isBuy then
...
function PreorderOverLapCell:SetCountChangeCallback(callback)
  self.countChangeCallback = callback
  callback(self:CanExpress())
end

function PreorderOverLapCell:SetCanExpress(needCredit)
  self.canExpress = needCredit >= GameConfig.Exchange.SendMoneyLimit
  self.isQuotaEnough = needCredit <= MyselfProxy.Instance:GetQuota()
end

function PreorderOverLapCell:CanExpress()
  return self.canExpress, self.isQuotaEnough
end

function PreorderOverLapCell:Exit()
  TimeTickManager.Me():ClearTick(self, 1001)
  PreorderOverLapCell.super.Exit(self)
end
