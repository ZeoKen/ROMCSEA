...
function InnerSingleModel:DoConstruct(data)
  data[1][Asset_Role.PartIndexEx.SkinQuality] = Asset_RolePart.SkinQuality.Bone4
  data[1][Asset_Role.PartIndexEx.Download] = true
...
function InnerSingleModel:DelayCall(delay, delayCall, delayCallArg)
  ...
  self.delayTimeFlag = UnityUnscaledTime + delay
end

function InnerSingleModel:Update()
  if self.delayTimeFlag and UnityUnscaledTime >= self.delayTimeFlag then
...
function InnerSingleModel:PlayEmoji(emoji, rotation)
  ...
  if self.follow then
    ...
    Game.RoleFollowManager:RegisterFollow(self.follow.transform, self.model.complete, tempVector3, tempOffset, RoleDefines_EP.Top, nil, nil, false)
...
function InnerSingleModel:RotateDelta(delta)
  if delta then
    self.model:RotateDelta(delta)
  end
end
...
function UIMultiModel:Init()
  ...
  if not BackwardCompatibilityUtil.CompatibilityMode_V54 then
    self.camera.gameObject:AddComponent(NonprimaryCamera)
  end
  ...
  self.dynamicBg = self:FindGO("dynamic_Bg")
  self.dynamicMeshRenderer = self.dynamicBg:GetComponent(MeshRenderer)
...
function UIMultiModel:SetDynamicBgSize(value)
  value = value or {
    28,
    14,
    10
  }
  LuaVector3.Better_Set(tempVector3, value[1], value[2], value[3])
  self.dynamicBg.transform.localScale = tempVector3
end
...
function UIMultiModel:ChangeCameraConfig(params)
  ...
  if self.camera then
    ...
    self.camera.orthographic = params.orthographic
  end
end
...
function UIMultiModel:PlayRandomEmoji(index, emojiArray, rotation, durationRange)
  local smodel = self:_GetSingleModelByIndex(index)
  if smodel and emojiArray and 1 < #emojiArray then
    ...
    smodel:PlayEmoji(emoji, rotation)
    if durationRange and 1 < #durationRange then
...
function UIMultiModel:SetMeshRendererBg(path)
  if self.dynamicMeshRenderer then
    local materials = self.dynamicMeshRenderer.materials
    if 0 < #materials then
      self.material = materials[1]
      if self.material then
        PictureManager.Instance:SetPetRenderTexture(path, self.material)
      end
    end
  end
end
