...
function WorldMapAreaCell:UpdateLevelRange()
  local minLevel, maxLevel
  local childMaps = self.data.childMaps
  if childMaps then
    for i = 1, #childMaps do
      local mapId = childMaps[i].id
      local mapInfo = Table_Map[mapId]
      if mapInfo then
        local lvRange = mapInfo.LvRange
        if lvRange[1] then
          if not minLevel or minLevel > lvRange[1] then
            minLevel = lvRange[1]
          end
          if not maxLevel or maxLevel < lvRange[2] then
            maxLevel = lvRange[2]
          end
        end
      end
...
function WorldMapAreaCell:AddMapTeamMember()
  ...
  if self.sealSymbol.gameObject.activeSelf then
    LuaVector3.Better_Set(tempV3, 42, 0, 0)
...
function WorldMapAreaCell:UpdateSealSymbol()
  ...
  LuaVector3.Better_Set(tempV3, 42, 42, 0)
  self.sealSymbol.transform.localPosition = tempV3
...
function WorldMapAreaCell:UpdateMapGuide()
  ...
  if matched == true then
    if self.guildGO == nil then
      ...
      LuaVector3.Better_Set(tempV3, 2, 2, 2)
