...
function TalentSkillCell:GetSimulateSkillItemData()
  if self:IsChanged() then
    ...
    if not self.simulateData then
      self.simulateData = SkillItemData.new(simulateID, 0, 0, self.data.profession, nil, nil, nil, self.data.extraMaxLevel)
      self.simulateData:SetLearned(0 < self.simulateLevel)
    elseif self.simulateData.id ~= simulateID then
      self.simulateData:Reset(simulateID, 0, 0, self.data.profession, nil, nil, nil, self.data.extraMaxLevel)
    end
