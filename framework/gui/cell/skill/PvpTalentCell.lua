...
function PvpTalentCell:FindObjs()
  ...
  self.gridScrollview = self:FindComponent("gridScrollview", UIScrollView)
end

function PvpTalentCell:InitTalentList()
  local gridTalents = self:FindComponent("gridTalents", UIGrid)
  self.listTalents = UIGridListCtrl.new(gridTalents, TalentSkillCell, "TalentSkillCell")
...
function PvpTalentCell:SetData(data)
  ...
  self.gridScrollview:ResetPosition()
end
...
function PvpTalentCell:OnCellDestroy()
