...
function PetComposePreviewCell:SetBaseInfo()
  ...
  for i = 1, 3 do
    ...
    if childId and childId == self.rootId then
      self.previewGrid.cellWidth = 2 < nodeCount and 100 or 150
      self.line.width = (nodeCount - 1) * self.previewGrid.cellWidth
      return
    end
  end
  if self.recursive then
    self.previewGrid.cellWidth = 2 < nodeCount and 370 or 600
  else
    self.previewGrid.cellWidth = 2 < nodeCount and 150 or 200
  end
  self.line.width = (nodeCount - 1) * self.previewGrid.cellWidth
end
