...
function NormalStageListCell:ErrorMsg()
  if self.questNotFinish then
    MsgManager.ShowMsgByIDTable(101, {
      self.data.staticData.Quest
    })
  end
  if self.data.state == SubStageData.LockState then
    ...
    if previousSub ~= nil then
      MsgManager.ShowMsgByIDTable(102, {
        previousSub.mainStage.staticData.name,
        previousSub.staticData.Name
      })
    end
  end
end
...
function NormalStageListCell:SetData(data)
  ...
  if data.staticData.Quest ~= nil and QuestProxy.Instance:isMainQuestCompleteByStepId(data.staticData.Quest) == false then
