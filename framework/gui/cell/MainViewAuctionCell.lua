...
function MainViewAuctionCell:UpdateAuction(totalSec, min, sec)
  if self.data and totalSec ~= nil and min ~= nil then
    self.label.text = string.format(self.data.Name, min, sec)
  end
end

function MainViewAuctionCell:OnCellDestroy()
  RedTipProxy.Instance:UnRegisterUI(SceneTip_pb.EREDSYS_AUCTION_RECORD, self.sprite)
end
