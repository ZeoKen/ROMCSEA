...
function PetWorkSpaceCell:FindObjs()
  ...
  self.emoji.transform.localPosition = LuaGeometry.GetTempVector3()
...
function PetWorkSpaceCell:PlayChooseEffect()
  self.chooseEff = self:PlayUIEffect(EffectMap.UI.PetWorkSelected, self.effectContainer)
end
...
function PetWorkSpaceCell:_refreshWorkingEff()
  ...
  if data:IsUnlock() and isWorking and not data:IsOverCfgTime() and not data:MaxRewardLimited() then
    if nil == self.workingEff then
      self.workingEff = self:PlayUIEffect(EffectMap.UI.PetWork, self.workingEffContainer)
    end
...
local tempColor = LuaColor(0.00392156862745098, 0.00784313725490196, 0.011764705882352941, 0.6274509803921569)
