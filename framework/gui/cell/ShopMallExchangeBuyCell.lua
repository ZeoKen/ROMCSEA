...
local EquipPurchaseLimit = GameConfig.Exchange.EquipPurchaseLimit
...
function ShopMallExchangeBuyCell:FindObjs()
  ...
  self.priceIcon = self:FindGO("PriceIcon")
  if self.priceIcon then
    self.priceIcon = self.priceIcon:GetComponent(UISprite)
  ...
  if self.money then
    ...
    self.money_Icon = self:FindGO("MoneyIcon"):GetComponent(UISprite)
    IconManager:SetItemIcon(Table_Item[100].Icon, self.money_Icon)
  end
  ...
  if self.discountMoney then
    ...
    self.discountMoneyIcon = self:FindGO("DiscountMoneyIcon"):GetComponent(UISprite)
    IconManager:SetItemIcon(Table_Item[100].Icon, self.discountMoneyIcon)
  end
...
function ShopMallExchangeBuyCell:UpdateBuyCount(change)
  ...
  if count < 1 then
    self.countChangeRate = 1
    return
  elseif forceOverlap then
    if count > EquipPurchaseLimit then
      self.countChangeRate = 1
      return
  ...
  self:UpdateBuyPlusSubtract(count, maxCount)
...
function ShopMallExchangeBuyCell:InputOnChange()
  ...
  if self.infoData then
    ...
    elseif forceOverlap then
      if count > EquipPurchaseLimit then
        self.buyCountInput.value = maxCount
      elseif count > self.infoData.count then
...
function ShopMallExchangeBuyCell:ContinueBuying()
  local infoData = self.infoData
  local count = self.buyCountInput and tonumber(self.buyCountInput.value) or 1
  if count > self.processCount then
    TableUtility.TableClear(itemInfo)
    itemInfo.itemid = infoData.itemid
    itemInfo.price = infoData.price
    itemInfo.count = 1
...
function ShopMallExchangeBuyCell:Exit()
  TimeTickManager.Me():ClearTick(self, 1001)
  ShopMallExchangeBuyCell.super.Exit(self)
end
