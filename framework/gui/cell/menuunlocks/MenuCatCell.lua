...
function MenuCatCell:PlayEffect(effectName)
  ...
  self.effect = Asset_Effect.PlayOneShotOn(path, self.container.transform, MenuCatCell._HandleMidEffectShow, self)
  if self.effect then
...
function MenuCatCell._HandleMidEffectShow(effectHandle, owner, assetEffect)
