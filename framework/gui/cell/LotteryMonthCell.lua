...
function LotteryMonthCell:FindObjs()
  ...
  self.year.gameObject:SetActive(not BranchMgr.IsJapan())
  self.month.gameObject:SetActive(not BranchMgr.IsJapan())
end
...
function LotteryMonthCell:SetPicture(picUrl)
  if self.isLoad then
    return
  end
  if picUrl == nil then
  ...
  local bytes = LotteryProxy.Instance:DownloadMagicPicFromUrl(picUrl)
  if bytes then
...
function LotteryMonthCell:OnCellDestroy()
  self:DestroyPicture()
end
