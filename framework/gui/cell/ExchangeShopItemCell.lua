...
local _ICON = {"item_5261", "item_140"}
...
function ExchangeShopItemCell:FindObjs()
  ...
  self.medalIcon = self:FindComponent("Medal", UISprite, self.extraDesc)
  self.contributionIcon = self:FindComponent("Contribution", UISprite, self.extraDesc)
  IconManager:SetItemIcon(_ICON[1], self.medalIcon)
  IconManager:SetItemIcon(_ICON[2], self.contributionIcon)
  self.extraMedalIcon = self:FindComponent("Medal", UISprite, self.extraGift)
  self.extraContributionIcon = self:FindComponent("Contribution", UISprite, self.extraGift)
...
function ExchangeShopItemCell:SetData(data)
  ...
  if data then
    ...
    if data.staticData then
      ...
      if "table" == type(limit) and 0 < #limit then
...
function ExchangeShopItemCell:UpdateLeftTime()
  ...
  if 0 < totalSec then
    ...
    self.leftTime.text = 0 < day and string.format(ZhString.QuotaCard_Surplus, day) or string.format(ZhString.MVPFightInfoBord_LeftTime, hour, min, sec)
  end
end
...
function ExchangeShopItemCell:SetBtnState()
  ...
  if staticData.ExchangeType == ExchangeShopProxy.EnchangeType.COINS then
    ...
    if 1 < #staticData.Item then
      ...
      IconManager:SetItemIcon(Table_Item[staticData.Item[1][1]].Icon, self.extraMedalIcon)
      IconManager:SetItemIcon(Table_Item[staticData.Item[2][1]].Icon, self.extraContributionIcon)
    end
  else
