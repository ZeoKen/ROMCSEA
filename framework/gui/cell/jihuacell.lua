...
jihuacell.ChooseColor = LuaColor(0.10588235294117647, 0.3686274509803922, 0.6941176470588235)
jihuacell.NormalColor = LuaColor(0.13333333333333333, 0.13333333333333333, 0.13333333333333333)
jihuacell.ChooseImgColor = LuaColor(0.12156862745098039, 0.4549019607843137, 0.7490196078431373)
jihuacell.NormalImgColor = LuaColor(0, 0, 0, 1)
...
function jihuacell:Init()
  ...
  self.Title2AwardBP = self:FindGO("AwardBP", self.Title2)
  self.Title2AwardBPCount = self:FindComponent("AwardBPCount", UILabel, self.Title2)
  self.Back1 = self:FindGO("Back1")
...
function jihuacell:setSelected(isSelected)
  ...
  if Slua.IsNull(self.expand) == false then
    ...
    self.Back2_UISprite.height = self.Back1_UISprite.height + self.description.height + 10
...
function jihuacell:SetData(data)
  ...
  if self.data and self.data.dwid and data.isGoodData ~= true then
    ...
    if itemData.AllTaskNumber then
      ...
      self.Content_Btn.gameObject.transform.localPosition = LuaGeometry.GetTempVector3(275.9, -39.7, 0)
    else
      self.State1.gameObject:SetActive(false)
      self.Content_Btn.gameObject.transform.localPosition = LuaGeometry.GetTempVector3(275.9, -13, 0)
    end
    self.gotoMode = itemData.gotomode
    self.finish = itemData.Finish or 0
    self.finishParams = itemData.Finish_Params or 0
    ...
    if itemData.Reward then
      ...
      if rewardTeamids and 0 < #rewardTeamids then
    ...
    if itemData.BattlePassExp and next(itemData.BattlePassExp) and itemData.BattlePassExp[1] then
      self.Title2AwardBP.gameObject:SetActive(true)
      self.Title2AwardBPCount.gameObject:SetActive(true)
      self.Title2AwardBPCount.text = "x" .. itemData.BattlePassExp[1]
    else
      self.Title2AwardBP.gameObject:SetActive(false)
      self.Title2AwardBPCount.gameObject:SetActive(false)
    end
    ...
    elseif data and data.isGoodData then
      ...
      if data.LimitNum ~= 0 then
        self.cellTitle2.text = string.format(ZhString.LimitPurchase, data.LimitNum)
      else
      ...
      self.NodeForShop_Lab_UILabel.text = FunctionNewRecharge.FormatMilComma(self.price)
