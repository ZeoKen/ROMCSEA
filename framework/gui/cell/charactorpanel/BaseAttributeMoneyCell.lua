...
function BaseAttributeMoneyCell:SetData(data)
  ...
  if itemData then
    ...
    elseif data.type == 1 then
      value = -data.num
      self.name.text = nameText .. ZhString.Charactor_Infomation_FuZhai_Acc
    end
    ...
    elseif name == "GARDEN" then
      ...
      self.value.text = value
    elseif name == "LUCKY_DICE" then
      value = Game.Myself and Game.Myself.data.userdata:Get(UDEnum.LUCKY_DICE) or 0
      self.value.text = value
      self.gameObject:SetActive(0 < value)
