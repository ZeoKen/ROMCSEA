...
function PersonalPhoto:GetOriginImage(pos_index, timestamp, progress_callback, success_callback, error_callback, is_keep_previous_callback, is_through_personalphotocallback, extension)
  if not is_through_personalphotocallback then
    ...
    if self:IsGettingO(pos_index) then
      return
    end
  ...
  if isLatestLocal then
    ...
    if not isDownloading then
      local downloadPath = ApplicationHelper.persistentDataPath .. "/" .. NetIngPersonalPhoto.Ins():GetTempDownloadPathOfLocal(photoID, true, extension)
      NetIngPersonalPhoto.Ins():Download(Game.Myself.data.id, pos_index, photoID, timestamp, function(x)
        local progressValue = x
        self.callback:FireProgress(pos_index, progressValue, true)
        if is_through_personalphotocallback and progress_callback ~= nil then
        ...
        if is_through_personalphotocallback and error_callback ~= nil then
...
function PersonalPhoto:GetThumbnail(pos_index, timestamp, progress_callback, success_callback, error_callback, is_keep_previous_callback, extension)
  ...
  if self:IsGettingT(pos_index) then
    return
  end
  ...
  if isLatestLocal then
    ...
    if not isDownloading and not self:IsGetingOForMakeT(pos_index) and not self:IsMakingTWhenGetThumbnail(pos_index) then
      local downloadPath = ApplicationHelper.persistentDataPath .. "/" .. NetIngPersonalPhoto.Ins():GetTempDownloadPathOfLocal(photoID, false, extension)
      NetIngPersonalPhoto.Ins():Download(Game.Myself.data.id, pos_index, photoID, timestamp, function(x)
        local progressValue = x
        ...
        PersonalPhotoHelper.Ins():GetOriginImage(pos_index, timestamp, nil, function(x, y)
...
function PersonalPhoto:SaveAndUpload(pos_index, bytes, timestamp, progress_callback, success_callback, error_callback)
  ...
  if self:IsSaving(pos_index) then
    ...
    m = m or 0
    m = m + 1
    self.isSaving[pos_index] = m
    LocalPersonalPhoto.Ins():SaveAsync(photoID, bytes, timestamp, function()
      local tempUploadRootPath = ApplicationHelper.persistentDataPath .. "/" .. NetIngPersonalPhoto.Ins():GetTempUploadRootPathOfLocal()
      if not FileHelper.ExistDirectory(tempUploadRootPath) then
        FileHelper.CreateDirectory(tempUploadRootPath)
      end
      local tempUploadPathOfLocal = ApplicationHelper.persistentDataPath .. "/" .. NetIngPersonalPhoto.Ins():GetTempUploadPathOfLocal(photoID)
      FileDirectoryHandler.WriteFile(tempUploadPathOfLocal, bytes, function()
        local n = self.isSaving[pos_index]
        n = n - 1
        self.isSaving[pos_index] = n
        if not BranchMgr.IsTW() then
          NetIngPersonalPhoto.Ins():FormUpload(pos_index, photoID, progress_callback, function()
          ...
          NetIngPersonalPhoto.Ins():Upload(Game.Myself.data.id, pos_index, photoID, progress_callback, function()
            NetIngPersonalPhoto.Ins():SetExist(Game.Myself.data.id, pos_index)
            FileHelper.DeleteFile(tempUploadPathOfLocal)
            if success_callback ~= nil then
              success_callback()
...
function PersonalPhoto:CheckExistOnServer(pos_index, exist_callback, error_callback, timestamp)
  NetIngPersonalPhoto.Ins():CheckExist(Game.Myself.data.id, pos_index, exist_callback, function()
    NetIngPersonalPhoto.Ins():CheckExist(Game.Myself.data.id, pos_index, exist_callback, error_callback, PhotoFileInfo.OldExtension, timestamp)
  end, PhotoFileInfo.Extension, timestamp)
end
...
function PersonalPhoto:ClearLocal(pos_index)
  local photoID = self:GetPhotoID(Game.Myself.data.id, pos_index)
  local downloadOPath = ApplicationHelper.persistentDataPath .. "/" .. NetIngPersonalPhoto.Ins():GetTempDownloadPathOfLocal(photoID, true)
  if FileHelper.ExistFile(downloadOPath) then
    FileHelper.DeleteFile(downloadOPath)
  end
  downloadOPath = ApplicationHelper.persistentDataPath .. "/" .. NetIngPersonalPhoto.Ins():GetTempDownloadPathOfLocal(photoID, true, PhotoFileInfo.OldExtension)
  if FileHelper.ExistFile(downloadOPath) then
    FileHelper.DeleteFile(downloadOPath)
  end
  local downloadTPath = ApplicationHelper.persistentDataPath .. "/" .. NetIngPersonalPhoto.Ins():GetTempDownloadPathOfLocal(photoID, false)
  if FileHelper.ExistFile(downloadTPath) then
    FileHelper.DeleteFile(downloadTPath)
  end
  downloadTPath = ApplicationHelper.persistentDataPath .. "/" .. NetIngPersonalPhoto.Ins():GetTempDownloadPathOfLocal(photoID, false, PhotoFileInfo.OldExtension)
  if FileHelper.ExistFile(downloadTPath) then
    FileHelper.DeleteFile(downloadTPath)
  end
  local tempUploadPathOfLocal = ApplicationHelper.persistentDataPath .. "/" .. NetIngPersonalPhoto.Ins():GetTempUploadPathOfLocal(photoID)
  ...
  local tempExtension = PhotoFileInfo.Extension
  PhotoFileInfo.Extension = PhotoFileInfo.OldExtension
  tempUploadPathOfLocal = ApplicationHelper.persistentDataPath .. "/" .. NetIngPersonalPhoto.Ins():GetTempUploadPathOfLocal(photoID)
  if FileHelper.ExistFile(tempUploadPathOfLocal) then
    FileHelper.DeleteFile(tempUploadPathOfLocal)
  end
  PhotoFileInfo.Extension = tempExtension
