...
function PersonalPhotoHelper:GetOriginImage(pos_index, timestamp, progress_callback, success_callback, error_callback, is_keep_previous_callback, is_through_personalphotocallback)
  ...
  if isLatestLocal then
    PersonalPhoto.Ins():GetOriginImage(pos_index, timestamp, progress_callback, success_callback, error_callback, is_keep_previous_callback, is_through_personalphotocallback)
  else
    if self.tabCachedParamsO[pos_index] == nil then
      self.tabCachedParamsO[pos_index] = {}
    end
    table.insert(self.tabCachedParamsO[pos_index], {
      timestamp = timestamp,
    ...
    if self.tabIsCheckingExistO[pos_index] == nil then
      NetIngPersonalPhoto.Ins():CheckExist(Game.Myself.data.id, pos_index, function()
        ...
        if self.tabStopFlagO[pos_index] == nil then
          ...
          for i = 1, #params do
            ...
            PersonalPhoto.Ins():GetOriginImage(pos_index, param.timestamp, param.progressCallback, param.successCallback, param.errorCallback, param.isKeepPreviousCallback, param.isThroughPersonalphotocallback, PhotoFileInfo.Extension)
          end
        end
        ...
        if self.tabStopFlagO[pos_index] == nil then
          ...
          for i = 1, #params do
            local param = params[i]
            PersonalPhoto.Ins():GetOriginImage(pos_index, param.timestamp, param.progressCallback, param.successCallback, param.errorCallback, param.isKeepPreviousCallback, param.isThroughPersonalphotocallback, PhotoFileInfo.OldExtension)
          end
        end
        self.tabCachedParamsO[pos_index] = nil
...
function PersonalPhotoHelper:GetThumbnail(pos_index, timestamp, progress_callback, success_callback, error_callback, is_keep_previous_callback)
  ...
  if isLatestLocal then
    PersonalPhoto.Ins():GetThumbnail(pos_index, timestamp, progress_callback, success_callback, error_callback, is_keep_previous_callback)
  else
    if self.tabCachedParamsT[pos_index] == nil then
      self.tabCachedParamsT[pos_index] = {}
    end
    table.insert(self.tabCachedParamsT[pos_index], {
      timestamp = timestamp,
      progressCallback = progress_callback,
      successCallback = success_callback,
      errorCallback = error_callback,
    ...
    if self.tabIsCheckingExistT[pos_index] == nil then
      NetIngPersonalPhoto.Ins():CheckExist(Game.Myself.data.id, pos_index, function()
        ...
        if self.tabStopFlagT[pos_index] == nil then
          ...
          for i = 1, #params do
            local param = params[i]
            PersonalPhoto.Ins():GetThumbnail(pos_index, param.timestamp, param.progressCallback, param.successCallback, param.errorCallback, param.isKeepPreviousCallback, PhotoFileInfo.Extension)
          end
        end
        ...
        if self.tabStopFlagT[pos_index] == nil then
          ...
          for i = 1, #params do
            local param = params[i]
            PersonalPhoto.Ins():GetThumbnail(pos_index, param.timestamp, param.progressCallback, param.successCallback, param.errorCallback, param.isKeepPreviousCallback, PhotoFileInfo.OldExtension)
          end
        end
        self.tabCachedParamsT[pos_index] = nil
      end, PhotoFileInfo.Extension)
