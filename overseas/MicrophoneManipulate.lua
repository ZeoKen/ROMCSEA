...
MicrophoneManipulate.EncodeMode = 0
...
function MicrophoneManipulate:StartRecord()
  self.recordingStartTime = UnityRealtimeSinceStartup
  ...
  self.preMicrophonePosition = 0
  self.processedSample = 0
  while 0 >= Microphone.GetPosition(nil) do
  end
...
function MicrophoneManipulate.CanSpeech()
  ...
  elseif BranchMgr.IsTW() then
    return not BackwardCompatibilityUtil.CompatibilityMode_Vspeech
  else
    return false
  end
end
